    ...
    if first_step or take_step:
        ...
        allreduced_accumulated_gradients = [
            ...
            if g is None
                ...
                if config.amp and config.fp16_compression
                else Compression.none,
            )
            for g in accumulator.gradients
        ]
        (clipped_accumulated_gradients, _) = tf.clip_by_global_norm(
            allreduced_accumulated_gradients, clip_norm=clip_norm
        )
        # Weight update
...
def main(e2e_start_time):
    ...
    parser.add_argument(
        "--log_freq", type=int, default=10, help="Training metrics logging frequency"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="run a fixed number of steps only",
    )
    ...
    parser.add_argument(
        "--lr_decay_power", type=float, default=0.5, help="LR decay power"
    )
    parser.add_argument(
        "--opt_beta_1", type=float, default=0.878, help="Optimizer beta1"
    )
    parser.add_argument(
        "--opt_beta_2", type=float, default=0.974, help="Optimizer beta2"
    )
    ...
    parser.add_argument(
        "--results_dir", type=str, default=None, help="Path to store all model results"
    )
    parser.add_argument(
        "--skip_checkpoint",
        action="store_true",
    ...
    iter_checkpoint = tf.train.Checkpoint(
        ...
        world_size=tf.Variable(get_world_size()),
        rank=tf.Variable(get_rank()),
    )
    iter_manager = tf.train.CheckpointManager(
        iter_checkpoint,
        os.path.join(
            config.checkpoints_dir, "iter_ckpt_rank_" + "{:02}".format(get_rank())
        ),
    ...
    while int(checkpoint.step) <= config.num_train_steps:
        ...
        if (
            int(checkpoint.step) == config.num_train_steps
            and (local_step % args.gradient_accumulation_steps == 0)
        ) or (
            (local_step + 1)
        ...
        if not config.skip_checkpoint and (
            ...
            if is_main_process():
                ...
                log(
                    " ** Saved model checkpoint for step {}: {}".format(step, save_path)
                )
        ...
        local_step += 1
        if config.steps_this_run != -1 and (
            local_step % (config.steps_this_run * args.gradient_accumulation_steps) == 0
        ):
            # terminating run sooner as steps_this_run has been reached
            log(
                "terminating as steps_this_run:{} has been reached".format(
                    config.steps_this_run
                )
            )
            break
