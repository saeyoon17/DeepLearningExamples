    ...
    if args.action == "download":
        ...
        if (
            ...
            for _dir in ["train", "test"]:
                if not os.path.exists(
                    directory_structure["sharded"] + "/" + args.dataset + "/" + _dir
                ):
                    os.makedirs(
                        directory_structure["sharded"] + "/" + args.dataset + "/" + _dir
                    )
        ...
        for _dir in ["train", "test"]:
            electra_preprocessing_command = (
                "python /workspace/electra/build_pretraining_dataset.py"
            )
            electra_preprocessing_command += (
                " --corpus-dir="
            ...
            electra_preprocessing_command += (
                " --do-lower-case" if args.do_lower_case else " --no-lower-case"
            )
            electra_preprocessing_command += " --max-seq-length=" + str(
                args.max_seq_length
            )
            ...
            electra_preprocessing_process = subprocess.Popen(
                electra_preprocessing_command, shell=True
            )
