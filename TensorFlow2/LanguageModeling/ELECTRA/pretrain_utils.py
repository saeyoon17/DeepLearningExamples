...
def get_dataset(config, batch_size, num_cpu_threads=4, world_size=1, rank=0):
    ...
    d = d.repeat()
    d = d.shuffle(
        buffer_size=len(input_files), seed=config.seed, reshuffle_each_iteration=False
    )

    cycle_length = min(num_cpu_threads, len(input_files))
    d = d.interleave(
        tf.data.TFRecordDataset, cycle_length=cycle_length, deterministic=True
    )
...
def sample_from_softmax(logits, disallow=None):
    ...
    gumbel_noise = tf.cast(
        -tf.math.log(-tf.math.log(uniform_noise + 1e-9) + 1e-9), logits.dtype
    )
    return tf.one_hot(
