...
from squad_utils import (RawResult, SquadResult, SquadV1Processor,
                         SquadV2Processor, get_answers,
                         squad_convert_examples_to_features)
from tokenization import ElectraTokenizer
from tqdm import tqdm
from utils import format_step, get_rank, get_world_size, is_main_process, log
...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir", default=None, type=str, required=True, help="Path to dataset."
    )
    parser.add_argument(
        "--output_dir",
        default=".",
    ...
    if not args.do_train and (
        not args.init_checkpoint or args.init_checkpoint == "None"
    ):
...
def get_dataset_from_features(
    features, batch_size, drop_remainder=True, ngpu=8, mode="train", v2=False
):
...
def train_step(
    ...
    with tf.GradientTape() as tape:
        ...
        if len(start_positions.shape) > 1:
            start_positions = tf.squeeze(
                start_positions, axis=-1, name="squeeze_start_positions"
            )
        if len(end_positions.shape) > 1:
            end_positions = tf.squeeze(
                end_positions, axis=-1, name="squeeze_end_positions"
            )
        if (
            is_impossible is not None
            and len(is_impossible.shape) > 1
            and v2
            and cls_logits is not None
        ):
            is_impossible = tf.squeeze(
                is_impossible, axis=-1, name="squeeze_is_impossible"
            )

        # sometimes the start/end positions are outside our model inputs, we ignore these terms
        ignored_index = start_logits.shape[1]
        start_positions = tf.clip_by_value(
            start_positions, 0, ignored_index, name="clip_start_positions"
        )
        end_positions = tf.clip_by_value(
            end_positions, 0, ignored_index, name="clip_end_positions"
        )

        start_loss = loss(
            y_true=start_positions, y_pred=tf.cast(start_logits, tf.float32)
        )
        end_loss = loss(y_true=end_positions, y_pred=tf.cast(end_logits, tf.float32))
        loss_value = (start_loss + end_loss) / 2

        if v2:
            cls_loss_value = loss_class(
                y_true=is_impossible, y_pred=tf.cast(cls_logits, tf.float32)
            )
...
def main():
    ...
    if args.vocab_file is None:
        tokenizer = ElectraTokenizer.from_pretrained(
            electra_model, cache_dir=args.cache_dir
        )
    else:
    ...
    train_examples = (
        processor.get_train_examples(args.data_dir) if args.do_train else None
    )
    dev_examples = (
        processor.get_dev_examples(args.data_dir) if args.do_predict else None
    )
    ...
    total_train_steps = (
        int((len_train_features * EPOCHS / BATCH_SIZE) / get_world_size()) + 1
    )
    train_steps_per_epoch = (
        int((len_train_features / BATCH_SIZE) / get_world_size()) + 1
    )
    ...
    train_dataset = (
        get_dataset_from_features(
            train_features, BATCH_SIZE, v2=args.version_2_with_negative
        )
        if args.do_train
        else []
    )
    ...
    for epoch in range(EPOCHS):
        if args.do_train:
            ...
            for iter, inputs in enumerate(epoch_iterator):
                ...
                if (
                    args.max_steps > 0
                    and (epoch * train_steps_per_epoch + iter) > args.max_steps
                ):
                    break
                ...
                epoch_perf_avg.update_state(
                    1.0 * BATCH_SIZE / (time.time() - iter_start)
                )
            ...
            if not args.skip_checkpoint:
                if args.ci:
                    ...
                    checkpoint_name = (
                        "checkpoints/electra_base_qa_v2_{}_epoch_{}_ckpt".format(
        ...
        if args.do_predict and (
            args.evaluate_during_training or epoch == args.num_train_epochs - 1
        ):
            if not args.do_train:
            ...
            if is_main_process():
                ...
                for (
                    ...
                    for iter_ in range(input_ids.shape[0]):
                        ...
                        if not args.joint_head:
                            ...
                            unique_id = int(dev_feature.unique_id)
                            output = [
                                output[iter_].numpy().tolist() for output in outputs
                            ]
                ...
                answers, nbest_answers = get_answers(
                    dev_examples, dev_features, all_results, args
                )
                ...
                if args.do_eval:
                    ...
                    log(
                        "Epoch: {:03d} Results: {}".format(
                            epoch, eval_out.decode("UTF-8")
                        )
                    )
                    log(
                ...
                log(
                    "**LATENCY SUMMARY** - Epoch: {:03d},  Ave: {:6.3f} ms, 90%: {:6.3f} ms, 95%: {:6.3f} ms, 99%: {:6.3f} ms".format(
                        ...
                        sum(latency_all[: int(len(latency_all) * 0.9)])
                        / int(len(latency_all) * 0.9)
                        * 1000,
                        sum(latency_all[: int(len(latency_all) * 0.95)])
                        / int(len(latency_all) * 0.95)
                        * 1000,
                        sum(latency_all[: int(len(latency_all) * 0.99)])
                        / int(len(latency_all) * 0.99)
                        * 1000,
                    )
                )
                dllogger.log(
                    step=tuple(),
                    data={
                        "inference_sequences_per_second": float(
                            infer_perf_avg.result().numpy()
                        ),
