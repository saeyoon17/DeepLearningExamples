...
def get_profiler_callback(
    model_dir, profile_steps, enable_tensorboard, steps_per_epoch
):
...
def set_gpu_thread_mode_and_count(
    gpu_thread_mode, datasets_num_private_threads, num_gpus, per_gpu_thread_count
):
    ...
    if not datasets_num_private_threads:
        ...
        logging.info(
            "Set datasets_num_private_threads to %s", datasets_num_private_threads
        )
