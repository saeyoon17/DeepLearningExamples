...
def config_benchmark_logger(flag_obj=None):
    ...
    try:
        ...
        if (
            not hasattr(flag_obj, "benchmark_logger_type")
            or flag_obj.benchmark_logger_type == "BaseBenchmarkLogger"
        ):
            ...
            from official.benchmark import \
                benchmark_uploader as bu  # pylint: disable=g-import-not-at-top
...
class BenchmarkFileLogger(BaseBenchmarkLogger):
    ...
    def log_run_info(self, model_name, dataset_name, run_params, test_id=None):
        ...
        with tf.io.gfile.GFile(
            os.path.join(self._logging_dir, BENCHMARK_RUN_LOG_FILE_NAME), "w"
        ) as f:
            try:
