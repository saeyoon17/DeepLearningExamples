...
class DistributedExecutor(object):
    ...
    def _save_config(self, model_dir):
        ...
        if model_dir:
            ...
            params_dict.save_params_dict_to_yaml(
                self._params, model_dir + "/params.yaml"
            )
        else:
        ...
        def _replicated_step(inputs):
            ...
            with tf.GradientTape() as tape:
                ...
                if isinstance(metric, tf.keras.metrics.Metric):
                    ...
                    logging.error(
                        "train metric is not an instance of " "tf.keras.metrics.Metric."
                    )
    ...
    def _create_train_step(self, strategy, model, loss_fn, optimizer, metric=None):
        ...
        """
        _replicated_step = self._create_replicated_step(
            strategy, model, loss_fn, optimizer, metric
        )
    ...
    def train(
        ...
        with strategy.scope():
            ...
            if latest_checkpoint_file:
                ...
                logging.info(
                    "Loading from checkpoint file completed. Init step %d", initial_step
                )
        ...
        while current_step < total_steps:
            ...
            if (
                ...
                and (current_step - last_save_checkpoint_step) >= save_freq
            ):
                _save_checkpoint(
    ...
    def _run_evaluation(self, test_step, current_training_step, metric, test_iterator):
        ...
        logging.info(
            "Step: [%d] Validation metric = %f", current_training_step, metric_result
        )
    ...
    def evaluate_from_model_dir(
        ...
        def terminate_eval():
            tf.logging.info(
                "Terminating eval after %d seconds of no checkpoints" % eval_timeout
            )
            return True
