...
from official.utils.misc import distribution_utils, tpu_lib
...
def run_customized_training_loop(
    ...
    required_arguments = [model_fn, loss_fn, model_dir, steps_per_epoch, train_input_fn]
    if [arg for arg in required_arguments if arg is None]:
        raise ValueError(
            "`model_fn`, `loss_fn`, `model_dir`, "
    ...
    with distribution_utils.get_strategy_scope(strategy):
        ...
        use_float16 = isinstance(optimizer, tf.keras.mixed_precision.LossScaleOptimizer)
        ...
        def _replicated_step(inputs, first_batch=False):
            ...
            if hvd:
                tape = hvd.DistributedGradientTape(
                    ...
                    compression=Compression.fp16 if use_float16 else Compression.none,
                )
        ...
        sub_model_checkpoint = (
            tf.train.Checkpoint(model=sub_model) if sub_model_export_name else None
        )
        ...
        while current_step < total_training_steps:
            ...
            if (
                not hvd or hvd.rank() == 0
            ) and steps_from_save >= FLAGS.save_checkpoint_steps:
        ...
        if not hvd or hvd.rank() == 0:
            ...
            if sub_model_export_name:
                _save_checkpoint(
                    sub_model_checkpoint, model_dir, "%s.ckpt" % sub_model_export_name
                )
            ...
            logging.info(
                "Total Training Time = %0.2f for Sequences = %d",
                total_time,
                total_sentences,
            )
            if total_time != 0:
                logging.info(
                    "Throughput Average (sequences/sec) with overhead = %0.2f",
                    total_sentences / total_time,
                )
            if perf_wo_n != 0:
                logging.info(
                    "Throughput Average (sequences/sec) = %0.2f", perf_wo / perf_wo_n
                )
            logging.info("-----------------------------")
            ...
            if dllogging and perf_wo_n != 0:
                dllogging.logger.log(
                    step=(),
                    data={"throughput_train": perf_wo / perf_wo_n},
                    verbosity=Verbosity.DEFAULT,
                )
                dllogging.logger.log(
                    step=(),
                    data={"total_loss": training_summary["train_loss"]},
                    verbosity=Verbosity.DEFAULT,
                )
