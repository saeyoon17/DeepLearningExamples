...
class SequenceBeamSearch(object):
    ...
    def _create_initial_state(self, initial_ids, initial_cache):
        ...
        initial_log_probs = tf.constant(
            [[0.0] + [-float("inf")] * (self.beam_size - 1)], dtype=self.dtype
        )
        ...
        finished_scores = tf.ones(
            [self.batch_size, self.beam_size], dtype=self.dtype
        ) * -inf(self.dtype)
        ...
        if self.padded_decode:
            state_shape_invariants = {
                ...
                _StateKeys.ALIVE_LOG_PROBS: tf.TensorShape(
                ...
                _StateKeys.FINISHED_SCORES: tf.TensorShape(
                    [self.batch_size, self.beam_size]
                ),
                _StateKeys.FINISHED_FLAGS: tf.TensorShape(
                    [self.batch_size, self.beam_size]
                ),
            }
        else:
    ...
    def _continue_search(self, state):
        ...
        lowest_finished_scores += (1.0 - tf.cast(finished_batches, self.dtype)) * -inf(
            self.dtype
        )
    ...
    def _search_step(self, state):
        ...
        alive_state = self._get_new_alive_state(
            new_seq, new_log_probs, new_finished_flags, new_cache
        )

        # Combine newly finished sequences with existing finished sequences, and
        # collect the top k scoring sequences.
        finished_state = self._get_new_finished_state(
            state, new_seq, new_log_probs, new_finished_flags
        )
    ...
    def _get_new_alive_state(
        self, new_seq, new_log_probs, new_finished_flags, new_cache
    ):
    ...
    def _get_new_finished_state(
        self, state, new_seq, new_log_probs, new_finished_flags
    ):
...
def sequence_beam_search(
    ...
    """
    batch_size = (
        initial_ids.shape.as_list()[0] if padded_decode else tf.shape(initial_ids)[0]
    )
