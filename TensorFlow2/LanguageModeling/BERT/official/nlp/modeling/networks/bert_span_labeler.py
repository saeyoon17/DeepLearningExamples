...
class BertSpanLabeler(tf.keras.Model):
    ...
    def __init__(
        self, network, initializer="glorot_uniform", output="logits", **kwargs
    ):
        ...
        # evaluate calls.
        start_logits = tf.keras.layers.Lambda(tf.identity, name="start_positions")(
            start_logits
        )
        end_logits = tf.keras.layers.Lambda(tf.identity, name="end_positions")(
            end_logits
        )
