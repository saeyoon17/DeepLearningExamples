...
class TransformerLayerTest(keras_parameterized.TestCase):
    ...
    def test_layer_invocation(self):
        ...
        batch_size = 6
        input_data = 10 * np.random.random_sample((batch_size, sequence_length, width))
    ...
    def test_layer_invocation_with_float16_dtype(self):
        ...
        # Create a 3-dimensional input (the first dimension is implicit).
        data_tensor = tf.keras.Input(shape=(sequence_length, width), dtype=tf.float16)
        # Create a 2-dimensional input (the first dimension is implicit).
        mask_tensor = tf.keras.Input(shape=(sequence_length, sequence_length))
        output_tensor = test_layer([data_tensor, mask_tensor])
        ...
        batch_size = 6
        input_data = (
            10 * np.random.random_sample((batch_size, sequence_length, width))
        ).astype(np.float16)
        # The attention mask should be of shape (batch, from_seq_len, to_seq_len),
        # which here is (batch, sequence_length, sequence_length)
        mask_data = np.random.randint(
    ...
    def test_layer_restoration_from_config(self):
        ...
        new_call_list = new_model.get_layer(
            name="transformer_scaffold"
        )._attention_layer.list
