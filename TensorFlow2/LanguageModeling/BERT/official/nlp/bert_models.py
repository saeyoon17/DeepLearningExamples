...
from official.nlp.modeling import losses, networks
from official.nlp.modeling.networks import (bert_classifier, bert_pretrainer,
                                            bert_span_labeler)
...
class BertPretrainLossAndMetricLayer(tf.keras.layers.Layer):
    ...
    def _add_metrics(
        ...
        self.add_metric(
            next_sentence_accuracy, name="next_sentence_accuracy", aggregation="mean"
        )
...
def pretrain_model(
    bert_config, seq_length, max_predictions_per_seq, float_type, initializer=None
):
    ...
    masked_lm_weights = tf.keras.layers.Input(
        shape=(max_predictions_per_seq,), name="masked_lm_weights", dtype=tf.int32
    )
...
class BertSquadLogitsLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        final_hidden_input = tf.keras.backend.reshape(
            sequence_output, [-1, num_hidden_units]
        )
...
def squad_model(
    bert_config, max_seq_length, float_type, initializer=None, hub_module_url=None
):
    ...
    if not hub_module_url:
        ...
        return (
            bert_span_labeler.BertSpanLabeler(
                network=bert_encoder, initializer=initializer
            ),
            bert_encoder,
        )
...
def classifier_model(
    ...
    output = tf.keras.layers.Dense(
        num_labels, kernel_initializer=initializer, name="output", dtype=float_type
    )(output)
    return (
        tf.keras.Model(
            inputs={
