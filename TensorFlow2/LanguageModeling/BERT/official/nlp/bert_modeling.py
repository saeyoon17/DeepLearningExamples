...
class AlbertConfig(BertConfig):
    ...
    def __init__(
        self, embedding_size, num_hidden_groups=1, inner_group_num=1, **kwargs
    ):
...
class EmbeddingLookup(tf.keras.layers.Layer):
    ...
    def __init__(
        self, vocab_size, embedding_size=768, initializer_range=0.02, **kwargs
    ):
    ...
    def call(self, inputs):
        ...
        if self.use_position_embeddings:
            position_embeddings = tf.expand_dims(
                tf.slice(self.position_embeddings, [0, 0], [seq_length, width]), axis=0
            )
...
class Attention(tf.keras.layers.Layer):
    ...
    def reshape_to_matrix(self, input_tensor):
        ...
        if ndims < 2:
            raise ValueError(
                "Input tensor must have at least rank 2."
