...
def main(args):
    ...
    if args.action == "download":
        ...
        if args.dataset == "bookscorpus":
            ...
            if args.skip_wikiextractor == 0:
                path_to_wikiextractor_in_container = (
                    "/workspace/wikiextractor/WikiExtractor.py"
                )
                wikiextractor_command = (
                    path_to_wikiextractor_in_container
                    + " "
            ...
            output_filename = (
                directory_structure["formatted"]
                + "/pubmed_baseline_one_article_per_line.txt"
            )
            pubmed_formatter = PubMedTextFormatting.PubMedTextFormatting(
                pubmed_path, output_filename, recursive=True
            )
        ...
        if (
            ...
            if not os.path.exists(
                directory_structure["sharded"] + "/" + args.dataset + "/test"
            ):
                os.makedirs(
                    directory_structure["sharded"] + "/" + args.dataset + "/test"
                )
        ...
        if not os.path.exists(
            directory_structure["tfrecord"] + "/" + args.dataset + "/training"
        ):
            os.makedirs(
                directory_structure["tfrecord"] + "/" + args.dataset + "/training"
            )
        ...
        if not os.path.exists(
            directory_structure["tfrecord"] + "/" + args.dataset + "/test"
        ):
        ...
        def create_record_worker(
            filename_prefix, shard_id, output_format="tfrecord", split="training"
        ):
            bert_preprocessing_command = (
                "python /workspace/bert_tf2/create_pretraining_data.py"
            )
            bert_preprocessing_command += (
                " --input_file="
            ...
            bert_preprocessing_command += (
                " --do_lower_case" if args.do_lower_case else ""
            )
        ...
        for i in range(args.n_training_shards):
            last_process = create_record_worker(
                output_file_prefix + "_training", i, "tfrecord", "training"
            )
        ...
        for i in range(args.n_test_shards):
            last_process = create_record_worker(
                output_file_prefix + "_test", i, "tfrecord", "test"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dupe_factor", type=int, help="Specify the duplication factor", default=5
    )
