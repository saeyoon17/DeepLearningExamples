...
flags.DEFINE_string(
    "input_files", None, "File path to retrieve training data for pre-training."
)
...
flags.DEFINE_integer(
    "max_predictions_per_seq", 20, "Maximum predictions per sequence_output."
)
flags.DEFINE_integer("train_batch_size", 32, "Total batch size for training.")
flags.DEFINE_integer(
    "num_steps_per_epoch", 1000, "Total number of training steps to run per epoch."
)
flags.DEFINE_float(
    "warmup_steps", 10000, "Warmup steps for Adam weight decay optimizer."
)
...
def get_pretrain_dataset_fn(
    input_file_pattern, seq_length, max_predictions_per_seq, global_batch_size
):
...
def run_customized_training(
    ...
    train_input_fn = get_pretrain_dataset_fn(
        input_files, max_seq_length, max_predictions_per_seq, train_batch_size
    )
