...
from official.utils.misc import distribution_utils, keras_utils, tpu_lib
...
def squad_loss_fn(
    start_positions, end_positions, start_logits, end_logits, loss_factor=1.0
):
...
def get_dataset_fn(
    input_file_pattern, max_seq_length, global_batch_size, is_training, use_horovod
):
    ...
    def _dataset_fn(ctx=None):
        ...
        batch_size = (
            ctx.get_per_replica_batch_size(global_batch_size)
            if ctx
...
def predict_squad_customized(
    strategy, input_meta_data, bert_config, predict_tfrecord_path, num_steps
):
    ...
    if strategy:
        predict_iterator = iter(
            strategy.experimental_distribute_datasets_from_function(predict_dataset_fn)
        )
    else:
        predict_iterator = iter(predict_dataset_fn())

    if FLAGS.mode == "trt_predict":
        squad_model = tf_trt.TFTRTModel(
            FLAGS.savedmodel_dir, "amp" if FLAGS.use_fp16 else "fp32"
        )

    elif FLAGS.mode == "sm_predict":
        squad_model = tf_trt.SavedModel(
            FLAGS.savedmodel_dir, "amp" if FLAGS.use_fp16 else "fp32"
        )
    ...
    if FLAGS.benchmark:
        ...
        logging.info(
            "Throughput Average (sequences/sec) = %0.2f", ss_sentences_per_second
        )
...
def train_squad(strategy, input_meta_data, custom_callbacks=None, run_eagerly=False):
    ...
    if strategy:
        logging.info(
            "Training using customized training loop with distribution" " strategy."
        )
    ...
    loss_fn = get_loss_fn(
        ...
        if FLAGS.scale_loss and strategy
        else 1.0
    )
...
def predict_squad(strategy, input_meta_data):
    ...
    eval_writer = squad_lib.FeatureWriter(
        filename=os.path.join(FLAGS.model_dir, "eval.tf_record"), is_training=False
    )
    eval_features = []
    ...
    all_results = predict_squad_customized(
        strategy, input_meta_data, bert_config, eval_writer.filename, num_steps
    )
...
dynamic_batching {{
    ...
    config_values = {
        ...
        "gpu_list": ", ".join(
            [x.name.split(":")[-1] for x in tf.config.list_physical_devices("GPU")]
        ),
