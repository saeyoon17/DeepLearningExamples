...
def generate_detections_per_image_tpu(
    ...
    box_outputs = tf.reshape(box_outputs, [num_boxes, num_classes, 4])[
        :, 1:num_classes, :
    ]
...
def generate_detections_gpu(
    ...
    with tf.name_scope("generate_detections"):
        ...
        anchor_boxes = tf.expand_dims(anchor_boxes, axis=2) * tf.ones(
            [1, 1, num_classes - 1, 1]
        )
    ...
    return (
        post_nms_num_valid_boxes,
        post_nms_boxes,
        tf.cast(post_nms_classes, dtype=tf.float32),
        post_nms_scores,
    )
