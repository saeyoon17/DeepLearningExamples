...
from mrcnn_tf2.runtime.callbacks import (DLLoggerMetricsCallback,
                                         DLLoggerPerfCallback,
                                         PretrainedWeightsLoadingCallback)
...
def run_training(dataset, params):
    ...
    mask_rcnn_model.fit(
        ...
        or (dataset.train_size // params.global_train_batch_size),
...
def create_model(params):
    model = MaskRCNN(params=params, trainable="train" in params.mode)
...
def create_callbacks(params):
    yield DLLoggerMetricsCallback(dllogger=dllogger, log_every=params.log_every)
    ...
    if params.log_tensorboard:
        yield tf.keras.callbacks.TensorBoard(
            log_dir=params.log_tensorboard, update_freq="batch"
        )
