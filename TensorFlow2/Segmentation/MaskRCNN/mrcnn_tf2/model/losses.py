...
class MaskRCNNLoss(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        (
            ...
            mask_height,
            mask_width,
        ) = mask_outputs.get_shape().as_list()

        weights = tf.tile(
            tf.reshape(
                tf.greater(select_class_targets, 0), [batch_size, num_masks, 1, 1]
            ),
...
class FastRCNNLoss(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        box_outputs = tf.reshape(
            box_outputs, [batch_size, num_rois, self._num_classes, 4]
        )

        box_indices = tf.reshape(
            class_targets
            + tf.tile(
                tf.expand_dims(tf.range(batch_size) * num_rois * self._num_classes, 1),
                [1, num_rois],
            )
            + tf.tile(
                tf.expand_dims(tf.range(num_rois) * self._num_classes, 0),
                [batch_size, 1],
            ),
            [-1],
        )
        ...
        for level in range(int(self._min_level), int(self._max_level + 1)):
            ...
            score_losses.append(
                _rpn_score_loss(
                    ...
                    normalizer=tf.cast(
                        self._batch_size * self._rpn_batch_size_per_im, dtype=tf.float32
                    ),
                )
            )
...
def _sigmoid_cross_entropy(
    multi_class_labels, logits, weights, sum_by_non_zeros_weights=False
):

    assert weights.dtype == tf.float32

    sigmoid_cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(
        labels=multi_class_labels, logits=logits, name="x-entropy"
    )
    ...
    if sum_by_non_zeros_weights:
        ...
        sigmoid_cross_entropy = tf.math.divide_no_nan(
            sigmoid_cross_entropy, num_non_zeros, name="sum_by_non_zeros_weights"
        )
...
def _softmax_cross_entropy(onehot_labels, logits):
    ...
    softmax_cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
        labels=tf.stop_gradient(onehot_labels), logits=logits
    )
    ...
    softmax_cross_entropy = tf.math.divide_no_nan(
        softmax_cross_entropy, num_non_zeros, name="softmax_cross_entropy"
    )
...
def _rpn_box_loss(box_outputs, box_targets, normalizer=1.0, delta=1.0 / 9):
    ...
    with tf.name_scope("rpn_box_loss"):
        ...
        box_loss = _huber_loss(
            y_true=box_targets, y_pred=box_outputs, weights=mask, delta=delta
        )
...
def _fast_rcnn_class_loss(class_outputs, class_targets_one_hot, normalizer=1.0):
    ...
    with tf.name_scope("fast_rcnn_class_loss"):
        ...
        class_loss = _softmax_cross_entropy(
            onehot_labels=class_targets_one_hot, logits=class_outputs
        )
...
def _fast_rcnn_box_loss(
    box_outputs, box_targets, class_targets, normalizer=1.0, delta=1.0
):
    """Computes box regression loss."""
    # delta is typically around the mean value of regression target.
    # for instances, the regression targets of 512x512 input with 6 anchors on
