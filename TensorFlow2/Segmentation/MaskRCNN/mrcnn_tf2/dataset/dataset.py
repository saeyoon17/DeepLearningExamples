...
class Dataset:
    ...
    def __init__(self, params):
        ...
        self._train_files = glob.glob(
            os.path.join(self._params.data_dir, TRAIN_SPLIT_PATTERN)
        )
        self._eval_files = glob.glob(
            os.path.join(self._params.data_dir, EVAL_SPLIT_PATTERN)
        )
    ...
    def train_fn(self, batch_size):
        ...
        data = data.cache()
        data = data.shuffle(
            buffer_size=4096, reshuffle_each_iteration=True, seed=self._params.seed
        )
        data = data.repeat()
    ...
    def eval_fn(self, batch_size):
        ...
        if self._params.eval_samples:
            self._logger.info(
                f"Amount of samples limited to {self._params.eval_samples}"
            )
