...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_eval", action="store_true", help="Disables evaluation after training."
    )
    ...
    checkpoint_path = os.path.join(
        flags.weights_dir, "rn50_tf_amp_ckpt_v20.06.0/nvidia_rn50_tf_amp"
    )
    ...
    if flags.gpus is not None:
        cmd_train = (
            f'CUDA_VISIBLE_DEVICES={",".join(map(str, range(flags.gpus)))} ' + cmd_train
        )
