...
def iou(y_true, y_pred, smooth=1.0e-9):
    """
...
def focal_loss(y_true, y_pred):
    ...
    weight = tf.multiply(y_true_c, tf.pow(tf.subtract(1.0, model_out), gamma))
    fl = tf.multiply(alpha, tf.multiply(weight, ce))
    reduced_fl = tf.reduce_max(fl, axis=-1)
    return tf.reduce_mean(reduced_fl)


def ssim_loss(y_true, y_pred, smooth=1.0e-9):
    """
...
class DiceCoefficient(tf.keras.metrics.Metric):
    ...
    def __init__(self, post_processed: bool, classes: int, name="dice_coef", **kwargs):
        """
        ...
        self.dice_value = self.add_weight(
            name="dice_value",
            initializer="zeros",
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        if self.post_processed:
            if self.classes == 1:
                y_true_ = y_true
                y_pred_ = tf.where(y_pred > 0.5, 1.0, 0.0)
            else:
    ...
    def dice_coef(self, y_true, y_pred, smooth=1.0e-9):
        """
