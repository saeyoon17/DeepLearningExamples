...
def check_image_and_mask(cfg, mode):
    ...
    """
    images_path = join_paths(cfg.WORK_DIR, cfg.DATASET[mode].IMAGES_PATH)
    mask_path = join_paths(cfg.WORK_DIR, cfg.DATASET[mode].MASK_PATH)
    ...
    for image in tqdm(all_images):
        ...
        if not (
            os.path.exists(join_paths(images_path, image))
            and os.path.exists(join_paths(mask_path, mask_name))
        ):
