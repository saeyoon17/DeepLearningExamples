...
from tensorflow.compat.v1.saved_model import signature_constants, tag_constants
...
def export_model(model_dir, prec, tf_trt_model_dir=None):
    ...
    dummy_input = tf.constant(
        tf.zeros(input_shape, dtype=tf.float32 if prec == "fp32" else tf.float16)
    )
...
class TFTRTModel:
    def __init__(self, model_dir, precision, output_tensor_name="output_1"):
        ...
        saved_model_loaded = tf.saved_model.load(
            temp_tftrt_dir, tags=[tag_constants.SERVING]
        )
