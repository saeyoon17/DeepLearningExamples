...
import yaml
from runtime.runner_utils import (build_stats, get_dataset_builders,
                                  get_learning_rate_params, get_metrics,
                                  get_optimizer_params, preprocess_image_files,
                                  train_step)
...
from utils.tf_utils import get_num_flops, get_num_params
...
class Runner(object):
    def __init__(self, flags, logger):
        ...
        try:
            ...
            print(
                ...
                # ======================================================#
                  """.format(
                    params, flops, img_size, img_size
                )
            )
        except:
            print(
    ...
    def train(self):
        ...
        global_batch_size_tr = (
            self.params.train_batch_size * hvd.size() * self.params.grad_accum_steps
        )
        train_epochs = self.params.max_epochs
        assert (
            train_epochs >= self.params.n_stages
        ), "each training stage requires at least 1 training epoch"
        train_steps = (
            self.params.steps_per_epoch
            if self.params.steps_per_epoch
            else self.params.train_num_examples
            // global_batch_size_tr
        ...
        validation_steps = (
            self.params.eval_num_examples // global_batch_size_eval
            if "eval" in self.params.mode
            else None
        )

        # set up lr schedule
        learning_rate = optimizer_factory.build_learning_rate(
            params=get_learning_rate_params(
                name=self.params.lr_decay,
        ...
        if self.params.grad_accum_steps > 1:
            ...
            optimizer = hvd.DistributedOptimizer(
                ...
                if self.params.hvd_fp16_compression
        ...
        callbacks += custom_callbacks.get_callbacks(
            ...
            batch_size=global_batch_size_tr
            / self.params.grad_accum_steps,  # throughput calc: 1 batch is 1 step
            log_steps=self.params.log_steps,
            model_dir=self.params.model_dir,
            save_checkpoint_freq=train_steps
            * self.params.save_checkpoint_freq,  # conditioned on batch index
        ...
        if not n_stages or n_stages == 1:
            ...
            ram_list = np.linspace(
                base_randaug_mag, self.params.raug_magnitude, n_stages
            )
            ...
            for stage in range(resumed_stage, n_stages):
                ...
                image_size = int(
                    base_img_size + (target_img_size - base_img_size) * ratio
                )
        ...
        if not self.params.skip_eval and self.validation_builder is not None:
            eval_callbacks.append(
                custom_callbacks.EvalTimeHistory(
                    batch_size=self.params.eval_batch_size, logger=self.logger
                )
            )
            ...
            build_stats(
                ...
                eval_callbacks,
                self.logger,
                comment="eval using original weights",
            )
        else:
            build_stats(
                history,
        ...
        if self.params.moving_average_decay > 0:
            ...
            if not self.params.skip_eval and self.validation_builder is not None:
                ...
                eval_callbacks.append(
                    custom_callbacks.MovingAverageCallback(
                ...
                build_stats(
                    history,
                    ema_validation_output,
                    callbacks,
                    eval_callbacks,
                    self.logger,
                    comment="eval using EMA weights",
                )
        ...
        if hvd.rank() == 0 and self.params.export_SavedModel:
            if not self.params.skip_eval and self.validation_builder is not None:
                ...
                if self.params.moving_average_decay > 0 and float(
                    ema_validation_output[1]
                ) > float(validation_output[1]):
                    self.ema_opt = optimizer_factory.fetch_optimizer(
                        self.model, optimizer_factory.MovingAverage
                    )
                    self.ema_opt.swap_weights()
            self.model.save(
                self.params.model_dir + "/savedmodel",
    ...
    def evaluate(self):
        ...
        if self.params.weights_format == "saved_model":
            self.model = tf.keras.models.load_model(
                self.params.model_dir + "/savedmodel",
                custom_objects={"HvdMovingAverage": optimizer_factory.HvdMovingAverage},
            )
            ...
            resumed_step = self.load_model_weights(
                self.params.model_dir, expect_partial=True
            )
            if resumed_step > 0:
                print(
                    """
        ...
        if (
            self.params.moving_average_decay > 0
            and self.params.weights_format != "saved_model"
        ):
            # =================================================
            # Define Datasets
            # =================================================
        ...
        for round_num in range(self.params.n_repeat_eval):
            ...
            if not self.params.skip_eval and self.validation_builder is not None:
                ...
                build_stats(
                    None,
                    worker_validation_output,
                    None,
                    eval_callbacks,
                    self.logger,
                    comment="eval using original weights: Round {} ".format(round_num),
                )

        if (
            self.params.export_SavedModel
            and self.params.weights_format != "saved_model"
        ):
            if self.params.moving_average_decay > 0 and float(
                ema_worker_validation_output[1]
            ) > float(worker_validation_output[1]):
                self.ema_opt = optimizer_factory.fetch_optimizer(
                    self.model, optimizer_factory.MovingAverage
                )
                self.ema_opt.swap_weights()
    ...
    def predict(self, img_dir, checkpoint_path=None, print_results=True):
        ...
        try:
            ...
            images = preprocess_image_files(
                img_dir,
                self.params.predict_img_size,
                self.params.predict_batch_size,
                dtype,
            )
            ...
            score = tf.nn.softmax(
                tf.convert_to_tensor(inference_results, dtype=tf.float32), axis=1
            )
            ...
            if print_results:
                for i, name in enumerate(file_names):
                    print(
                        "This {} image most likely belongs to {} class with a {} percent confidence.".format(
                            name,
                            tf.math.argmax(score[i]),
                            100 * tf.math.reduce_max(score[i]),
                        )
                    )
