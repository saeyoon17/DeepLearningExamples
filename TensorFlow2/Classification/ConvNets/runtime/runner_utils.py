...
from utils import callbacks, hvd_utils

__all__ = [
    "get_optimizer_params",
    "get_metrics",
    "get_learning_rate_params",
    "build_model_params",
    "get_models",
    "build_augmenter_params",
    "get_image_size_from_model",
    "get_dataset_builders",
    "build_stats",
    "parse_inference_input",
    "preprocess_image_files",
]


def get_optimizer_params(
    name, decay, epsilon, momentum, moving_average_decay, nesterov, beta_1, beta_2
):
    return {
        "name": name,
        "decay": decay,
...
def get_learning_rate_params(name, initial_lr, decay_epochs, decay_rate, warmup_epochs):
    return {
        "name": name,
...
def get_dataset_builders(params, one_hot, hvd_size=None):
    ...
    if "train" in params.mode:
        ...
        train_dataset_builder = dataset_factory.Dataset(
            ...
            augmenter_params=build_augmenter_params(
                params.augmenter_name,
                params.cutout_const,
    ...
    if "eval" in params.mode:
        ...
        validation_dataset_builder = dataset_factory.Dataset(
            data_dir=params.data_dir,
            index_file_dir=params.index_file,
            split="validation",
            num_classes=params.num_classes,
...
def build_stats(
    history, validation_output, train_callbacks, eval_callbacks, logger, comment=""
):
    stats = {}
    ...
    if history and history.history:
        ...
        stats["training_loss"] = float(
            hvd.allreduce(
                tf.constant(train_hist["loss"][-1], dtype=tf.float32), average=True
            )
        )
        ...
        if "categorical_accuracy" in train_hist:
            ...
            stats["training_accuracy_top_1"] = float(
                hvd.allreduce(
                    tf.constant(
                        train_hist["sparse_categorical_accuracy"][-1], dtype=tf.float32
                    ),
                    average=True,
                )
            )
            ...
            stats["training_accuracy_top_1"] = float(
                hvd.allreduce(
                    tf.constant(train_hist["accuracy"][-1], dtype=tf.float32),
                    average=True,
                )
            )
            stats["training_accuracy_top_5"] = float(
                hvd.allreduce(
                    tf.constant(train_hist["top_5_accuracy"][-1], dtype=tf.float32),
                    average=True,
                )
            )
    ...
    if eval_callbacks:
        for eval_callback in eval_callbacks:
            ...
            stats["avg_time_per_exp_eval"] = 1000.0 / stats["avg_exp_per_second_eval"]
    ...
    if self.local_step % grad_accum_steps == 0:
        ...
        gradients = [
            ...
            if g is None
                ...
                g / tf.cast(grad_accum_steps, g.dtype),
                compression=hvd.Compression.fp16
                if use_amp and hvd_fp16_compression
                else hvd.Compression.none,
            )
            for g in self.grad_accumulator.gradients
        ]
        if grad_clip_norm > 0:
            (gradients, gradients_gnorm) = tf.clip_by_global_norm(
                gradients, clip_norm=grad_clip_norm
            )
            self.gradients_gnorm.assign(
                gradients_gnorm
            )  # this will later appear on tensorboard
