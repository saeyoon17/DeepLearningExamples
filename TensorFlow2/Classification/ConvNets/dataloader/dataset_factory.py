...
from dataloader import Dali, augment, preprocessing
...
def mixup(batch_size, alpha, images, labels, defer_img_mixing):
    ...
    labels_mix = labels * labels_weight + labels[::-1] * (1.0 - labels_weight)
    if not defer_img_mixing:
        images_mix = images * img_weight + images[::-1] * (1.0 - img_weight)
    else:
...
def cutmix(images, labels, masks, defer_img_mixing):
    ...
    mixed_label = (1.0 - mix_area) * labels + mix_area * labels[::-1]
    masks = tf.cast(masks, images.dtype)
    if not defer_img_mixing:
        mixed_images = (1.0 - masks) * images + masks * images[::-1]
    else:
        # postpone image mixing
        mixed_images = images
...
def mixing(batch_size, mixup_alpha, cutmix_alpha, defer_img_mixing, features, labels):
    ...
    if mixup_alpha and cutmix_alpha:
        ...
        bs = batch_size // 2
        (img1, mix_weights), lab1 = mixup(
            bs, mixup_alpha, image[:bs], label[:bs], defer_img_mixing
        )
        (img2, mix_masks), lab2 = cutmix(
            image[bs:], label[bs:], mix_masks[bs:], defer_img_mixing
        )
        ...
        # only mixup
        (image, mix_weights), label = mixup(
            batch_size, mixup_alpha, image, label, defer_img_mixing
        )
...
def mixing_lite(images, mixup_weights, cutmix_masks, batch_size, do_mixup, do_cutmix):
    ...
    if do_mixup and do_cutmix:
        # split the batch half-half, and apply mixup and cutmix for each half.
        bs = batch_size // 2
        images_mixup = images[:bs] * mixup_weights + images[:bs][::-1] * (
            1.0 - mixup_weights
        )
        images_cutmix = (
            images[bs:] * (1.0 - cutmix_masks) * +images[bs:][::-1] * cutmix_masks
        )
        ...
        return images * mixup_weights + images[::-1] * (1.0 - mixup_weights)

    elif do_cutmix:
        return images * (1.0 - cutmix_masks) + images[::-1] * cutmix_masks
...
class Dataset:
    ...
    def dtype(self) -> tf.dtypes.DType:
        ...
        try:
            ...
            raise ValueError(
                "{} provided key. Invalid DType provided. Supported types: {}".format(
                    self._dtype, dtype_map.keys()
                )
            )
    ...
    def set_shapes(self, batch_size, features, labels):
        ...
        features["image"].set_shape(
            features["image"]
            .get_shape()
            .merge_with(tf.TensorShape([batch_size, None, None, None]))
        )
        labels["label"].set_shape(
            labels["label"].get_shape().merge_with(tf.TensorShape([batch_size, None]))
        )
    ...
    def build(self) -> tf.data.Dataset:
        ...
        if self._use_dali:
            print(
                "Using dali for {train} dataloading".format(
                    train="training" if self.is_training else "validation"
                )
            )
            tfrec_filenames = sorted(
                tf.io.gfile.glob(os.path.join(self._data_dir, "%s-*" % self._split))
            )
            tfrec_idx_filenames = sorted(
                tf.io.gfile.glob(os.path.join(self._index_file, "%s-*" % self._split))
            )
            ...
            print(
                "Using tf native pipeline for {train} dataloading".format(
                    train="training" if self.is_training else "validation"
                )
            )
    ...
    def parse_record(self, record: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
        ...
        if self.cutmix_alpha:
            features["cutmix_mask"] = cutmix_mask(
                self.cutmix_alpha, self._image_size, self._image_size
            )
        else:
