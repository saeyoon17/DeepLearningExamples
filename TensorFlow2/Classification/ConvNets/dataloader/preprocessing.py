...
def decode_crop_and_flip(image_bytes: tf.Tensor) -> tf.Tensor:
    ...
    crop_window = tf.stack([offset_height, offset_width, target_height, target_width])
    if decoded:
        ...
        cropped = tf.image.decode_and_crop_jpeg(image_bytes, crop_window, channels=3)
...
def resize_image(
    image_bytes: tf.Tensor, height: int = IMAGE_SIZE, width: int = IMAGE_SIZE
) -> tf.Tensor:
