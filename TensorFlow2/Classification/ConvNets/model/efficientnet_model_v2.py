...
from model.blocks import conv2d_block, fused_mb_conv_block, mb_conv_block
from model.common_modules import load_weights, round_filters, round_repeats
from model.layers import (gelu, get_activation, hard_swish, identity,
                          simple_swish)
...
DENSE_KERNEL_INITIALIZER = {
    ...
    "config": {"scale": 1 / 3.0, "mode": "fan_in", "distribution": "uniform"},
}
...
class Model(tf.keras.Model):
    ...
    def __init__(self, config: Dict[Text, Any] = None):
        ...
        if not is_predict:
            ...
            is_tr_split = tf.keras.layers.Input(
                shape=(1)
            )  # indicates whether we use tr or eval data loader
    ...
    def _build(self, input: List[tf.keras.layers.Input]):
        ...
        def cond_mixing(args):
            ...
            return tf.cond(
                tf.keras.backend.equal(is_tr_split[0], 0),
        ...
        if len(input) > 1:
            ...
            if self.config.defer_img_mixing:
                ...
                is_tr_split = input[3]
                x = tf.keras.layers.Lambda(cond_mixing)(
                    [images, mixup_weights, cutmix_masks, is_tr_split]
                )
        ...
        x = tf.keras.layers.GlobalAveragePooling2D(
            name="top_pool", data_format=data_format
        )(x)
