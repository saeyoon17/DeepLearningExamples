...
def mb_conv_block(inputs: tf.Tensor, block: dict, config: dict, prefix: Text = None):
    ...
    """
    use_se = (
        config.mparams.use_se
        if "use_se" in config.mparams
        else block["se_ratio"] is not None
    )
