...
def fused_mb_conv_block(
    inputs: tf.Tensor, block: dict, config: dict, prefix: Text = None
):
    ...
    if block["se_ratio"] is not None:
        ...
        se = tf.keras.layers.GlobalAveragePooling2D(
            name=prefix + "se_squeeze", data_format=data_format
        )(x)
    ...
    if block["expand_ratio"] == 1:
        x = tf.keras.layers.Activation(activation, name=prefix + "project_activation")(
            x
        )

    # Add identity so that quantization-aware training can insert quantization
    # ops correctly.
    x = tf.keras.layers.Activation(get_activation("identity"), name=prefix + "id")(x)
