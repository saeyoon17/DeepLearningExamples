...
def get_callbacks(
    ...
    if ema_decay > 0:
        ...
        callbacks.append(
            MovingAverageCallback(intratrain_eval_using_ema=intratrain_eval_using_ema)
        )
...
class CustomTensorBoard(tf.keras.callbacks.TensorBoard):
    ...
    def __init__(
        self, log_dir: str, track_lr: bool = False, initial_step: int = 0, **kwargs
    ):
        super(CustomTensorBoard, self).__init__(log_dir=log_dir, **kwargs)
        self.step = initial_step
        self._track_lr = track_lr

    def on_batch_begin(self, epoch: int, logs: MutableMapping[str, Any] = None) -> None:
        self.step += 1
        if logs is None:
            logs = {}
...
class MovingAverageCallback(tf.keras.callbacks.Callback):
    ...
    def set_model(self, model: tf.keras.Model):
        ...
        self.ema_opt = optimizer_factory.fetch_optimizer(
            model, optimizer_factory.MovingAverage
        )
...
class TimeHistory(tf.keras.callbacks.Callback):
    ...
    def average_steps_per_second(self):
        ...
        return (self.global_steps - self.step_per_epoch) / sum(
            self.epoch_runtime_log[1:]
        )
    ...
    def average_examples_per_second(self):
        ...
        if len(self.throughput) == 1:
            return self.throughput[
                0
            ]  # this throughput is inaccurate because the first step is warmup
        ind = max(
            int(0.1 * len(self.throughput)), 1
        )  # ensures exclusion of the first step (warmup step)
        return sum(self.throughput[ind:]) / (
            len(self.throughput[ind:])
        )  # removed +1 from denominator
        ...
        if steps_since_last_log >= self.log_steps:
            ...
            if self.summary_writer:
                with self.summary_writer.as_default():
                    tf.summary.scalar(
                        "global_step/sec", steps_per_second, self.global_steps
                    )
                    tf.summary.scalar(
                        "examples/sec", examples_per_second, self.global_steps
                    )
