...
class GradientAccumulator(object):
    ...
    def normalize(self):
        ...
        for gradient in self._get_replica_gradients():
            if gradient is not None:
                gradient.assign(
                    gradient * tf.cast(1 / self._accum_steps, gradient.dtype)
                )
...
class HvdMovingAverage(MovingAverage):
    ...
    def update_average(self, step: tf.Tensor):
        ...
        if step < self._start_step:
            ...
            decay = tf.minimum(average_decay, (1.0 + decay) / (10.0 + decay))
        else:
            decay = average_decay
...
def build_learning_rate(
    ...
    if decay_type == "exponential":
        ...
        lr = tf.keras.experimental.CosineDecay(
            initial_learning_rate=base_lr, decay_steps=decay_steps, alpha=0.0
        )
