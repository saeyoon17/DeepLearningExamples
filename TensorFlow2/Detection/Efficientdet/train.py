...
from model import (anchors, callback_builder, coco_metric, dataloader,
                   efficientdet_keras, label_util, optimizer_builder,
                   postprocess)
...
flags.DEFINE_bool(
    "benchmark", False, "Train for a fixed number of steps for performance"
)
flags.DEFINE_integer(
    "benchmark_steps",
    100,
    "Train for these many steps to benchmark training performance",
)
flags.DEFINE_bool("use_fake_data", False, "Use fake input.")
flags.DEFINE_bool("use_xla", True, "Use XLA")
flags.DEFINE_bool("amp", True, "Enable mixed precision training")
flags.DEFINE_bool(
    "set_num_threads", True, "Set inter-op and intra-op parallelism threads"
)
flags.DEFINE_string("log_filename", "time_log.txt", "Filename for dllogger logs")
flags.DEFINE_integer(
    "log_steps", 1, "Interval of steps between logging of batch level stats"
)
...
flags.DEFINE_integer(
    "checkpoint_period", 10, "Save ema model weights after every X epochs for eval"
)
flags.DEFINE_string(
...
def main(_):
    ...
    # Check data path
    if (
        FLAGS.training_file_pattern is None
        or FLAGS.val_file_pattern is None
        or FLAGS.val_json_file is None
    ):
        raise RuntimeError(
    ...
    steps_per_epoch = (
        FLAGS.num_examples_per_epoch + (FLAGS.batch_size * get_world_size()) - 1
    ...
    train_from_epoch = util_keras.restore_ckpt(
        model,
        params["model_dir"],
    ...
    history = model.fit(
        ...
        validation_steps=(FLAGS.eval_samples // FLAGS.eval_batch_size)
        if FLAGS.validate
        else None,
    )
    ...
    for callback in callbacks:
        if isinstance(callback, callback_builder.TimeHistory):
            if callback.epoch_runtime_log:
                ...
                stats["avg_fps_training_per_GPU"] = (
                    callback.average_examples_per_second / get_world_size()
                )
    ...
    if os.path.exists(os.path.join(FLAGS.model_dir, "ema_weights")):
            ...
            % sorted(
                set(
                    [
                        ...
                        for f in os.listdir(
                            os.path.join(FLAGS.model_dir, "ema_weights")
                        )
                        if "emackpt" in f
                    ]
                ),
        ...
        ckpt = os.path.join(
            FLAGS.model_dir, "ema_weights", "emackpt-" + str(ckpt_epoch)
        )
    ...
    if FLAGS.benchmark == False and (
        FLAGS.training_mode == "train" or FLAGS.num_epochs < 200
    ):
