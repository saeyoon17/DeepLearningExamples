...
from model import anchors, efficientdet_keras, iou_utils
...
class StableFocalLoss(tf.keras.losses.Loss):
    ...
    def call(self, y, y_pred):
        ...
        modulator = tf.exp(
            gamma * y_true * negative_pred
            - gamma * tf.math.log1p(tf.exp(negative_pred))
        )
    ...
    def _detection_loss(self, cls_outputs, box_outputs, labels, loss_vals):
        ...
        for level in levels:
            ...
            cls_targets_at_level = tf.one_hot(
                labels["cls_targets_%d" % (level + 3)], self.config.num_classes
            )
