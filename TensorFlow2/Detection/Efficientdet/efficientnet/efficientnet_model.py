...
from efficientnet.common_modules import (load_weights, round_filters,
                                         round_repeats)
from efficientnet.layers import (gelu, get_activation, hard_swish, identity,
                                 simple_swish)
...
def efficientnet(image_input: tf.keras.layers.Input, config: dict, features_only: bool):
    ...
    for stack_idx, block in enumerate(blocks):
        ...
        elif (stack_idx == len(blocks) - 1) or blocks[stack_idx + 1]["strides"][0] > 1:
            is_reduction = True
            reduction_idx += 1
        ...
        if block["num_repeat"] > 1:
            block.update({"input_filters": block["output_filters"], "strides": (1, 1)})
