...
from model import (coco_metric, efficientdet_keras, inference, label_util,
                   optimizer_builder, postprocess)
...
class StopEarlyCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if ((epoch + 1) > (0.75 * self.num_epochs) and self.stop_75) or (
            (epoch + 1) == 300
        ):
...
class COCOEvalCallback(tf.keras.callbacks.Callback):
    def __init__(
        self, eval_dataset, eval_freq, start_eval_epoch, eval_params, logger, **kwargs
    ):
...
def get_callbacks(
    ...
    if is_main_process():
        ...
        if params["moving_average_decay"]:
            emackpt_callback = AverageModelCheckpoint(
                filepath=os.path.join(
                    params["model_dir"], "ema_weights", "emackpt-{epoch:02d}"
                ),
        ...
        if time_history:
            time_callback = TimeHistory(
                params["batch_size"] * get_world_size(),
                logger=logger,
    ...
    if params.get("sample_image", None):
        display_callback = DisplayCallback(
            params.get("sample_image", None), os.path.join(params["model_dir"], "train")
        )
