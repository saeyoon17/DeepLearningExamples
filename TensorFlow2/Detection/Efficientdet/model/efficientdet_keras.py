...
from model import (activation_builder, dataloader, fpn_configs,
                   normalization_builder, postprocess)
from utils import hparams_config, model_utils, util_keras
...
class OpAfterCombine(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        self.bn = util_keras.build_batch_norm(
            is_training_bn=self.is_training_bn, data_format=self.data_format, name="bn"
        )
        ...
        for level_id in range(0, self.max_level - self.min_level + 1):
            ...
            for i in range(self.repeats):
                ...
                if i > 0 and self.survival_prob:
                    image = model_utils.drop_connect(
                        image, training, self.survival_prob
                    )
        ...
        **kwargs
        ...
        for _ in range(max_level - min_level):
            ...
            self.con2d_t_bns.append(
                util_keras.build_batch_norm(
                    is_training_bn=is_training_bn, data_format=data_format, name="bn"
                )
            )
...
class EfficientDetNet(tf.keras.Model):
    ...
    def __init__(self, model_name=None, config=None, name=""):
        ...
        if "efficientnet" in backbone_name:
            override_params = {
                "batch_norm": normalization_builder.batch_norm_class(is_training_bn),
                "relu_fn": functools.partial(
                    activation_builder.activation_fn, act_type=config.act_type
                ),
            ...
            self.backbone = efficientnet_model.EfficientNet().from_name(
                ...
                features_only=True,
                model_weights_path=config.backbone_init,
