...
def define_command_line_flags():
    flags.DEFINE_enum(
        "mode",
        default="train",
        enum_values=["inference", "eval", "train", "deploy"],
    ...
    flags.DEFINE_integer(
        "batch_size", default=64 * 1024, help="Batch size used for training"
    )
    flags.DEFINE_bool(
        "run_eagerly",
        default=False,
        help="Disable all tf.function decorators for debugging",
    )

    flags.DEFINE_bool(
        "dummy_model",
        default=False,
        help="Use a dummy model for benchmarking and debugging",
    )

    flags.DEFINE_list(
        "top_mlp_dims", [1024, 1024, 512, 256, 1], "Linear layer sizes for the top MLP"
    )
    flags.DEFINE_list(
        "bottom_mlp_dims", [512, 256, 128], "Linear layer sizes for the bottom MLP"
    )
    ...
    flags.DEFINE_integer(
        "loss_scale",
        default=1024,
        help="Static loss scale to use with mixed precision training",
    )
    ...
    flags.DEFINE_integer(
        "max_steps",
        default=-1,
        help="Stop the training/inference after this many optimiation steps",
    )

    flags.DEFINE_bool(
        "embedding_trainable",
        default=True,
        help="If True the embeddings will be trainable, otherwise frozen",
    )

    flags.DEFINE_enum(
        "dot_interaction",
        default="custom_cuda",
        enum_values=["custom_cuda", "tensorflow", "dummy"],
        help="Dot interaction implementation to use",
    )

    flags.DEFINE_integer(
        "embedding_dim", default=128, help="Number of columns in the embedding tables"
    )

    flags.DEFINE_integer(
        "evals_per_epoch", default=1, help="Number of evaluations per epoch"
    )
    flags.DEFINE_float(
        "print_freq", default=100, help="Number of steps between debug prints"
    )
    ...
    flags.DEFINE_integer(
        "decay_start_step",
        default=48000,
        help="Optimization step at which to start the poly LR decay",
    )
    flags.DEFINE_integer(
        "decay_steps",
        default=24000,
    ...
    flags.DEFINE_integer(
        "profiler_start_step", default=None, help="Step at which to start profiling"
    )
    flags.DEFINE_integer("profiled_rank", default=1, help="Rank to profile")

    flags.DEFINE_integer(
        "inter_op_parallelism", default=None, help="Number of inter op threads"
    )
    flags.DEFINE_integer(
        "intra_op_parallelism", default=None, help="Number of intra op threads"
    )
    ...
    flags.DEFINE_string(
        "log_path",
        default="dlrm_tf_log.json",
        help="Path to JSON file for storing benchmark results",
    )

    # dataset and dataloading settings
    flags.DEFINE_string("dataset_path", default=None, help="Path to dataset directory")
    flags.DEFINE_string(
        "feature_spec",
...
from model import Dlrm, DlrmTrainer, DummyDlrm, evaluate
from tensorflow.keras.mixed_precision import LossScaleOptimizer
from utils import IterTimer, dist_print, init_logging
...
def inference_benchmark(validation_pipeline, dlrm, timer, FLAGS):
    ...
    auc, test_loss, latencies = evaluate(
        validation_pipeline,
        dlrm,
        timer,
...
def validate_cmd_line_flags():
    ...
    if (
        FLAGS.restore_checkpoint_path is not None
        and FLAGS.saved_model_input_path is not None
    ):
        raise ValueError(
            "Incompatible cmd-line flags."
    ...
    if FLAGS.saved_model_input_path is not None and FLAGS.mode == "train":
        raise ValueError(
            "Training from a SavedModel is not supported."
    ...
    if FLAGS.cpu and FLAGS.dot_interaction == "custom_cuda":
        raise ValueError(
            '"custom_cuda" dot interaction not supported for CPU. '
...
def main(argv):
    ...
    train_pipeline, validation_pipeline = create_input_pipelines(
        FLAGS, dlrm.local_table_ids
    )

    if FLAGS.optimizer == "sgd":
        embedding_optimizer = tf.keras.optimizers.SGD(
            learning_rate=FLAGS.learning_rate, momentum=0
        )
        if FLAGS.amp:
            embedding_optimizer = LossScaleOptimizer(
                embedding_optimizer, initial_scale=FLAGS.loss_scale, dynamic=False
            )
        ...
        if FLAGS.amp:
            ...
            mlp_optimizer = LossScaleOptimizer(
                mlp_optimizer, initial_scale=FLAGS.loss_scale, dynamic=False
            )
    ...
    if FLAGS.mode == "inference":
        ...
        dlrm.save_model_if_path_exists(
            FLAGS.saved_model_output_path,
        ...
        test_auc, test_loss, _ = evaluate(
            validation_pipeline, dlrm, timer, auc_thresholds=FLAGS.auc_thresholds
        )
        if hvd.rank() == 0:
            dllogger.log(data=dict(auc=test_auc, test_loss=test_loss), step=tuple())
        return

    eval_points = compute_eval_points(
        train_batches=len(train_pipeline), evals_per_epoch=FLAGS.evals_per_epoch
    )
    ...
    for epoch in range(FLAGS.epochs):
        ...
        for step in range(len(train_pipeline)):
            ...
            if (
                FLAGS.profiler_start_step
                and step == FLAGS.profiler_start_step + 100
                and hvd.rank() == FLAGS.profiled_rank
            ):
            ...
            if step in eval_points:
                test_auc, test_loss, _ = evaluate(
                    validation_pipeline, dlrm, timer, FLAGS.auc_thresholds
                )
                dist_print(
    ...
    dlrm.save_model_if_path_exists(
        FLAGS.saved_model_output_path, save_input_signature=FLAGS.save_input_signature
    )
