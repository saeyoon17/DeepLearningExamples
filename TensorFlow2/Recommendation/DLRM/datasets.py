...
from defaults import (CATEGORICAL_CHANNEL, DTYPE_SELECTOR, LABEL_CHANNEL,
                      NUMERICAL_CHANNEL)
from feature_spec import FEATURES_SELECTOR, FILES_SELECTOR, FeatureSpec
...
class DummyDataset:
    def __init__(
        ...
        cat_features_count = (
            len(categorical_feature_cardinalities)
            if categorical_feature_cardinalities is not None
            else 0
        )
        num_features_count = (
            num_numerical_features if num_numerical_features is not None else 0
        )

        self.numerical_features = (
            tf.random.uniform(
                shape=[batch_size // num_workers, num_numerical_features],
                dtype=tf.float32,
            )
            if num_features_count
            else -1
        )
        self.labels = tf.cast(
            tf.random.uniform(
                shape=[batch_size // num_workers, 1], maxval=2, dtype=tf.int32
            ),
            tf.float32,
        )
        self.categorical_features = (
            tf.concat(
                [
                    tf.random.uniform(
                        shape=[batch_size, 1], maxval=cardinality, dtype=tf.int32
                    )
                    for cardinality in categorical_feature_cardinalities
                ],
                axis=1,
            )
...
fspec_type_to_tf_type = {"int8": tf.int8, "int16": tf.int16, "int32": tf.int32}
...
def create_reader(filename, bytes_per_batch):
    ...
    reader = tf.data.FixedLengthRecordDataset(
        filenames=[filename], record_bytes=bytes_per_batch, footer_bytes=footer_bytes
    )
...
class TfRawBinaryDataset:
    ...
    def __init__(
        ...
        self._create_readers(
            feature_spec, local_categorical_feature_names, numerical_features_enabled
        )
        self._categorical_types_tf = [
            fspec_type_to_tf_type[feature_spec.feature_spec[feature][DTYPE_SELECTOR]]
            for feature in local_categorical_feature_names
        ]

    def _create_readers(
        self, feature_spec, local_categorical_feature_names, numerical_features_enabled
    ):
        ...
        self._number_of_numerical_features = (
            len(numerical_features) if numerical_features_enabled else 0
        )
        ...
        bytes_per_feature = {
            feature_name: np.dtype(
                feature_spec.feature_spec[feature_name][DTYPE_SELECTOR]
            ).itemsize
        ...
        for chunk in chosen_instance:
            ...
            if first_feature in set_of_categorical_features:
                ...
                if not numerical_features_enabled:
                    ...
                    continue  # skip chunk
                numerical_bytes_per_batch = (
                    bytes_per_feature[numerical_features[0]]
                    * len(numerical_features)
                    * self._batch_size
                )
                self._numerical, batches = create_reader(
                    path_to_open, numerical_bytes_per_batch
                )
                ...
                self._label, batches = create_reader(
                    path_to_open, label_bytes_per_batch
                )
            else:
            ...
            if number_of_batches is not None:
                if batches != number_of_batches:
                    raise ValueError(
                        f"Size mismatch. Expected: {number_of_batches}, got: {batches}"
                    )
            else:
                number_of_batches = batches

        self._categorical = tuple(
            categorical_feature_readers[feature]
            for feature in local_categorical_feature_names
        )
    ...
    def decode_batch(self, labels, numerical_features, categorical_features):
        ...
        if self._number_of_numerical_features > 0:
            ...
            numerical_features = tf.reshape(
                numerical_features, shape=[-1, self._number_of_numerical_features]
            )
            numerical_features = numerical_features[
                self.dp_begin_idx : self.dp_end_idx, :
            ]
