...
class DotBasedInteractTest(test.TestCase):
    def input(self, batch_size, num_rows, num_cols, dtype):
        ...
        bottom_mlp_output = tf.random.uniform(
            shape=[batch_size, mlp_rows, num_cols], dtype=dtype
        )
        embeddings = tf.random.uniform(
            shape=[batch_size, emb_rows, num_cols], dtype=dtype
        )
    ...
    def forward(self, batch_size, num_rows, num_cols, dtype):
        with self.test_session() as sess:
            with ops.device("/gpu:0"):
                input, bottom_mlp_output = self.input(
                    batch_size, num_rows, num_cols, dtype
                )
                expected = dot_based_interact_native(input, bottom_mlp_output)
                result = dot_based_interact_ops.dot_based_interact(
                    input, bottom_mlp_output
                )
    ...
    def backward(self, batch_size, num_rows, num_cols, dtype):
        with self.test_session() as sess:
            with ops.device("/gpu:0"):
                ...
                with tf.GradientTape() as tape:
                    output = dot_based_interact_ops.dot_based_interact(
                        input, bottom_mlp_output
                    )
    ...
    def test_fp32_extended_1(self):
        ...
        for batch_size, num_rows, num_cols in self.extended_argset_1:
            self.assertAllClose(
                *self.forward(batch_size, num_rows, num_cols, tf.float32), rtol=1e-03
            )

    def test_grad_fp32_extended_1(self):
        for batch_size, num_rows, num_cols in self.extended_argset_1:
            self.assertAllClose(
                *self.backward(batch_size, num_rows, num_cols, tf.float32), rtol=1e-03
            )

    def test_fp16_extended_1(self):
        for batch_size, num_rows, num_cols in self.extended_argset_1:
            self.assertAllCloseAccordingToType(
                *self.forward(batch_size, num_rows, num_cols, tf.float16)
            )

    def test_grad_fp16_extended_1(self):
        for batch_size, num_rows, num_cols in self.extended_argset_1:
            self.assertAllCloseAccordingToType(
                *self.backward(batch_size, num_rows, num_cols, tf.float16)
            )
