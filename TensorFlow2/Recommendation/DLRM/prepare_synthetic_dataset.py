...
from defaults import DTYPE_SELECTOR, TEST_MAPPING, TRAIN_MAPPING
...
flags.DEFINE_integer(
    ...
    default=int(32768),
    help="Batch size - number of unique records",
)
...
flags.DEFINE_string(
    ...
    default="/tmp/dlrm_synthetic_data",
    help="Destination of the saved synthetic dataset",
)

flags.DEFINE_integer(
    "seed", default=12345, help="Set a seed for generating synthetic data"
)


def write_dataset_to_disk(
    dataset_train, dataset_test, feature_spec: FeatureSpec
) -> None:
    ...
    for mapping_name, dataset in zip(
        (TRAIN_MAPPING, TEST_MAPPING), (dataset_train, dataset_test)
    ):
        file_streams = []
        label_path, numerical_path, categorical_paths = feature_spec.get_mapping_paths(
            mapping_name
        )
        try:
            ...
            for _ in tqdm.tqdm(
                ...
                categoricals = tf.split(
                    categorical, number_of_categorical_features, axis=1
                )
                assert numerical.shape[-1] == number_of_numerical_features
                assert len(categoricals) == number_of_categorical_features

                numerical_f.write(
                    numerical.numpy().astype("float16").tobytes()
                )  # numerical is always float16
                label_f.write(
                    label.numpy().astype("bool").tobytes()
                )  # label is always boolean
                for cat_type, cat_tensor, cat_file in zip(
                    categorical_features_types, categoricals, categorical_fs
                ):
...
def main(argv):
    ...
    if FLAGS.feature_spec is not None:
        ...
        fspec = FeatureSpec.get_default_feature_spec(
            number_of_numerical_features=FLAGS.num_numerical_features,
    ...
    train_dataset = DummyDataset(
        ...
        num_numerical_features=number_of_numerical_features,
    ...
    write_dataset_to_disk(
        dataset_train=train_dataset, dataset_test=test_dataset, feature_spec=fspec
    )
