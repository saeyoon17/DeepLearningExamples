    ...
    if frequency_limit:
        ...
        for fl in frequency_limit:
            ...
            if len(frequency_pair) == 1:
                ...
                df = df.filter(
                    (col("column_id") != int(frequency_pair[0]) - CAT_COLS[0])
        ...
        if default_limit:
            ...
            df = df.filter(
                (~col("column_id").isin(remain)) | (col("count") >= default_limit)
            )
...
def assign_id_with_window(df):
    ...
    return df.withColumn("id", row_number().over(windowed)).withColumnRenamed(
        "count", "model_count"
    )
...
def assign_low_mem_partial_ids(df):
    ...
    return df.withColumn(
        "mono_id", monotonically_increasing_id() - shiftLeft(col("part_id"), 33)
    )


def assign_low_mem_final_ids(df):
    # Now we can find the minimum and maximum mono_ids within a given column/partition pair
    sub_model = df.groupBy("column_id", "part_id").agg(
        max("mono_id").alias("top"), min("mono_id").alias("bottom")
    )
    ...
    sub_model = sub_model.withColumn("running_sum", sum("diff").over(windowed)).na.fill(
        0, ["running_sum"]
    )
    ...
    joined = joined.join(
        sub_model,
        (col("i_column_id") == col("column_id")) & (col("part_id") == col("i_part_id")),
    )
    ...
    ret = joined.select(
        col("column_id"),
        col("data"),
        (col("mono_id") - col("bottom") + col("running_sum") + 1)
        .cast(IntegerType())
        .alias("id"),
        col("model_count"),
    )
...
def get_column_models(combined_model):
    for i in CAT_COLS:
        model = combined_model.filter("column_id == %d" % (i - CAT_COLS[0])).drop(
            "column_id"
        )
        yield i, model
...
def skewed_join(df, model, col_name, cutoff):
    ...
    # a separate column
    b_model = broadcast(
        model.filter(col("model_count") >= cutoff)
    ...
    df = df.withColumn(
        ...
        when(col("id_tmp").isNotNull() | col(col_name).isNull(), rand_column).otherwise(
            lit(-1)
        ),
    )
    ...
    df = df.withColumn(
        col_name, when(col("id_tmp").isNotNull(), None).otherwise(col(col_name))
    )
    ...
    nb_model = (
        model.withColumn(
            "join_rand", when(col("model_count") < cutoff, lit(-1)).otherwise(lit(-2))
        )
...
def apply_models(df, models, broadcast_model=False, skew_broadcast_pct=1.0):
    ...
    for i, model, original_rows, would_broadcast in models:
        ...
        if not (would_broadcast or broadcast_model):
            ...
            df = (
                df.join(model, col_name, how="left")
                .drop(col_name)
...
def day_from_input_file(df):
    return df.withColumn(
        "day", substring_index(input_file_name(), "_", -1).cast(IntegerType())
    )
...
def load_column_models(spark, model_folder, count_required):
    for i in CAT_COLS:
        ...
        if count_required:
            values = df.agg(
                sum("model_count").alias("sum"), count("*").alias("size")
            ).collect()
        else:
...
def _parse_args():
    ...
    parser.add_argument(
        "--mode", required=True, choices=["generate_models", "transform"]
    )
    ...
    parser.add_argument(
        "--write_mode", choices=["overwrite", "errorifexists"], default="errorifexists"
    )
    ...
    parser.add_argument(
        "--output_partitioning", choices=["day", "none"], default="none"
    )
...
def _main():
    ...
    if args.mode == "generate_models":
        spark.conf.set(
            ...
            args.days * args.dict_build_shuffle_parallel_per_day,
        )
        with _timed("generate models"):
            col_counts = get_column_counts_with_frequency_limit(
                df, args.frequency_limit
            )
            if args.low_mem:
                ...
                save_combined_model(
                    assign_low_mem_final_ids(
                        load_low_mem_partial_ids(spark, args.model_folder)
                    ),
                    args.model_folder,
                    args.write_mode,
                )
    ...
    if args.mode == "transform":
        spark.conf.set(
            "spark.sql.shuffle.partitions",
            args.days * args.apply_shuffle_parallel_per_day,
        )
        with _timed("transform"):
            ...
            df.write.parquet(
                args.output_folder, mode=args.write_mode, partitionBy=partitionBy
            )
