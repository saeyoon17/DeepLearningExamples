...
import numpy as np
import yaml
from sim.data.defaults import (CARDINALITY_SELECTOR, DIMENSIONS_SELECTOR,
                               DTYPE_SELECTOR, LABEL_CHANNEL,
                               NEGATIVE_HISTORY_CHANNEL,
                               POSITIVE_HISTORY_CHANNEL,
                               TARGET_ITEM_FEATURES_CHANNEL, TEST_MAPPING,
                               TRAIN_MAPPING, USER_FEATURES_CHANNEL)
...
class FeatureSpec:
    ...
    def get_default_features_names(number_of_user_features, number_of_item_features):
        ...
        user_features_names = [
            user_feature_fstring.format(i) for i in range(number_of_user_features)
        ]
    ...
    def get_default_feature_spec(
        ...
        all_features_names = FeatureSpec.get_default_features_names(
        ...
        user_features = {
            ...
            for i, (f_name, cardinality) in enumerate(
                zip(
                    all_features_names[1 : 1 + number_of_user_features],
                    user_features_cardinalities,
                )
            )
        }
        ...
        for i, cardinality in enumerate(item_features_cardinalities):
            for j, (channel, dictionary) in enumerate(item_channels_info):
                ...
                feature_name = all_features_names[
                    1 + number_of_user_features + i + j * number_of_item_features
                ]
