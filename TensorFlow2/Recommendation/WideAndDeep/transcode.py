...
from data.feature_spec import (FEATURES_SELECTOR, FILES_SELECTOR,
...
def parse_args():
    ...
    parser.add_argument(
        "--input", type=str, default="", help="Path to input data directory"
    )
    parser.add_argument(
        "--feature_spec_in",
        type=str,
...
def main():
    ...
    for mapping_name, mapping in fspec_in.source_spec.items():
        ...
        for chunk in mapping:
            ...
            path_to_load = os.path.join(
                fspec_in.base_directory, chunk[FILES_SELECTOR][0]
            )
            chunk_iterator = pd.read_csv(
                path_to_load,
                header=None,
                chunksize=batch_size,
                names=chunk[FEATURES_SELECTOR],
            )
        ...
        for chunk_id, chunks in enumerate(zipped):
            ...
            for feature in multihot_features:
                mapping_df[feature] = mapping_df[feature].apply(
    ...
    for mapping_name in old_source_spec.keys():
        ...
        assert (
            got_partitions > min_partitions
        ), f"Not enough partitions generated for mapping:{mapping_name}. Expected at least {min_partitions}, got {got_partitions}"
        ...
        new_source_spec[mapping_name].append(
            {
    ...
    fspec_out = FeatureSpec(
        feature_spec=fspec_in.feature_spec,
        source_spec=new_source_spec,
        channel_spec=fspec_in.channel_spec,
        metadata=fspec_in.metadata,
    )
