...
from global_metrics import (AvgMeter, CompositeMeter, MaxMeter, MinMeter,
                            PercentileMeter)
...
def add_fit_args(parser):
    ...
    train.add_argument(
        "--mode",
        default="train_val",
        choices=("train_val", "train", "val", "pred"),
        help="mode",
    )
    ...
    train.add_argument("--batch-size", type=int, default=192, help="the batch size")
    train.add_argument("--num-epochs", type=int, default=90, help="number of epochs")
    train.add_argument(
        "--run-epochs",
        type=int,
        default=-1,
    ...
    train.add_argument("--mom", type=float, default=0.875, help="momentum for sgd")
    train.add_argument(
        "--wd", type=float, default=1 / 32768, help="weight decay for sgd"
    )
    train.add_argument(
    ...
    train.add_argument(
        "--model-prefix", type=str, default="model", help="model checkpoint prefix"
    )
    train.add_argument(
        "--save-frequency",
        type=int,
    ...
    train.add_argument(
        "--begin-epoch", type=int, default=0, help="start the model from an epoch"
    )
    train.add_argument("--load", help="checkpoint to load")
    train.add_argument(
        "--test-io", action="store_true", help="test reading speed without training"
    )
    train.add_argument(
        "--test-io-mode", default="train", choices=("train", "val"), help="data to test"
    )
    train.add_argument(
        "--log",
        type=str,
        ...
        for i, (step, next_step) in enumerate(zip(steps, steps[1:])):
            if next_step > x:
                return (
                    args.lr
                    * 0.5
                    * (1 + math.cos(math.pi * (x - step) / (next_step - step)))
                    * (args.lr_factor**i)
                )
        return 0
...
def load_model(args, model):
    file = list(glob.glob(f"{args.workspace}/{args.model_prefix}_*.params"))
    if len(file) == 0:
        return -1
...
def save_checkpoint(
    ...
    if (
        ...
        or save_frequency == 0
        or ("horovod" in kvstore and hvd.rank() != 0)
    ):
        return
...
def model_pred(args, model, image):
    ...
    for i, ind in enumerate(top):
        ...
        logging.info(
            "{:2d}. {:5.2f}% -> {}".format(
                i + 1, output[ind].asscalar() * 100, classes[ind]
            )
        )
...
def model_score(args, net, val_data, metric, kvstore):
    ...
    for batches in val_data:
        ...
        label = [
            b.label[0][: len(b.data[0]) - b.pad]
            for b in batches
            if len(b.data[0]) != b.pad
        ]
        outputs = [net(X) for X, b in zip(data, batches)]
        outputs = [
            o[: len(b.data[0]) - b.pad]
            for o, b in zip(outputs, batches)
            if len(b.data[0]) != b.pad
        ]
...
def model_fit(
    ...
    def transform_data(images, labels):
        if args.mixup != 0:
            coeffs = mx.nd.array(
                np.random.beta(args.mixup, args.mixup, size=images.shape[0])
            ).as_in_context(images.context)
            ...
            ret_labels = (
                label_coeffs * ret_labels + (1 - label_coeffs) * ret_labels[::-1]
            )
        else:
            ret_images = images
    ...
    for epoch in range(begin_epoch, min(run_epoch, num_epoch)):
        ...
        if args.mode == "train_val":
            ...
            dllogger_epoch_data.update(
                starmap(
                    lambda key, val: ("val.{}".format(key), val), duration_stats.items()
                )
            )
    ...
    if args.test_io:
        ...
        for i, batch in enumerate(data_iter):
            ...
            if (i + 1) % args.disp_batches == 0:
                logging.info(
                    "Batch [{}]\tSpeed: {:.2f} samples/sec".format(
