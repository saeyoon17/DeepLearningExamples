...
parser.add_argument(
    "-o", "--output", metavar="OUT", required=True, help="path to benchmark report"
)
parser.add_argument(
    "-n",
    "--ngpus",
    ...
    type=int_list,
    required=True,
    help="numbers of gpus separated by comma",
)
parser.add_argument(
    "-b",
    "--batch-sizes",
...
parser.add_argument(
    "-i", "--benchmark-iters", metavar="I", type=int, default=100, help="iterations"
)
parser.add_argument(
    "-e", "--epochs", metavar="E", type=int, default=1, help="number of epochs"
)
parser.add_argument(
    "-w", "--warmup", metavar="N", type=int, default=0, help="warmup epochs"
)
parser.add_argument(
    "--timeout", metavar="T", type=str, default="inf", help="timeout for each run"
)
parser.add_argument(
    "--mode",
    metavar="MODE",
    choices=("train_val", "train", "val"),
    default="train_val",
    help="benchmark mode",
)
...
for m in res["metric_keys"]:
    ...
    for n in args.ngpus:
        ...
        for bs in args.batch_sizes:
            print(
                "{:.5g}".format(res["metrics"][str(n)][str(bs)][m]).center(column_len),
                end="|",
                file=sys.stderr,
            )
        print(file=sys.stderr)
