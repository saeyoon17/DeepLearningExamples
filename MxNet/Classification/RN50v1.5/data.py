...
def add_data_args(parser):
    ...
    data.add_argument(
        "--data-train-idx", type=str, default="", help="the index of training data"
    )
    data.add_argument("--data-val", type=str, help="the validation data")
    data.add_argument(
        "--data-val-idx", type=str, default="", help="the index of validation data"
    )
    data.add_argument(
        "--data-pred",
        type=str,
        help="the image on which run inference (only for pred mode)",
    )

    data.add_argument(
        "--data-backend",
        choices=("dali-gpu", "dali-cpu", "mxnet", "synthetic"),
        default="dali-gpu",
...
def add_data_aug_args(parser):
    ...
    aug.add_argument(
        "--random-crop", type=int, default=0, help="if or not randomly crop the image"
    )
    aug.add_argument(
        "--random-mirror",
        type=int,
    ...
    aug.add_argument(
        "--pca-noise", type=float, default=0, help="pca noise, whose range is [0, 1]"
    )
    aug.add_argument(
        "--random-resized-crop",
        type=int,
...
def get_data_loader(args):
    if args.data_backend == "dali-gpu":
        return lambda *args, **kwargs: dali.get_rec_iter(
            *args, **kwargs, dali_cpu=False
        )
    ...
    def __next__(self):
        ...
        for i in range(len(self.ctx)):
            ...
            pad = max(
                0,
                min(
                    data.pad - (len(self.ctx) - i - 1) * self.batch_size,
                    self.batch_size,
                ),
            )
            ret.append(
...
def get_rec_iter(args, kv=None):
    ...
    if args.input_layout == "NHWC":
        raise ValueError(
            "ImageRecordIter cannot handle layout {}".format(args.input_layout)
        )
...
class SyntheticDataIter(DataIter):
    ...
    def next(self):
        ...
        if self.cur_iter <= self.max_iter:
            return [
                DataBatch(data=(data,), label=(label,), pad=0)
...
def load_image(args, path, ctx=mx.cpu()):
    ...
    if args.image_shape[0] == 4:
        ...
        image = nd.concat(
            image,
            nd.zeros((1, *image.shape[1:]), dtype=image.dtype, ctx=image.context),
            dim=dim,
        )
    return image
