...
def add_model_args(parser):
    ...
    model.add_argument(
        "--arch",
        default="resnetv15",
        choices=["resnetv1", "resnetv15", "resnextv1", "resnextv15", "xception"],
...
class Builder:
    def __init__(
        ...
        self.linear_initializer = lambda groups=1: mx.init.Xavier(
            rnd_type="gaussian", factor_type="in", magnitude=2 * (groups**0.5)
        )
    ...
    def max_pool(self, pool_size, strides=1, padding=True):
        ...
        return self.sequence(
            self.transpose(self.pooling_layout),
            nn.MaxPool2D(
                pool_size, strides=strides, padding=padding, layout=self.pooling_layout
            ),
        )

    def conv(
        self, channels, kernel_size, padding=True, strides=1, groups=1, in_channels=0
    ):
    ...
    def separable_conv(
        self, channels, kernel_size, in_channels, padding=True, strides=1
    ):
        return self.sequence(
            self.conv(
                in_channels,
    ...
    def dense(self, units, in_units=0):
        return nn.Dense(
            units, in_units=in_units, weight_initializer=self.linear_initializer()
        )
...
class Transpose(HybridBlock):
    ...
    def __repr__(self):
        ...
        return s.format(name=self.__class__.__name__, content=content)
...
class BatchNormAddRelu(nn.BatchNorm):
    def __init__(self, *args, **kwargs):
        ...
        if self._kwargs.pop("act_type") != "relu":
            raise ValueError(
                "BatchNormAddRelu can be used only with ReLU as activation"
            )

    def hybrid_forward(self, F, x, y, gamma, beta, running_mean, running_var):
        return F.BatchNormAddRelu(
            data=x,
            addend=y,
            gamma=gamma,
            beta=beta,
...
class ResNetBottleNeck(HybridBlock):
    def __init__(
        ...
        body = [
            builder.conv(
                channels * mult // 4, 1, strides=stride1, in_channels=in_channels
            ),
            builder.batchnorm_relu(),
...
class XceptionBlock(HybridBlock):
    def __init__(self, builder, definition, in_channels, relu_at_beginning=True):
        ...
        for channels1, channels2 in zip(definition, definition[1:] + [0]):
            if channels1 > 0:
                body.append(
                    builder.separable_conv(channels1, 3, in_channels=last_channels)
                )
                if channels2 > 0:
...
resnet_spec = {
    18: (ResNetBasicBlock, [2, 2, 2, 2], [64, 64, 128, 256, 512]),
