...
class TritonEvaluator(MetricEvaluator):
        ...
        for ids, x, y_pred, y_real in tqdm(runner, unit="batch", mininterval=10):
            if self.save_predictions:
                self.example_history.append(
                    x["target__6"][:, : self.config.encoder_length]
                )
            ids_full.append(ids)
    ...
    def predict_xgboost(self, dataloader, max_batch_size, server_url="localhost:8001"):
        grpc_client = triton_grpc.InferenceServerClient(url=server_url, verbose=False)
        out = []
        labels = []
        ids = []
        weights = []
        for i, (test_step, test_label) in enumerate(dataloader):
            ...
            for j in range(num_iters):
                ...
                triton_input_grpc = triton_grpc.InferInput("input__0", dims, "FP32")
        ...
        if self.save_predictions:
            ...
            for n, g in labels_ids.groupby("_id_"):
                ...
                windows_labels = np.lib.stride_tricks.sliding_window_view(
                    labels_all, dataloader.example_length
                )
                self.example_history.append(
                    windows_labels.copy()[:, : dataloader.encoder_length]
                )
            self.example_history = np.concatenate(self.example_history, axis=0)[
                :, :, np.newaxis
            ]
