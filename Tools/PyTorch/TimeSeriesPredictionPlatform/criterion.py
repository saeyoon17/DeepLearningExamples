...
class TSPP_criterion_wrapper(nn.Module):
    ...
    def __init__(self, criterion, cl_start_horizon=None, cl_update=None):
        ...
        assert bool(cl_start_horizon) == bool(
            cl_update
        ), "Both cl_start_horizon and cl_update have to be set or unset at the same time"
    ...
    def forward(self, preds, labels, weights=None, **kwargs):
        ...
        if disallowed_kwargs:
            raise TypeError(
                f"Invalid keyword arguments {disallowed_kwargs} for {type(self.criterion)}"
            )
        kwargs = {
            name: arg for name, arg in kwargs.items() if name in self.base_arguments
        }
        ...
        if self.training:
            if self.curr_horizon:
                ...
                weights = (
                    weights[:, : self.curr_horizon] if weights is not None else None
                )
