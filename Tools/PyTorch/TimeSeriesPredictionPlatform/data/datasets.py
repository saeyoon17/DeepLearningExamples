...
import pandas as pd
import torch
from data.data_utils import (DTYPE_MAP, FEAT_NAMES, FEAT_ORDER, DataTypes,
                             InputTypes, group_ids, translate_features)
from data.xgb_util import (data_label_split, feat_adder, load_xgb_df,
                           select_test_group, target_shift,
...
class TSBaseDataset(Dataset):
    def __init__(
        self, features, df, encoder_length, example_length, stride=1, **kwargs
    ):
        super().__init__()
...
class TSDataset(TSBaseDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.group_lens = [
            (g.shape[0] - self.example_length + 1) // self.stride for g in self.grouped
        ]
    ...
    def get_probabilities(self):
        ...
        for i in range(len(self.grouped)):
            ...
            sampled.append(
                sample_weights[np.arange(0, self.stride * group_len, self.stride)]
            )
...
class TSMultiIDDatasetBase(TSBaseDataset):
        ...
        **kwargs,
        ...
        for fname, ftype in zip(FEAT_NAMES, FEAT_ORDER):
            names = [
                ...
                if (f.feature_type, f.feature_embed_type) == ftype
            ]
            if names:
                df = pd.concat(
                    [v for k, v in self.tables.items() if k in names], axis=1
                )
        ...
        del self.tables
        self._n_timeslices = (
            next(len(df) for df in self.data.values() if df is not None)
            - self.example_length
            + 1
        ) // self.stride
    ...
    def load(self):
        ...
        self.tables = {}
        for f in self.features:
            self.tables[f.name] = data.pivot(
                index=time_col_name, columns=id_col_name, values=f.name
            )
...
class TSMultiTargetDataset(TSMultiIDDatasetBase):
    ...
    def __getitem__(self, idx):
        ...
        out = {
            k: torch.from_numpy(
                v[idx * self.stride : idx * self.stride + self.example_length]
            )
            if v is not None
            else torch.empty(0)
...
class TSMultiIDDataset(TSMultiIDDatasetBase):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            features, df, encoder_length, example_length, stride, collumns_to_collapse
        )
    ...
    def __getitem__(self, idx):
        ...
        out = {
            k: torch.from_numpy(
                v[e_idx * self.stride : e_idx * self.stride + self.example_length, :]
            )
            if v is not None
            else torch.empty(0)
            for k, v in self.data.items()
        ...
        out["s_cat"] = (
            out["s_cat"][:, g_idx].unsqueeze(1)
            if out["s_cat"].numel()
            else out["s_cat"]
        )
        out["s_cont"] = (
            out["s_cont"][:, g_idx].unsqueeze(1)
            if out["s_cont"].numel()
            else out["s_cont"]
        )
        out["id"] = out["id"][:, g_idx]
...
class StatDataset(Dataset):
    def __init__(
        ...
        ), "Length of example longer than encoder length"
        ...
        self.weight_features = [
            ...
            if feature.feature_type == InputTypes.WEIGHT
        ]
        self.encoder_length = encoder_length
        self.example_length = example_length
        ...
        self.stride = stride
        self.split = split
        self.id_col_name = next(
            x.name for x in self.features if x.feature_type == InputTypes.ID
        )
        self.col_dtypes = {
            v.name: DTYPE_MAP[v.feature_embed_type] for v in self.features
        }
        if isinstance(df, pd.DataFrame):
            self.data = df.astype(self.col_dtypes)
        else:
            self.data = pd.read_csv(
                os.path.join(path_stat, "full.csv"), dtype=self.col_dtypes
            )
        self.data = self.data.groupby(self.id_col_name).filter(
        ...
        self.exog = [
            ...
            in [InputTypes.KNOWN, InputTypes.OBSERVED, InputTypes.STATIC]
        ...
        self._cum_examples_in_group = np.cumsum(
            [
                (len(group[group[self.time_feature] > split]) - self.horizon)
                // self.stride
                + 1
                for group in self.grouped
            ]
    ...
    def __getitem__(self, idx):
        ...
        if self.ds_type == "test":
            ...
            test_out = {
                "endog": test_slice[self.endog],
                "exog": test_slice[self.exog],
                "id": test_slice[self.id_col_name],
            }
            ...
            if (self.encoder_length - self.stride * e_idx) > 0:
                train_slice = train[
                    -(self.encoder_length - self.stride * e_idx) :
                ].append(
                    test[
                        max(0, self.stride * e_idx - self.encoder_length) : self.stride
                        * e_idx
                    ]
                )
            else:
            ...
            train_out = {
                "endog": train_slice[self.endog],
                "exog": train_slice[self.exog],
            }
            return train_out
...
class XGBDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        for feat in lag_features_conf:
            assert (
                feat.get("min_value", None) is not None
                or feat.get("value", None) is not None
            )
            if feat.get("min_value", None) is not None:
                assert (
                    feat.get("max_value", None) is not None
                    and feat.get("min_value") > 0
                    and feat.get("max_value") > feat.get("min_value")
                )
                self.lag_features[feat.name] = list(
                    range(feat.get("min_value"), feat.get("max_value") + 1)
                )
            else:
        ...
        for feat in moving_average_features_conf:
            ...
            self.moving_average_features[feat.name] = self.moving_average_features.get(
                feat.name, []
            ) + [feat.window_size]
        self.horizon = example_length - encoder_length
        self.target = [
            feature.name for feature in features if feature.feature_type == "TARGET"
        ]
        self.observed = [
            feature.name for feature in features if feature.feature_type == "OBSERVED"
        ]
        self.known = [
            feature.name
            for feature in features
            if feature.feature_type in ["KNOWN", "STATIC"]
        ]
        assert (
            len(self.target) == 1
        ), "Only 1 target feature is currently supported with xgboost"
        self.data = load_xgb_df(dest_path, features, ds_type)
        self.extra_columns = [
            [f"{k}_{i}" for i in v] for k, v in self.lag_features.items()
        ]
        if MultiID:
            ...
            self.moving_average_features[
                f"{target}_{i}"
            ] = self.moving_average_features.pop(target, [])
            self.data = xgb_multiID_preprocess(
                self.data, features, time_series_count
            )  # XXX need to work with
        self.data = feat_adder(
            self.data, self.lag_features, self.moving_average_features
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.ds_type == "test":
            data_step = select_test_group(
                data_step, self.encoder_length, self.example_length
            )
    ...
    def __getitem__(self, idx):
        ...
        for k, v in self.ndata.items():
            subgraph.ndata[k] = torch.from_numpy(
                v[
                    node_ids,
                    t_idx * self.stride : t_idx * self.stride + self.example_length,
                    :,
                ]
            )
...
def create_datasets(config, input_df=None):
    ...
    if input_df is not None:
        ...
        test = dataset_class(
            df=input_df, **common_args, **specific_args, ds_type="test"
        )
        train = None
        valid = None
    else:
        ...
        path_template = path_template.format(
            extension="bin" if config.get("binarized", False) else "csv"
        )

        train = dataset_class(
            df=path_template.format(subset="train"),
            **common_args,
            **specific_args,
            ds_type="train",
        )
        ...
        test = dataset_class(
            df=path_template.format(subset="test"),
            **common_args,
            **specific_args,
            ds_type="test",
        )
