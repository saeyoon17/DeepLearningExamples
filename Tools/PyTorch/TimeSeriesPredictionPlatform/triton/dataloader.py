...
def get_dataloader_fn(model_dir, batch_size):
    ...
    input_names_dict = {
        ...
        "o_cont": "o_cont__5",
        "target": "target__6",
        "sample_weight": "sample_weight__7",
        "id": "id__8",
    }
    ...
    data_loader = DataLoader(
        ...
        collate_fn=get_collate_fn(
            config.trainer.config.model_type,
            config.trainer.config.encoder_length,
            test=True,
        ),
    )
    ...
    def _get_dataloader():
        for step, (batch, labels, _) in enumerate(data_loader):
            ...
            x = {
                input_names_dict[key]: batch[key].numpy()
                if batch[key].numel()
                else np.full([bs, 1], np.nan)
                for key in input_names_dict.keys()
            }
            weights = (
                batch.ndata["weight"]
            ...
            x["weight__9"] = (
                weights[:, config.dataset.config.encoder_length :, :].numpy()
            ...
            ids = ids[:, 0].numpy()
            y_real = {
                "target__0": np.tile(labels.numpy(), (1, 1, tile_num))
            }  # Probably need to expand the final dims here as well
            yield (ids, x, y_real)
