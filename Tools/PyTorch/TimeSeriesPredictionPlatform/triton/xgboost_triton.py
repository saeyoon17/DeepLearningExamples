...
def format_checkpoint(ckpt, total_features, max_batch_size):
    ...
    os.makedirs(
        os.path.join(main_output_path, "deployment", "navigator_workspace"),
        exist_ok=True,
    )
    os.makedirs(
        os.path.join(
            main_output_path, "deployment", "navigator_workspace", "model-store"
        ),
        exist_ok=True,
    )
    # make model-store
    ...
    for ckpt in os.listdir(checkpoint_path):
        ...
        model.save_model(
            os.path.join(model_store_path, model_name, "1", "xgboost.model")
        )
        # grab the config
        triton_config = generate_config(
        ...
        config_path = os.path.join(
            os.path.join(model_store_path, model_name), "config.pbtxt"
        )
