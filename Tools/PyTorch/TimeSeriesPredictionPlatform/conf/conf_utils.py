...
from data.data_utils import DataTypes, FeatureSpec, InputTypes
from omegaconf import OmegaConf

OmegaConf.register_new_resolver("and", lambda x, y: x and y, use_cache=True)
OmegaConf.register_new_resolver(
    "feature.selector",
    lambda x, feat_type, embed_type: OmegaConf.create(
        [
            elem
            for elem in x
            if elem.feature_type == feat_type and elem.feature_embed_type == embed_type
        ]
    ),
)
...
OmegaConf.register_new_resolver(
    ...
    lambda x: OmegaConf.create([elem.cardinality for elem in x]),
)
...
def sum_nested(*args):
    if len(args) == 1 and isinstance(args[0], (int, float)):
        return args[0]
    return sum(
        arg if isinstance(arg, (int, float)) else sum_nested(*arg) for arg in args
    )
