...
from .backends import AggregatorBackend, AverageMeter, TensorBoardBackend
...
def jsonlog_2_df(path, keys):
    with open(path, "r") as f:
        ...
        log = [
            l for l in log if l["type"] == "LOG" and isinstance(l["step"], (int, list))
        ]
...
def setup_logger(config, resume_training=False):
    ...
    if is_main_process():
        backends = [
            ...
            JSONStreamBackend(
                verbosity=dllogger.Verbosity.VERBOSE, filename=log_filename, append=True
            ),
            AggregatorBackend(
                verbosity=dllogger.Verbosity.VERBOSE, agg_dict={"loss": AverageMeter}
            ),
            StdOutBackend(
