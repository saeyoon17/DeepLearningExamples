...
def preprocess(src_path, dst_path, config):
    ...
    train = pd.concat(
        [x[1] for x in train.groupby(id_col) if len(x[1]) >= config.example_length]
    )
    valid = pd.concat(
        [x[1] for x in valid.groupby(id_col) if len(x[1]) >= config.example_length]
    )
    test = pd.concat(
        [x[1] for x in test.groupby(id_col) if len(x[1]) >= config.example_length]
    )
...
def standarize_electricity(path):
    ...
    df = pd.read_csv(
        os.path.join(path, "LD2011_2014.txt"), index_col=0, sep=";", decimal=","
    )
...
def standarize_volatility(path):
    df = pd.read_csv(
        os.path.join(path, "oxfordmanrealizedvolatilityindices.csv"), index_col=0
    )  # no explicit index
...
def standarize_favorita(data_folder):
    ...
    print(
        "Saving processed file to {}".format(
            os.path.join(data_folder, "standarized.csv")
        )
    )
