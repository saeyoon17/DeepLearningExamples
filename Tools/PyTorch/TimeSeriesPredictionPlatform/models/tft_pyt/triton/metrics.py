    ...
    def metrics(self):
        ...
        if self.config.scale_per_id:
            ...
            unscaled_predictions = torch.stack(
                [
                    _unscale_per_id(
                        self.config, predictions[:, :, i], ids, self.scalers
                    )
                    for i in range(len(self.config.quantiles))
                ],
                dim=-1,
            )
            unscaled_targets = _unscale_per_id(
                self.config, targets[:, :, 0], ids, self.scalers
            ).unsqueeze(-1)
        else:
            ids = None
            unscaled_predictions = torch.stack(
                [
                    _unscale(self.config, predictions[:, :, i], self.scalers[""])
                    for i in range(len(self.config.quantiles))
                ],
                dim=-1,
            )
            unscaled_targets = _unscale(
                self.config, targets[:, :, 0], self.scalers[""]
            ).unsqueeze(-1)
        ...
        return {
            "test_p10": q_risk[0].cpu().numpy(),
            "test_p50": q_risk[1].cpu().numpy(),
            "test_p90": q_risk[2].cpu().numpy(),
        }
