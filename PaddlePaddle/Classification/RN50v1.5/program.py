...
def create_feeds(image_shape):
    ...
    feeds["label"] = paddle.static.data(name="label", shape=[None, 1], dtype="int64")
...
def create_fetchs(out, feeds, class_num, label_smoothing=0, mode=Mode.TRAIN):
    ...
    for key in metric_dict:
        if mode != Mode.TRAIN and paddle.distributed.get_world_size() > 1:
            ...
            metric_dict[key] = metric_dict[key] / paddle.distributed.get_world_size()

        fetchs[key] = (metric_dict[key], AverageMeter(key, "7.4f", need_avg=True))
...
def create_strategy(args, is_train=True):
    ...
    paddle.set_flags(
        {"FLAGS_cudnn_exhaustive_search": True, "FLAGS_conv_workspace_size_limit": 4096}
    )
...
def build(args, main_prog, startup_prog, step_each_epoch, is_train=True):
    ...
    if args.run_scope == RunScope.EVAL_ONLY and paddle.distributed.get_world_size() > 1:
...
def run(
    args, dataloader, exe, program, fetchs, epoch, mode=Mode.TRAIN, lr_scheduler=None
):
    """
    Execute program.
    ...
    metric_dict["compute_time"] = AverageMeter("compute_time", ".5f", postfix=" s,")
    ...
    while True:
        ...
        with profiler.profile_tag(
            ...
            results = exe.run(program=program, feed=feed_dict, fetch_list=fetch_list)
        ...
        if idx % args.print_interval == 0:
            ...
            log_msg["ips"] = batch_size * num_trainers / metric_dict["batch_time"].val
            if mode == Mode.TRAIN:
