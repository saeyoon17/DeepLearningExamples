...
from utils.save_load import init_program, mkdir_if_not_exist, save_model
...
def evaluate(args, exe, logits, dev_program, data_loader):
    ...
    for step, batch in enumerate(data_loader):
        start_logits_tensor, end_logits_tensor = exe.run(
            dev_program, feed=batch, fetch_list=[*logits]
        )
        ...
        if (
            args.benchmark
            and step == args.benchmark_warmup_steps + args.benchmark_steps
        ):
    ...
    if args.benchmark:
        ...
        dllogger.log(
            step=tuple(),
            data={
                "inference_sequences_per_second": args.predict_batch_size
        ...
        dllogger.log(
            ...
            data={
                ...
                "inference_sequences_per_second": len(data_loader.dataset.features)
                / time_to_infer,
            },
        )
    ...
    answers, nbest_answers = get_answers(
        args, data_loader.dataset.examples, data_loader.dataset.features, all_results
    )
...
def main(args):
    ...
    tokenizer = BertTokenizer(
        vocab_file=args.vocab_file, do_lower_case=args.do_lower_case, max_len=512
    )
    ...
    init_program(args, program=main_program, exe=exe, model=model, task=Task.squad)

    if args.do_train:
        dllogger.log(step="PARAMETER", data={"train_start": True})
        dllogger.log(
            step="PARAMETER",
            data={"training_samples": len(train_data_loader.dataset.examples)},
        )
        dllogger.log(
            step="PARAMETER",
            data={"training_features": len(train_data_loader.dataset.features)},
        )
        ...
        for epoch in range(args.epochs):
            for batch in train_data_loader:
                ...
                if (
                    args.benchmark
                    and global_step
                    >= args.benchmark_warmup_steps + args.benchmark_steps
                ):
                    break
        ...
        if trainer_id == 0:
            model_path = os.path.join(args.output_dir, args.bert_model, "squad")
