...
def create_strategy(use_amp, use_distributed_fused_lamb=False):
    ...
    if use_distributed_fused_lamb:
        ...
        build_strategy.reduce_strategy = (
            paddle.static.BuildStrategy.ReduceStrategy._NoReduce
        )
    else:
        build_strategy.fuse_all_reduce_ops = True
        build_strategy.reduce_strategy = (
            paddle.static.BuildStrategy.ReduceStrategy.AllReduce
        )
...
def dist_optimizer(args, optimizer):
    ...
    use_distributed_fused_lamb = (
        True if args.optimizer == "DistributedFusedLamb" else False
    )
    build_strategy, exec_strategy = create_strategy(
        args.amp, use_distributed_fused_lamb
    )
    ...
    if args.amp:
        ...
        custom_black_list = (
            ["lookup_table", "lookup_table_v2"] if args.use_pure_fp16 else None
        )
...
def run(exe, program, args, lr_scheduler, loss, train_dataloader, progress=None):
    """
    Execute program.
    ...
    if args.steps_this_run is not None:
        if args.steps_this_run + last_step > max_steps:
            ...
            if args.benchmark:
                steps_this_run = min(
                    steps_this_run, args.benchmark_warmup_steps + args.benchmark_steps
                )
            max_steps = steps_this_run + last_step
            logging.warning(f"{steps_this_run} steps will be performed in this run.")
    ...
    while True:
        for batch in train_dataloader:
            ...
            if (
                train_iter % (save_steps * gradient_merge_steps) == 0
                ...
                if trainer_id == 0:
                    model_path = os.path.join(
                        args.output_dir,
                        args.bert_model,
                        "phase1" if args.phase1 else "phase2",
                        f"{global_step}",
                    )
                    progress = {
                        "epoch": epoch,
                    ...
                    save_model(program, model_path, args.model_prefix, progress)
