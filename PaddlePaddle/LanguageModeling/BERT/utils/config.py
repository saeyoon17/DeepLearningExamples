...
def _get_full_path_of_ckpt(args):
    ...
    if not os.path.exists(args.from_checkpoint):
        logging.warning(f"Start training from scratch since no checkpoint is found.")
        args.from_checkpoint = None
        args.last_step_of_checkpoint = 0
        return

    target_from_checkpoint = os.path.join(args.from_checkpoint, args.model_prefix)
    if args.last_step_of_checkpoint is None:
        ...
        target_from_checkpoint = os.path.join(args.from_checkpoint, step_with_prefix)
    else:
        try:
...
def check_and_process_args(args, task=Task.pretrain):
    if task == Task.pretrain:
        assert not (
            args.from_checkpoint is not None and args.from_pretrained_params is not None
        ), (
...
def add_global_args(parser, task=Task.pretrain):
    ...
    if task == Task.squad:
        ...
        group.add_argument(
            "--epochs", type=int, default=3, help="The number of epochs for training."
        )
...
def add_training_args(parser, task=Task.pretrain):
    ...
    group.add_argument(
        "--learning-rate", type=float, default=1e-4, help="The initial learning rate."
    )
    group.add_argument(
        "--warmup-start-lr",
        type=float,
    ...
    group.add_argument(
        "--weight-decay", type=float, default=0.01, help="The weight decay coefficient."
    )
    group.add_argument(
        "--max-seq-length",
        default=512,
    ...
    if task == Task.pretrain:
        group.add_argument(
            "--batch-size", type=int, default=32, help="The batch size for training"
        )
        group.add_argument(
            "--phase1",
            action="store_true",
