...
def main():
    ...
    parser = ArgumentParser(
        description="Train a Variational Autoencoder for Collaborative Filtering in TensorFlow"
    )
    parser.add_argument("--train", action="store_true", help="Run training of VAE")
    parser.add_argument("--test", action="store_true", help="Run validation of VAE")
    parser.add_argument(
        "--inference_benchmark",
        action="store_true",
    ...
    parser.add_argument("--anneal_cap", type=float, default=0.1, help="Annealing cap")
    parser.add_argument(
        "--lam", type=float, default=1.00, help="Regularization parameter"
    )
    parser.add_argument("--lr", type=float, default=0.004, help="Learning rate")
    parser.add_argument("--beta1", type=float, default=0.90, help="Adam beta1")
    parser.add_argument("--beta2", type=float, default=0.90, help="Adam beta2")
    parser.add_argument(
        "--top_results",
        type=int,
        default=100,
    ...
    parser.add_argument(
        "--trace", action="store_true", default=False, help="Save profiling traces"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--seed", type=int, default=0, help="Random seed for TensorFlow and numpy"
    )
    parser.add_argument(
        "--data_dir",
        default="/data",
    ...
    if args.batch_size_train % hvd.size() != 0:
        raise ValueError(
            "Global batch size should be a multiple of the number of workers"
        )
    ...
    vae = VAE(
        ...
        batch_size_train=args.local_batch_size,
    ...
    if args.train:
        vae.train(
            ...
            validation_data_true=validation_data_true,
            metrics=metrics,
