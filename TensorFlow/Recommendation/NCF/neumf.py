...
def neural_mf(
    ...
    xmfu = tf.nn.embedding_lookup(
        user_embed[:, :mf_dim], users, partition_strategy="div"
    )
    xmfi = tf.nn.embedding_lookup(
        item_embed[:, :mf_dim], items, partition_strategy="div"
    )
    # MLP Network Embeddings
    xmlpu = tf.nn.embedding_lookup(
        user_embed[:, mf_dim:], users, partition_strategy="div"
    )
    xmlpi = tf.nn.embedding_lookup(
        item_embed[:, mf_dim:], items, partition_strategy="div"
    )
    ...
    logits = tf.layers.Dense(
        1, kernel_initializer=tf.keras.initializers.lecun_uniform()
    ).apply(logits)
...
def compute_eval_metrics(logits, dup_mask, val_batch_size, K):
    ...
    dcg = tf.log(2.0) / tf.log(tf.cast(positive_ranks, tf.float32) + 2)
...
def ncf_model_ops(users, items, labels, dup_mask, params, mode="TRAIN"):
    ...
    with tf.device(None), tf.variable_scope("neumf", custom_getter=custom_getter):
        # Model graph
        logits = neural_mf(
            users,
            items,
