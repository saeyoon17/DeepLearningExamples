...
def parse_args():
    ...
    parser.add_argument(
        "--batch_sizes",
        default="1,4,16,64,256,1024,4096,16384,65536,262144,1048576",
        type=str,
    ...
    parser.add_argument(
        "--xla", dest="xla", action="store_true", default=False, help="Enable XLA"
    )
    parser.add_argument(
        "--log_path",
        default="log.json",
...
def main():
    ...
    for batch_size in batch_sizes:
        ...
        for i in range(args.num_batches):
            ...
            _ = sess.run(
                logits_op,
                feed_dict={users: users_batch, items: items_batch, dropout: 0.0},
            )
            end = time.time()
        ...
        result_data[f"batch_{batch_size}_mean_throughput"] = batch_size / np.mean(
            latencies
        )
