...
from pyspark.sql.types import (ArrayType, FloatType, IntegerType, MapType,
...
documents_df = (
    documents_meta_df.join(
        documents_categories_grouped_df,
        on=F.col("document_id_doc")
...
if evaluation:
    validation_set_df = spark.read.parquet(
        OUTPUT_BUCKET_FOLDER + "validation_set.parquet"
    ).alias("validation_set")

    validation_set_df.select("uuid_event").distinct().createOrReplaceTempView(
        "users_to_profile"
    )
    validation_set_df.select(
        "uuid_event", "document_id_promo"
    ).distinct().createOrReplaceTempView("validation_users_docs_to_ignore")
    ...
    test_set_df = clicks_test_df.join(promoted_content_df, on="ad_id", how="left").join(
        events_df, on="display_id", how="left"
    )

    test_set_df.select("uuid_event").distinct().createOrReplaceTempView(
        "users_to_profile"
    )
    test_set_df.select(
...
page_views_train_df = (
  ...
  """
    ...
    .alias("views")
    .join(
        documents_df, on=F.col("document_id_pv") == F.col("document_id_doc"), how="left"
    )
    .filter(
...
categories_docs_counts = (
    documents_categories_df.groupBy("category_id").count().rdd.collectAsMap()
)
...
with open(
    OUTPUT_BUCKET_FOLDER + "topics_docs_counts" + df_filenames_suffix + ".pickle", "wb"
) as output:
    pickle.dump(topics_docs_counts, output)

entities_docs_counts = (
    documents_entities_df.groupBy("entity_id").count().rdd.collectAsMap()
)
...
page_views_by_user_df = (
    page_views_train_df.select(
        "uuid_pv",
        "document_id_pv",
        ...
        float_list_null_to_empty_list_udf("cat_confidence_level_list").alias(
            "cat_confidence_level_list"
        ),
        int_list_null_to_empty_list_udf("topic_id_list").alias("topic_id_list"),
        float_list_null_to_empty_list_udf("top_confidence_level_list").alias(
            "top_confidence_level_list"
        ),
        str_list_null_to_empty_list_udf("entity_id_list").alias("entity_id_list"),
        float_list_null_to_empty_list_udf("ent_confidence_level_list").alias(
            "ent_confidence_level_list"
        ),
    )
    .groupBy("uuid_pv")
    .agg(
        F.collect_list("document_id_pv").alias("document_id_pv_list"),
...
def generate_user_profile(
    docs_aspects_list, docs_aspects_confidence_list, aspect_docs_counts
):
    docs_aspects = []
    for doc_aspects_list, doc_aspects_confidence_list in zip(
        docs_aspects_list, docs_aspects_confidence_list
    ):
...
generate_categories_user_profile_map_udf = F.udf(
    lambda docs_aspects_list, docs_aspects_confidence_list: generate_user_profile(
        docs_aspects_list, docs_aspects_confidence_list, categories_docs_counts
    ),
...
generate_topics_user_profile_map_udf = F.udf(
    lambda docs_aspects_list, docs_aspects_confidence_list: generate_user_profile(
        docs_aspects_list, docs_aspects_confidence_list, topics_docs_counts
    ),
    MapType(IntegerType(), ArrayType(FloatType()), False),
)
...
generate_entities_user_profile_map_udf = F.udf(
    lambda docs_aspects_list, docs_aspects_confidence_list: generate_user_profile(
        docs_aspects_list, docs_aspects_confidence_list, entities_docs_counts
    ),
    MapType(StringType(), ArrayType(FloatType()), False),
)

users_profile_df = (
    page_views_by_user_df.withColumn("views", get_list_len_udf("document_id_pv_list"))
    .withColumn(
        "categories",
        generate_categories_user_profile_map_udf(
            "category_id_lists", "cat_confidence_level_lists"
        ),
    )
    .withColumn(
        "topics",
        generate_topics_user_profile_map_udf(
            "topic_id_lists", "top_confidence_level_lists"
        ),
    )
    .withColumn(
        "entities",
        generate_entities_user_profile_map_udf(
            "entity_id_lists", "ent_confidence_level_lists"
        ),
    )
    .select(
