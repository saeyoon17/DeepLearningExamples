...
from tensorflow_transform.tf_metadata import (dataset_metadata, dataset_schema,
                                              metadata_io)
from trainer.features import (BOOL_COLUMNS, CATEGORICAL_COLUMNS,
                              DISPLAY_ID_AND_IS_LEAK_ENCODED_COLUMN,
                              DISPLAY_ID_COLUMN,
                              DOC_CATEGORICAL_MULTIVALUED_COLUMNS,
                              FLOAT_COLUMNS, FLOAT_COLUMNS_LOG_BIN_TRANSFORM,
                              FLOAT_COLUMNS_SIMPLE_BIN_TRANSFORM, INT_COLUMNS,
                              IS_LEAK_COLUMN, LABEL_COLUMN)
...
parser.add_argument(
    "--prebatch_size", help="Prebatch size in created tfrecords", type=int, default=4096
)
...
train_feature_vectors_exported_df = spark.read.parquet(
    OUTPUT_BUCKET_FOLDER + train_feature_vector_gcs_folder_name
)
...
train_feature_vectors_integral_csv_rdd_df = (
    train_feature_vectors_exported_df.select(
        "label",
        "display_id",
        "ad_id",
        "document_id",
    ...
    .withColumn("featvec", to_array("feature_vector"))
    .select(
        ["label"]
        + ["display_id"]
        + ["ad_id"]
        ...
        + ["is_leak"]
        + [
            format_number(element, FEAT_CSV_ORDERED_COLUMNS[index]).alias(
                FEAT_CSV_ORDERED_COLUMNS[index]
            )
            for index, element in enumerate(
                [col("featvec")[i] for i in range(len(feature_vector_labels_integral))]
            )
        ]
    )
    .replace(float("nan"), 0)
...
test_validation_feature_vectors_integral_csv_rdd_df = (
...
def make_spec(output_dir, batch_size=None):
    ...
    spec = {}
    spec[LABEL_COLUMN] = tf.FixedLenFeature(
        shape=fixed_shape, dtype=tf.int64, default_value=None
    )
    spec[DISPLAY_ID_COLUMN] = tf.FixedLenFeature(
        shape=fixed_shape, dtype=tf.int64, default_value=None
    )
    spec[IS_LEAK_COLUMN] = tf.FixedLenFeature(
        shape=fixed_shape, dtype=tf.int64, default_value=None
    )
    ...
    for name in FLOAT_COLUMNS_LOG_BIN_TRANSFORM:
        ...
        spec[name + "_log_01scaled"] = tf.FixedLenFeature(
            shape=fixed_shape, dtype=tf.float32, default_value=None
        )
    for name in INT_COLUMNS:
        spec[name + "_log_int"] = tf.FixedLenFeature(
            shape=fixed_shape, dtype=tf.int64, default_value=None
        )
        spec[name + "_log_01scaled"] = tf.FixedLenFeature(
            shape=fixed_shape, dtype=tf.float32, default_value=None
...
def compute_min_max_logs(df):
    ...
    float_expr = []
    for name in (
        trainer.features.FLOAT_COLUMNS_LOG_BIN_TRANSFORM + trainer.features.INT_COLUMNS
    ):
...
all_df = test_validation_feature_vectors_integral_csv_rdd_df.union(
    train_feature_vectors_integral_csv_rdd_df
)
...
def create_tf_example_spark(df, min_logs, max_logs):
    result = {}
    result[LABEL_COLUMN] = tf.train.Feature(
        int64_list=tf.train.Int64List(value=df[LABEL_COLUMN].to_list())
    )
    result[DISPLAY_ID_COLUMN] = tf.train.Feature(
        int64_list=tf.train.Int64List(value=df[DISPLAY_ID_COLUMN].to_list())
    )
    result[IS_LEAK_COLUMN] = tf.train.Feature(
        int64_list=tf.train.Int64List(value=df[IS_LEAK_COLUMN].to_list())
    )
    encoded_value = (
        df[DISPLAY_ID_COLUMN]
        .multiply(10)
        .add(df[IS_LEAK_COLUMN].clip(lower=0))
        .to_list()
    )
    result[DISPLAY_ID_AND_IS_LEAK_ENCODED_COLUMN] = tf.train.Feature(
        int64_list=tf.train.Int64List(value=encoded_value)
    )
    ...
    for name in FLOAT_COLUMNS_SIMPLE_BIN_TRANSFORM:
        ...
        result[name + "_binned"] = tf.train.Feature(
            int64_list=tf.train.Int64List(value=value)
        )
    for name in FLOAT_COLUMNS_LOG_BIN_TRANSFORM:
        value_prelim = (
            df[name].multiply(1000).apply(np.log1p).multiply(1.0 / np.log(2.0))
        )
        ...
        value = (
            value_prelim.add(-min_logs[nn])
            .multiply(1.0 / (max_logs[nn] - min_logs[nn]))
            .to_list()
        )
        result[nn] = tf.train.Feature(float_list=tf.train.FloatList(value=value))
    for name in INT_COLUMNS:
        value_prelim = df[name].apply(np.log1p).multiply(1.0 / np.log(2.0))
        value = value_prelim.astype("int64").to_list()
        result[name + "_log_int"] = tf.train.Feature(
            int64_list=tf.train.Int64List(value=value)
        )
        nn = name + "_log_01scaled"
        value = (
    ...
    for multi_category in DOC_CATEGORICAL_MULTIVALUED_COLUMNS:
        ...
        result[multi_category] = tf.train.Feature(
            int64_list=tf.train.Int64List(value=value)
        )
...
def _transform_to_tfrecords(rdds):
    ...
    for start_ind in range(
        0, num_rows, batch_size if batch_size is not None else 1
    ):  # for each batch
        if start_ind + batch_size - 1 > num_rows:  # if we'd run out of rows
            ...
            examples.append(
                (create_tf_example_spark(csv_slice, min_logs, max_logs), len(csv_slice))
            )
            return examples
        else:
            csv_slice = csv.iloc[
                start_ind : start_ind + (batch_size if batch_size is not None else 1)
            ]
        examples.append(
            (create_tf_example_spark(csv_slice, min_logs, max_logs), batch_size)
        )
    return examples
        ...
        if len(slice[0]) != batch_size:
            ...
            examples.append(
                (
                    bytearray(
                        (
                            create_tf_example_spark(slice[0], min_logs, max_logs)
                        ).SerializeToString()
                    ),
                    None,
                )
            ...
            if TEST_SET_MODE:
                ...
                examples.append(
                    (
                        bytearray(
                            (
                                create_tf_example_spark(csv_slice, min_logs, max_logs)
                            ).SerializeToString()
                        ),
                        None,
                    )
...
TEST_SET_MODE = False
train_features = train_feature_vectors_integral_csv_rdd_df.coalesce(
    30
).rdd.mapPartitions(_transform_to_slices)
...
train_left = train_not_full.coalesce(1).mapPartitions(
    _transform_to_tfrecords_from_reslice
)
all_train = train_examples_full.union(train_left)

TEST_SET_MODE = True
valid_features = test_validation_feature_vectors_integral_csv_rdd_df.coalesce(
    30
).rdd.mapPartitions(_transform_to_slices)
...
valid_left = valid_not_full.coalesce(1).mapPartitions(
    _transform_to_tfrecords_from_reslice
)
all_valid = valid_examples_full.union(valid_left)

all_train.saveAsNewAPIHadoopFile(
    LOCAL_DATA_TFRECORDS_DIR + train_output_string,
    "org.tensorflow.hadoop.io.TFRecordFileOutputFormat",
    keyClass="org.apache.hadoop.io.BytesWritable",
    valueClass="org.apache.hadoop.io.NullWritable",
)

all_valid.saveAsNewAPIHadoopFile(
    LOCAL_DATA_TFRECORDS_DIR + eval_output_string,
    "org.tensorflow.hadoop.io.TFRecordFileOutputFormat",
    keyClass="org.apache.hadoop.io.BytesWritable",
    valueClass="org.apache.hadoop.io.NullWritable",
)
