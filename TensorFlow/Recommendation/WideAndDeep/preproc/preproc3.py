...
conf = (
    ...
    .set("spark.local.dir", SPARK_TEMP_FOLDER)
)
...
timestamp_null_to_zero_int_udf = F.udf(
    lambda x: date_time_to_unix_epoch_treated(x), IntegerType()
)

INT_DEFAULT_NULL_VALUE = -1
int_null_to_minus_one_udf = F.udf(
    lambda x: x if x is not None else INT_DEFAULT_NULL_VALUE, IntegerType()
)
int_list_null_to_empty_list_udf = F.udf(
    lambda x: x if x is not None else [], ArrayType(IntegerType())
)
float_list_null_to_empty_list_udf = F.udf(
    lambda x: x if x is not None else [], ArrayType(FloatType())
)
str_list_null_to_empty_list_udf = F.udf(
    lambda x: x if x is not None else [], ArrayType(StringType())
)
...
truncate_day_from_timestamp_udf = F.udf(
    lambda ts: truncate_day_from_timestamp(ts), IntegerType()
)

extract_country_udf = F.udf(
    lambda geo: geo.strip()[:2] if geo is not None else "", StringType()
)

extract_country_state_udf = F.udf(
    lambda geo: geo.strip()[:5] if geo is not None else "", StringType()
)
...
def convert_odd_timestamp(timestamp_ms_relative):
    ...
    return datetime.datetime.fromtimestamp(
        (int(timestamp_ms_relative) + TIMESTAMP_DELTA) // 1000
    )
...
country_utc_dst_df = pd.read_csv(
    "preproc/data/country_codes_utc_dst_tz_delta.csv", keep_default_na=False
)

countries_utc_dst_dict = dict(
    zip(
        country_utc_dst_df["country_code"].tolist(),
        country_utc_dst_df["utc_dst_time_offset_cleaned"].tolist(),
    )
)
countries_utc_dst_broad = sc.broadcast(countries_utc_dst_dict)

us_states_utc_dst_df = pd.read_csv(
    "preproc/data/us_states_abbrev_bst.csv", keep_default_na=False
)

us_states_utc_dst_dict = dict(
    zip(
        us_states_utc_dst_df["state_abb"].tolist(),
        us_states_utc_dst_df["utc_dst_time_offset_cleaned"].tolist(),
    )
)
us_states_utc_dst_broad = sc.broadcast(us_states_utc_dst_dict)

ca_states_utc_dst_df = pd.read_csv(
    "preproc/data/ca_states_abbrev_bst.csv", keep_default_na=False
)

ca_countries_utc_dst_dict = dict(
    zip(
        ca_states_utc_dst_df["state_abb"].tolist(),
        ca_states_utc_dst_df["utc_dst_time_offset_cleaned"].tolist(),
    )
)
...
page_views_df = (
    spark.read.schema(page_views_schema)
    .options(header="true", inferschema="false", nullValue="\\N")
...
promoted_content_df = (
    spark.read.schema(promoted_content_schema)
    .options(header="true", inferschema="false", nullValue="\\N")
...
documents_meta_df = (
    spark.read.schema(documents_meta_schema)
    .options(header="true", inferschema="false", nullValue="\\N")
...
source_publishers_df = documents_meta_df.select(
...
max_pub = max(source_publishers_df.select(["publisher_id"]).dropna().collect())[
    "publisher_id"
]
max_pub
...
new_publishers = [
    (source, max_pub + 1 + nr) for nr, source in enumerate(source_ids_without_publisher)
]
...
documents_meta_df = documents_meta_df.drop("publisher_id").join(
    fixed_source_publishers_df, on="source_id"
)
documents_meta_df.count()
...
events_joined_df = (
    events_df.join(
        documents_meta_df.withColumnRenamed("source_id", "source_id_doc_event")
        .withColumnRenamed("publisher_id", "publisher_doc_event")
        .withColumnRenamed("publish_time", "publish_time_doc_event"),
        on=F.col("document_id_event") == F.col("document_id_doc"),
        how="left",
    )
    .join(
        page_views_df,
        on=[
            F.col("uuid_event") == F.col("uuid_pv"),
...
documents_categories_df = (
    spark.read.schema(documents_categories_schema)
    .options(header="true", inferschema="false", nullValue="\\N")
...
documents_categories_grouped_df = (
    documents_categories_df.groupBy("document_id_cat")
    .agg(
        F.collect_list("category_id").alias("category_id_list"),
...
documents_topics_df = (
    spark.read.schema(documents_topics_schema)
    .options(header="true", inferschema="false", nullValue="\\N")
...
documents_topics_grouped_df = (
    documents_topics_df.groupBy("document_id_top")
    .agg(
        F.collect_list("topic_id").alias("topic_id_list"),
...
documents_entities_df = (
    spark.read.schema(documents_entities_schema)
    .options(header="true", inferschema="false", nullValue="\\N")
...
documents_entities_grouped_df = (
    documents_entities_df.groupBy("document_id_ent")
    .agg(
        F.collect_list("entity_id").alias("entity_id_list"),
...
clicks_train_df = (
    spark.read.schema(clicks_train_schema)
    .options(header="true", inferschema="false", nullValue="\\N")
    .csv(DATA_BUCKET_FOLDER + "clicks_train.csv")
    .withColumn("dummyClicksTrain", F.lit(1))
    .alias("clicks_train")
)

clicks_train_joined_df = (
    clicks_train_df.join(promoted_content_df, on="ad_id", how="left")
    .join(
        documents_meta_df,
        on=F.col("promoted_content.document_id_promo")
        == F.col("documents_meta.document_id_doc"),
        how="left",
    )
...
user_profiles_df = (
    spark.read.parquet(OUTPUT_BUCKET_FOLDER + table_name)
...
if evaluation:
    ...
    validation_set_exported_df.select("display_id").distinct().createOrReplaceTempView(
    ...
    validation_set_df = (
      ...
      WHERE display_id = t.display_id)"""
        ...
        .join(
            documents_categories_grouped_df,
            on=F.col("document_id_promo")
            == F.col("documents_categories_grouped.document_id_cat"),
            how="left",
        )
        .join(
            documents_topics_grouped_df,
            on=F.col("document_id_promo")
            == F.col("documents_topics_grouped.document_id_top"),
            how="left",
        )
        .join(
            documents_entities_grouped_df,
            on=F.col("document_id_promo")
            == F.col("documents_entities_grouped.document_id_ent"),
            how="left",
        )
        .join(
            documents_categories_grouped_df.withColumnRenamed(
                "category_id_list", "doc_event_category_id_list"
            )
            .withColumnRenamed(
                "confidence_level_cat_list", "doc_event_confidence_level_cat_list"
            )
            .alias("documents_event_categories_grouped"),
            on=F.col("document_id_event")
            == F.col("documents_event_categories_grouped.document_id_cat"),
            how="left",
        )
        .join(
            documents_topics_grouped_df.withColumnRenamed(
                "topic_id_list", "doc_event_topic_id_list"
            )
            .withColumnRenamed(
                "confidence_level_top_list", "doc_event_confidence_level_top_list"
            )
            .alias("documents_event_topics_grouped"),
            on=F.col("document_id_event")
            == F.col("documents_event_topics_grouped.document_id_top"),
            how="left",
        )
        .join(
            documents_entities_grouped_df.withColumnRenamed(
                "entity_id_list", "doc_event_entity_id_list"
            )
            .withColumnRenamed(
                "confidence_level_ent_list", "doc_event_confidence_level_ent_list"
            )
            .alias("documents_event_entities_grouped"),
            on=F.col("document_id_event")
            == F.col("documents_event_entities_grouped.document_id_ent"),
            how="left",
        )
        .join(
            page_views_users_df,
            on=[
                F.col("clicks.uuid_event") == F.col("page_views_users.uuid_pv"),
                F.col("clicks.document_id_promo")
                == F.col("page_views_users.document_id_pv"),
            ],
            how="left",
        )
    )
    ...
    validation_set_ground_truth_df = (
        validation_set_df.filter("clicked = 1")
        .join(
            user_profiles_df,
    ...
    clicks_test_df = (
        spark.read.schema(clicks_test_schema)
        .options(header="true", inferschema="false", nullValue="\\N")
    ...
    test_set_df = (
        clicks_test_df.join(promoted_content_df, on="ad_id", how="left")
        .join(
            documents_meta_df,
            on=F.col("promoted_content.document_id_promo")
            == F.col("documents_meta.document_id_doc"),
        ...
        .join(
            page_views_users_df,
            on=[
                F.col("events.uuid_event") == F.col("page_views_users.uuid_pv"),
                F.col("promoted_content.document_id_promo")
                == F.col("page_views_users.document_id_pv"),
            ],
            how="left",
        )
...
def get_category_field_values_counts(field, df, min_threshold=10):
    category_counts = dict(
        list(
            filter(
                lambda x: not is_null(x[0]) and x[1] >= min_threshold,
                df.select(field)
                .groupBy(field)
                .count()
...
event_country_values_counts = get_category_field_values_counts(
    "event_country", events_df, min_threshold=10
)
len(event_country_values_counts)
# All non-null categories: 230

event_country_state_values_counts = get_category_field_values_counts(
    "event_country_state", events_df, min_threshold=10
)
len(event_country_state_values_counts)

event_geo_location_values_counts = get_category_field_values_counts(
    "geo_location_event", events_df, min_threshold=10
)
len(event_geo_location_values_counts)
# All non-null categories: 2988

doc_entity_id_values_counts = get_category_field_values_counts(
    "entity_id", documents_entities_df, min_threshold=10
)
...
ad_id_popularity = (
    ad_id_popularity_df.filter("views > 5")
    .select("ad_id", "ctr", "views")
    .rdd.map(lambda x: (x["ad_id"], (x["ctr"], x["views"], 1, 1)))
    .collectAsMap()
)
...
ad_id_avg_ctr = sum(map(lambda x: x[0], ad_id_popularity.values())) / float(
    len(ad_id_popularity)
)
ad_id_avg_ctr

ad_id_weighted_avg_ctr = sum(
    map(lambda x: x[0] * x[1], ad_id_popularity.values())
) / float(sum(map(lambda x: x[1], ad_id_popularity.values())))
ad_id_weighted_avg_ctr

ad_id_views_median = np.median(
    np.array(list(map(lambda x: x[1], ad_id_popularity.values())))
)
ad_id_views_median

ad_id_views_mean = sum(map(lambda x: x[1], ad_id_popularity.values())) / float(
    len(ad_id_popularity)
)
ad_id_views_mean

# ### Average CTR by document_id (promoted_content)
document_id_popularity_df = (
    train_set_df.groupby("document_id_promo")
    .agg(
...
document_id_popularity = (
    document_id_popularity_df.filter("views > 5")
    .select("document_id_promo", "ctr", "views", "distinct_ad_ids")
    .rdd.map(
        lambda x: (
            x["document_id_promo"],
            (x["ctr"], x["views"], x["distinct_ad_ids"], 1),
        )
    )
    .collectAsMap()
)
...
document_id_avg_ctr = sum(map(lambda x: x[0], document_id_popularity.values())) / float(
    len(document_id_popularity)
)
document_id_avg_ctr

document_id_weighted_avg_ctr = sum(
    list(map(lambda x: x[0] * x[1], document_id_popularity.values()))
) / float(sum(list(map(lambda x: x[1], document_id_popularity.values()))))
document_id_weighted_avg_ctr

document_id_views_median = np.median(
    np.array(list(map(lambda x: x[1], document_id_popularity.values())))
)
document_id_views_median

document_id_views_mean = sum(
    map(lambda x: x[1], document_id_popularity.values())
) / float(len(document_id_popularity))
document_id_views_mean

# ### Average CTR by (doc_event, doc_ad)
doc_event_doc_ad_avg_ctr_df = (
    train_set_df.groupBy("document_id_event", "document_id_promo")
    .agg(
        F.sum("clicked").alias("clicks"),
        F.count("*").alias("views"),
        F.countDistinct("ad_id").alias("distinct_ad_ids"),
...
doc_event_doc_ad_avg_ctr = (
    doc_event_doc_ad_avg_ctr_df.filter("views > 5")
    .select("document_id_event", "document_id_promo", "ctr", "views", "distinct_ad_ids")
    .rdd.map(
        lambda x: (
            (x["document_id_event"], x["document_id_promo"]),
            (x["ctr"], x["views"], x["distinct_ad_ids"], 1),
        )
    )
    .collectAsMap()
...
source_id_by_country_popularity_df = (
    train_set_df.select("clicked", "source_id", "event_country", "ad_id")
...
source_id_by_country_popularity = (
    source_id_by_country_popularity_df.filter(
        'views > 5 and source_id is not null and event_country <> ""'
    )
    .select("event_country", "source_id", "ctr", "views", "distinct_ad_ids")
    .rdd.map(
        lambda x: (
            (x["event_country"], x["source_id"]),
            (x["ctr"], x["views"], x["distinct_ad_ids"], 1),
        )
    )
    .collectAsMap()
...
source_id_by_country_avg_ctr = sum(
    map(lambda x: x[0], source_id_by_country_popularity.values())
) / float(len(source_id_by_country_popularity))
source_id_by_country_avg_ctr

source_id_by_country_weighted_avg_ctr = sum(
    map(lambda x: x[0] * x[1], source_id_by_country_popularity.values())
) / float(sum(map(lambda x: x[1], source_id_by_country_popularity.values())))
...
source_id_by_country_views_mean = sum(
    map(lambda x: x[1], source_id_by_country_popularity.values())
) / float(len(source_id_by_country_popularity))
source_id_by_country_views_mean

# ### Average CTR by source_id
source_id_popularity_df = (
    train_set_df.select("clicked", "source_id", "ad_id")
    .groupby("source_id")
    .agg(
        F.sum("clicked").alias("clicks"),
        F.count("*").alias("views"),
...
source_id_popularity = (
    source_id_popularity_df.filter("views > 10 and source_id is not null")
    .select("source_id", "ctr", "views", "distinct_ad_ids")
    .rdd.map(
        lambda x: (x["source_id"], (x["ctr"], x["views"], x["distinct_ad_ids"], 1))
    )
    .collectAsMap()
)
...
publisher_popularity_df = (
    train_set_df.select("clicked", "publisher_id", "ad_id")
...
publisher_popularity = (
    publisher_popularity_df.filter("views > 10 and publisher_id is not null")
    .select("publisher_id", "ctr", "views", "distinct_ad_ids")
    .rdd.map(
        lambda x: (x["publisher_id"], (x["ctr"], x["views"], x["distinct_ad_ids"], 1))
    )
    .collectAsMap()
)
...
advertiser_id_popularity_df = (
    train_set_df.select("clicked", "advertiser_id", "ad_id")
...
advertiser_id_popularity = (
    advertiser_id_popularity_df.filter("views > 10 and advertiser_id is not null")
    .select("advertiser_id", "ctr", "views", "distinct_ad_ids")
    .rdd.map(
        lambda x: (x["advertiser_id"], (x["ctr"], x["views"], x["distinct_ad_ids"], 1))
    )
    .collectAsMap()
)
...
campaign_id_popularity_df = (
    train_set_df.select("clicked", "campaign_id", "ad_id")
    .groupby("campaign_id")
    .agg(
        F.sum("clicked").alias("clicks"),
        F.count("*").alias("views"),
...
campaign_id_popularity = (
    campaign_id_popularity_df.filter("views > 10 and campaign_id is not null")
    .select("campaign_id", "ctr", "views", "distinct_ad_ids")
    .rdd.map(
        lambda x: (x["campaign_id"], (x["ctr"], x["views"], x["distinct_ad_ids"], 1))
    )
    .collectAsMap()
)
...
category_id_popularity_df = (
    train_set_df.join(
        documents_categories_df.alias("cat_local"),
        on=F.col("document_id_promo") == F.col("cat_local.document_id_cat"),
        how="inner",
    )
...
category_id_popularity = (
    category_id_popularity_df.filter("views > 10")
    .select(
        "category_id", "ctr", "views", "avg_confidence_level_cat", "distinct_ad_ids"
    )
    .rdd.map(
        lambda x: (
            x["category_id"],
            (x["ctr"], x["views"], x["distinct_ad_ids"], x["avg_confidence_level_cat"]),
        )
    )
    .collectAsMap()
)
...
sum(map(lambda x: x[1], category_id_popularity.values())) / float(
    len(category_id_popularity)
)
...
category_id_by_country_popularity_df = (
    train_set_df.join(
        documents_categories_df.alias("cat_local"),
        on=F.col("document_id_promo") == F.col("cat_local.document_id_cat"),
        how="inner",
...
category_id_by_country_popularity = (
    category_id_by_country_popularity_df.filter('views > 10 and event_country <> ""')
    .select(
        "event_country",
        "category_id",
        "ctr",
        "views",
        "avg_confidence_level_cat",
        "distinct_ad_ids",
    )
    .rdd.map(
        lambda x: (
            (x["event_country"], x["category_id"]),
            (x["ctr"], x["views"], x["distinct_ad_ids"], x["avg_confidence_level_cat"]),
        )
    )
    .collectAsMap()
...
category_id_by_country_popularity_broad = sc.broadcast(
    category_id_by_country_popularity
)

# ### Average CTR by Topic
topic_id_popularity_df = (
    train_set_df.join(
        documents_topics_df.alias("top_local"),
        on=F.col("document_id_promo") == F.col("top_local.document_id_top"),
        how="inner",
    )
...
topic_id_popularity = (
    topic_id_popularity_df.filter("views > 10")
    .select("topic_id", "ctr", "views", "avg_confidence_level_top", "distinct_ad_ids")
    .rdd.map(
        lambda x: (
            x["topic_id"],
            (x["ctr"], x["views"], x["distinct_ad_ids"], x["avg_confidence_level_top"]),
        )
    )
    .collectAsMap()
)
...
sum(map(lambda x: x[2] * x[1], topic_id_popularity.values())) / float(
    len(topic_id_popularity)
)

# ### Average CTR by (country, topic)
topic_id_by_country_popularity_df = (
    train_set_df.join(
        documents_topics_df.alias("top_local"),
        on=F.col("document_id_promo") == F.col("top_local.document_id_top"),
        how="inner",
...
topic_id_id_by_country_popularity = (
    topic_id_by_country_popularity_df.filter('views > 10 and event_country <> ""')
    .select(
        "event_country",
        "topic_id",
        "ctr",
        "views",
        "avg_confidence_level_top",
        "distinct_ad_ids",
    )
    .rdd.map(
        lambda x: (
            (x["event_country"], x["topic_id"]),
            (x["ctr"], x["views"], x["distinct_ad_ids"], x["avg_confidence_level_top"]),
        )
    )
    .collectAsMap()
...
topic_id_id_by_country_popularity_broad = sc.broadcast(
    topic_id_id_by_country_popularity
)

# ### Average CTR by Entity
entity_id_popularity_df = (
    train_set_df.join(
        documents_entities_df.alias("ent_local"),
        on=F.col("document_id_promo") == F.col("ent_local.document_id_ent"),
        how="inner",
    )
...
entity_id_popularity = (
    entity_id_popularity_df.filter("views > 5")
    .select("entity_id", "ctr", "views", "avg_confidence_level_ent", "distinct_ad_ids")
    .rdd.map(
        lambda x: (
            x["entity_id"],
            (x["ctr"], x["views"], x["distinct_ad_ids"], x["avg_confidence_level_ent"]),
        )
    )
    .collectAsMap()
)
...
sum(map(lambda x: x[1], entity_id_popularity.values())) / float(
    len(entity_id_popularity)
)

# ### Average CTR by (country, entity)
entity_id_by_country_popularity_df = (
    train_set_df.join(
        documents_entities_df.alias("ent_local"),
        on=F.col("document_id_promo") == F.col("ent_local.document_id_ent"),
        how="inner",
...
entity_id_by_country_popularity = (
    entity_id_by_country_popularity_df.filter('views > 5 and event_country <> ""')
    .select(
        "event_country",
        "entity_id",
        "ctr",
...
with open(
    OUTPUT_BUCKET_FOLDER + "categories_docs_counts" + df_filenames_suffix + ".pickle",
    "rb",
) as input_file:
...
with open(
    OUTPUT_BUCKET_FOLDER + "topics_docs_counts" + df_filenames_suffix + ".pickle", "rb"
) as input_file:
    topics_docs_counts = cPickle.load(input_file)
len(topics_docs_counts)

with open(
    OUTPUT_BUCKET_FOLDER + "entities_docs_counts" + df_filenames_suffix + ".pickle",
    "rb",
) as input_file:
...
publish_times_df = (
    train_set_df.filter("publish_time is not null")
    .select("document_id_promo", "publish_time")
    .distinct()
    .select(F.col("publish_time").cast(IntegerType()))
)

publish_time_percentiles = get_percentiles(
    publish_times_df, "publish_time", quantiles_levels=[0.5], max_error_rate=0.001
)
...
for d in ref_dates:
    print(
        ...
        get_time_decay_factor(d, alpha=TIME_DECAY_ALPHA),
    )
...
decay_factor_default = get_time_decay_factor(
    publish_time_median, timestamp_ref, alpha=TIME_DECAY_ALPHA
)
...
def get_confidence_sample_size(sample, max_for_reference=100000):
    ...
    ref_log = math.log(
        1 + max_for_reference, 2
    )  # Curiosly reference in log  with base 2 gives a slightly higher score, so I will keep
...
def get_popularity(an_id, a_dict):
    return (
        (
            a_dict[an_id][0],
            get_confidence_sample_size(a_dict[an_id][1] / float(a_dict[an_id][2]))
            * a_dict[an_id][3],
        )
        if an_id in a_dict
        else (None, None)
...
def get_weighted_avg_popularity_from_list(ids_list, confidence_ids_list, pop_dict):
    pops = list(
        filter(
            lambda x: x[0][0] is not None,
            [
                (get_popularity(an_id, pop_dict), confidence)
                for an_id, confidence in zip(ids_list, confidence_ids_list)
            ],
        )
    )
    # print("pops",pops)
...
def get_weighted_avg_country_popularity_from_list(
    event_country, ids_list, confidence_ids_list, pop_dict
):
    pops = list(
        filter(
            lambda x: x[0][0] is not None,
            [
                (get_popularity((event_country, an_id), pop_dict), confidence)
                for an_id, confidence in zip(ids_list, confidence_ids_list)
            ],
        )
    )
...
def get_popularity_score(
    ...
    avg_ctr, confidence = get_popularity(
    ...
    avg_ctr, confidence = get_popularity(
        (document_id_event, document_id), doc_event_doc_ad_avg_ctr_broad.value
    )
    if avg_ctr is not None:
    ...
    if source_id != -1:
        ...
        if event_country != "":
            avg_ctr, confidence = get_popularity(
                (event_country, source_id), source_id_by_country_popularity_broad.value
            )
        if avg_ctr is not None:
            probs.append(("pop_source_id_country", avg_ctr, confidence))

        avg_ctr, confidence = get_popularity(
            source_id, source_id_popularity_broad.value
        )
        if avg_ctr is not None:
    ...
    if publisher_id is not None:
        avg_ctr, confidence = get_popularity(
            publisher_id, publisher_popularity_broad.value
        )
        if avg_ctr is not None:
    ...
    if advertiser_id is not None:
        avg_ctr, confidence = get_popularity(
            advertiser_id, advertiser_id_popularity_broad.value
        )
        if avg_ctr is not None:
    ...
    if campaign_id is not None:
        avg_ctr, confidence = get_popularity(
            campaign_id, campaign_id_popularity_broad.value
        )
        if avg_ctr is not None:
    ...
    if output_detailed_list:
        ...
        if len(probs) > 0:
            ...
            weighted_avg_probs_by_confidence = sum(
                map(lambda x: x[1] * x[2], probs)
            ) / float(sum(map(lambda x: x[2], probs)))
...
def cosine_similarity_user_docs_aspects(
    user_aspect_profile, doc_aspect_ids, doc_aspects_confidence, aspect_docs_counts
):
    if (
        user_aspect_profile is None
        or len(user_aspect_profile) == 0
        or doc_aspect_ids is None
        or len(doc_aspect_ids) == 0
    ):
        return None, None
    ...
    similarity, intersections = cosine_similarity_dicts(
        doc_aspects_tfidf_confid, user_aspects_tfidf_confid
    )

    if intersections > 0:
        # P(A intersect B)_intersections = P(A)^intersections * P(B)^intersections
        random_error = math.pow(
            len(doc_aspects) / float(len(aspect_docs_counts)), intersections
        ) * math.pow(
            len(user_aspect_profile) / float(len(aspect_docs_counts)), intersections
        )
    else:
        # P(A not intersect B) = 1 - P(A intersect B)
        random_error = 1 - (
            (len(doc_aspects) / float(len(aspect_docs_counts)))
...
def cosine_similarity_doc_event_doc_ad_aspects(
    doc_event_aspect_ids,
    ...
    if (
        doc_event_aspect_ids is None
        or len(doc_event_aspect_ids) == 0
        or doc_ad_aspect_ids is None
        or len(doc_ad_aspect_ids) == 0
    ):
        return None, None
    ...
    similarity, intersections = cosine_similarity_dicts(
        doc_event_aspects_tfidf_confid, doc_ad_aspects_tfidf_confid
    )

    if intersections > 0:
        # P(A intersect B)_intersections = P(A)^intersections * P(B)^intersections
        random_error = math.pow(
            len(doc_event_aspect_ids) / float(len(aspect_docs_counts)), intersections
        ) * math.pow(
            len(doc_ad_aspect_ids) / float(len(aspect_docs_counts)), intersections
        )
    else:
        # P(A not intersect B) = 1 - P(A intersect B)
        random_error = 1 - (
            (len(doc_event_aspect_ids) / float(len(aspect_docs_counts)))
...
def get_user_cb_interest_score(
    user_views_count,
    user_categories,
    user_topics,
    user_entities,
    ...
    categories_similarity, cat_sim_confidence = cosine_similarity_user_docs_aspects(
        user_categories,
        category_ids_by_doc,
        cat_confidence_level_by_doc,
    ...
    if categories_similarity is not None:
        sims.append(
            ("user_doc_ad_sim_categories", categories_similarity, cat_sim_confidence)
        )

    topics_similarity, top_sim_confidence = cosine_similarity_user_docs_aspects(
        user_topics, topic_ids_by_doc, top_confidence_level_by_doc, topics_docs_counts
    )
    if topics_similarity is not None:
        sims.append(("user_doc_ad_sim_topics", topics_similarity, top_sim_confidence))

    entities_similarity, entity_sim_confid = cosine_similarity_user_docs_aspects(
        user_entities,
        entity_ids_by_doc,
        ent_confidence_level_by_doc,
    ...
    if entities_similarity is not None:
        sims.append(
            ("user_doc_ad_sim_entities", entities_similarity, entity_sim_confid)
        )
    ...
    if output_detailed_list:
        ...
        if len(sims) > 0:
            weighted_avg_sim_by_confidence = sum(
                map(lambda x: x[1] * x[2], sims)
            ) / float(sum(map(lambda x: x[2], sims)))
...
def get_doc_event_doc_ad_cb_similarity_score(
    ...
    if topics_similarity is not None:
        sims.append(
            ("doc_event_doc_ad_sim_topics", topics_similarity, top_sim_confidence)
        )
    ...
    if entities_similarity is not None:
        sims.append(
            ("doc_event_doc_ad_sim_entities", entities_similarity, entity_sim_confid)
        )
...
bool_feature_names = ["event_weekend", "user_has_already_viewed_doc"]
...
feature_vector_labels_integral_dict = dict(
    [(key, idx) for idx, key in enumerate(feature_vector_labels_integral)]
)
...
def set_feature_vector_cat_value_integral(field_name, field_value, feature_vector):
    if not is_null(field_value):  # and str(field_value) != '-1':
        feature_vector[feature_vector_labels_integral_dict[field_name]] = float(
            field_value
        )
...
def set_feature_vector_cat_top_multi_values_integral(
    ...
    top_values = list(
        filter(
            lambda z: z != -1,
            map(lambda y: y[0], sorted(zip(values, confidences), key=lambda x: -x[1])),
        )
    )[:top]
...
def get_ad_feature_vector_integral(
    ...
    try:
        ...
        if user_views_count is not None:
            feature_vector[feature_vector_labels_integral_dict["user_views"]] = float(
                user_views_count
            )

        if user_doc_ids_viewed is not None:
            feature_vector[
                feature_vector_labels_integral_dict["user_has_already_viewed_doc"]
        ...
        if timestamp_event > -1:
            ...
            if doc_ad_publish_time is not None:
                ...
                if 0 <= delta_days <= 365 * 10:  # 10 years
                    feature_vector[
                        feature_vector_labels_integral_dict[
                            "doc_ad_days_since_published"
                        ]
                    ] = float(delta_days)
            ...
            dt_local_timestamp_event = get_local_datetime(
                dt_timestamp_event, event_country, event_country_state
            )
            local_hour_bin = get_hour_bin(dt_local_timestamp_event.hour)
            feature_vector[
                feature_vector_labels_integral_dict["doc_event_hour"]
            ] = float(
                local_hour_bin
            )  # Hour for Decision Trees
            set_feature_vector_cat_value_integral(
                EVENT_HOUR_FV, local_hour_bin, feature_vector
            )  # Period of day for FFM

            # Weekend
            weekend = int(is_weekend(dt_local_timestamp_event))
            feature_vector[
                feature_vector_labels_integral_dict["event_weekend"]
            ] = float(weekend)
        ...
        pop_scores = get_popularity_score(
            event_country,
            ad_id,
            document_id,
            source_id,
        ...
        for score in pop_scores:
            ...
            feature_vector[
                feature_vector_labels_integral_dict[score[0] + conf_field_suffix]
            ] = score[2]
            feature_vector[
                feature_vector_labels_integral_dict[
        ...
        set_feature_vector_cat_value_integral(
            EVENT_COUNTRY_FV, event_country_code, feature_vector
        )

        # Process code for event_country_state
        if event_country_state in event_country_state_values_counts:
            event_country_state_code = event_country_state_values_counts[
                event_country_state
            ]
        else:
            event_country_state_code = event_country_state_values_counts[
        ...
        set_feature_vector_cat_value_integral(
            EVENT_COUNTRY_STATE_FV, event_country_state_code, feature_vector
        )

        # Process code for geo_location_event
        if geo_location_event in event_geo_location_values_counts:
            geo_location_event_code = event_geo_location_values_counts[
                geo_location_event
            ]
        else:
            geo_location_event_code = event_geo_location_values_counts[
        ...
        if traffic_source_pv is not None:
            feature_vector[
                feature_vector_labels_integral_dict[TRAFFIC_SOURCE_FV]
            ] = int(traffic_source_pv - 1)
        if platform_event is not None:
            feature_vector[
                feature_vector_labels_integral_dict[EVENT_PLATFORM_FV]
            ] = int(platform_event - 1)

        set_feature_vector_cat_value_integral(
            EVENT_GEO_LOCATION_FV, geo_location_event_code, feature_vector
        )
        ...
        set_feature_vector_cat_value_integral(
            AD_ADVERTISER_FV, advertiser_id, feature_vector
        )
        set_feature_vector_cat_value_integral(
            DOC_AD_SOURCE_ID_FV, source_id, feature_vector
        )
        set_feature_vector_cat_value_integral(
            DOC_AD_PUBLISHER_ID_FV, publisher_id, feature_vector
        )
        set_feature_vector_cat_value_integral(
            DOC_EVENT_SOURCE_ID_FV, doc_event_source_id, feature_vector
        )
        set_feature_vector_cat_value_integral(
            DOC_EVENT_PUBLISHER_ID_FV, doc_event_publisher_id, feature_vector
        )

        set_feature_vector_cat_top_multi_values_integral(
            DOC_AD_CATEGORY_ID_FV,
            doc_ad_category_ids,
            doc_ad_cat_confidence_levels,
            feature_vector,
            top=3,
        ...
        doc_ad_entity_ids_codes = [
            doc_entity_id_values_counts[x]
        ...
        raise Exception(
            "[get_ad_feature_vector_integral] ERROR PROCESSING FEATURE VECTOR! Params: {}".format(
                [
                    user_doc_ids_viewed,
                    user_views_count,
                    user_categories,
...
train_set_enriched_df = (
    ...
    .select(
        ...
        "document_id_event",
        "traffic_source_pv",
        int_list_null_to_empty_list_udf("doc_event_category_id_list").alias(
            "doc_event_category_id_list"
        ),
        float_list_null_to_empty_list_udf("doc_event_confidence_level_cat_list").alias(
            "doc_event_confidence_level_cat_list"
        ),
        int_list_null_to_empty_list_udf("doc_event_topic_id_list").alias(
            "doc_event_topic_id_list"
        ),
        float_list_null_to_empty_list_udf("doc_event_confidence_level_top_list").alias(
            "doc_event_confidence_level_top_list"
        ),
        str_list_null_to_empty_list_udf("doc_event_entity_id_list").alias(
            "doc_event_entity_id_list"
        ),
        float_list_null_to_empty_list_udf("doc_event_confidence_level_ent_list").alias(
        ...
        float_list_null_to_empty_list_udf("confidence_level_cat_list").alias(
            "confidence_level_cat_list"
        ),
        int_list_null_to_empty_list_udf("topic_id_list").alias("topic_id_list"),
        float_list_null_to_empty_list_udf("confidence_level_top_list").alias(
            "confidence_level_top_list"
        ),
        str_list_null_to_empty_list_udf("entity_id_list").alias("entity_id_list"),
        float_list_null_to_empty_list_udf("confidence_level_ent_list").alias(
...
train_set_feature_vectors_df = train_set_enriched_df.withColumn(
    "feature_vector",
...
train_set_feature_vectors_df.write.parquet(
    OUTPUT_BUCKET_FOLDER + train_feature_vector_gcs_folder_name, mode="overwrite"
)
...
is_leak_udf = F.udf(
    lambda max_timestamp_pv_leak, timestamp_event: int(
        is_leak(max_timestamp_pv_leak, timestamp_event)
    ),
    IntegerType(),
)
...
test_validation_set_enriched_df = (
    data_df.select(
        "display_id",
        "uuid_event",
        "event_country",
...
test_validation_set_feature_vectors_df = test_validation_set_enriched_df.withColumn(
    "feature_vector",
    get_ad_feature_vector_integral_udf(
        "user_doc_ids_viewed",
        "user_views_count",
