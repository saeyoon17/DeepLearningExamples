...
def create_parser():
    ...
    parser.add_argument(
        "--eval_batch_size", help="Evaluation batch size", default=32768, type=int
    )
    parser.add_argument(
        "--eval_steps",
    ...
    parser.add_argument("--num_epochs", help="Number of epochs", default=120, type=int)
    parser.add_argument(
        "--eval_epoch_interval",
    ...
    parser.add_argument("--xla", help="Enable XLA", default=False, action="store_true")
    parser.add_argument(
        "--gpu", help="Run computations on the GPU", default=False, action="store_true"
    )
    parser.add_argument(
        "--amp",
    ...
    parser.add_argument("--hvd", help="Use Horovod", action="store_true", default=False)
    parser.add_argument(
    ...
    parser.add_argument("--reader_num_threads", default=12, type=int)
    parser.add_argument("--parser_num_threads", default=3, type=int)
    parser.add_argument("--prefetch_buffer_size", default=1, type=int)
...
def construct_estimator(
    ...
    assert model_type in [
        ...
        DEEP,
        WIDE_N_DEEP,
    ], "Canned estimator only supports basic wide, deep, wnd"
    if model_type == WIDE:
        estimator = tf.estimator.LinearClassifier(
            feature_columns=wide_columns, config=run_config, optimizer=wide_optimizer
        )
...
def main(FLAGS):
    ...
    if not FLAGS.hvd or hvd.local_rank() == 0:
        tf.compat.v1.logging.warn(
            "command line arguments: {}".format(json.dumps(vars(FLAGS)))
        )
    ...
    if FLAGS.gpu:
        ...
        session_config = tf.compat.v1.ConfigProto(
            device_count={"GPU": 0}, log_device_placement=FLAGS.log_device_placement
        )
    ...
    save_checkpoints_steps = (
        FLAGS.benchmark_steps + 1
        if FLAGS.benchmark
        else int(FLAGS.eval_epoch_interval * steps_per_epoch)
    )
    ...
    if FLAGS.predict or FLAGS.evaluate:  # inference
        if FLAGS.benchmark:
            benchmark_hook = BenchmarkLoggingHook(
                global_batch_size=FLAGS.eval_batch_size,
        ...
        predict_result_iter = estimator.predict(
            input_fn=eval_input_fn, hooks=hooks, yield_single_examples=False
        )
        ...
        if FLAGS.benchmark:
            ...
            result = estimator.evaluate(
                eval_input_fn, hooks=hooks, steps=FLAGS.eval_steps
            )
            dllogger.log(
                step=(),
                data={
        ...
        if FLAGS.benchmark:
            ...
            train_spec = tf.estimator.TrainSpec(
                input_fn=train_input_fn, max_steps=max_steps, hooks=hooks
            )
            eval_spec = tf.estimator.EvalSpec(
                input_fn=eval_input_fn, throttle_secs=0, steps=FLAGS.eval_steps
            )
