...
def separate_input_fn(
    ...
    raw_dataset = tf.data.TFRecordDataset(
        filenames_dataset, num_parallel_reads=reader_num_threads
    )
    ...
    def consolidate_batch(elem):
        ...
        reshaped_elem = {
            key: tf.reshape(elem[key], [-1, elem[key].shape[-1]]) for key in elem
        }
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        ...
        parsed_dataset = parsed_dataset.map(
            consolidate_batch, num_parallel_calls=parser_num_threads
        )
