            ...
            if split == "train":
                ...
                if FLAGS.num_procs > 1:
                    ...
                    for shard, path in enumerate(self.train):
                        ...
                        for shuffle in range(num_shuffle):
                            print(
                                "Processing shard {} shuffle {}".format(shard, shuffle)
                            )
                            basename = "train-{:03d}-{:02d}".format(shard, shuffle)
                            np.random.shuffle(data_shard)
...
def load_record_info(record_info_dir, split, per_host_bsz, tgt_len, num_core_per_host):
...
def get_input_fn(
    record_info_dir, split, per_host_bsz, tgt_len, num_core_per_host, num_hosts=1
):
    """Creates input function."""
    record_info = load_record_info(
        record_info_dir, split, per_host_bsz, tgt_len, num_core_per_host
    )
...
if __name__ == "__main__":
    ...
    flags.DEFINE_integer("train_batch_size", 256, help="train batch size each host")
    flags.DEFINE_integer("valid_batch_size", 256, help="valid batch size each host")
    ...
    flags.DEFINE_integer("num_core_per_host", 8, help="number of GPUs per host")
    flags.DEFINE_bool(
        "debug",
        default=False,
    ...
    flags.DEFINE_integer("num_passes", 10, help="number of passes")
    flags.DEFINE_integer("num_shuffle", 4, help="number of shuffles for lm1b")
    flags.DEFINE_integer("batch_chunk", 1, help="number of accumulation steps")
