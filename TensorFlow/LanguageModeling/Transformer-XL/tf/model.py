...
def positionwise_FF(
    inp, d_model, d_inner, dropout, kernel_initializer, scope="ff", is_training=True
):
    output = inp
...
def rel_multihead_attn(
    ...
    with tf.variable_scope(scope):
        ...
        cat = (
            tf.concat([mems, w], 0) if mems is not None and mems.shape.ndims > 1 else w
        )
...
def transformer(
    ...
    with tf.variable_scope(scope):
        ...
        lookup_fn = (
            mul_adaptive_embedding_lookup if use_tpu else mask_adaptive_embedding_lookup
        )
