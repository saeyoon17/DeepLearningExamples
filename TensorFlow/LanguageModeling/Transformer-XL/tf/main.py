...
flags.DEFINE_string("model_dir", default="LM-TFM", help="Estimator model_dir.")
...
flags.DEFINE_bool(
    "jit_optimizer", default=True, help="Whether to enable XLA on optimizer"
)

# Optimization config
flags.DEFINE_float("learning_rate", default=0.01, help="Maximum learning rate.")
flags.DEFINE_float("clip", default=0.25, help="Gradient clipping value.")
# for cosine decay
flags.DEFINE_float("min_lr_ratio", default=0.1, help="Minimum ratio learning rate.")
flags.DEFINE_integer(
    "warmup_steps", default=1000, help="Number of steps for linear lr warmup."
)

# Training config
flags.DEFINE_integer("train_batch_size", default=256, help="Size of train batch.")
flags.DEFINE_integer("eval_batch_size", default=16, help="Size of valid batch.")
flags.DEFINE_integer(
    "train_steps", default=40000, help="Total number of training steps."
)
flags.DEFINE_integer(
    "log_interval", default=100, help="Number of iterations per repeat loop."
)
flags.DEFINE_integer(
    "save_steps", default=5000, help="number of steps for model checkpointing."
)
flags.DEFINE_integer("batch_chunk", default=1, help="Number of accumulation steps.")

# Evaluation config
flags.DEFINE_integer(
    "max_eval_batch", default=-1, help="Set -1 to turn off. Only used in test mode."
)
...
# Model config
flags.DEFINE_integer("tgt_len", default=192, help="Number of steps to predict")
flags.DEFINE_integer("mem_len", default=192, help="Number of steps to cache")
...
flags.DEFINE_integer("n_layer", default=16, help="Number of layers.")
flags.DEFINE_integer("d_model", default=512, help="Dimension of the model.")
flags.DEFINE_integer("d_embed", default=512, help="Dimension of the embeddings.")
flags.DEFINE_integer("n_head", default=8, help="Number of attention heads.")
flags.DEFINE_integer("d_head", default=64, help="Dimension of each attention head.")
flags.DEFINE_integer(
    "d_inner",
    default=2048,
...
def train(n_token, cutoffs, rank, local_rank, num_core_per_host):
    ...
    train_set = train_input_fn(
        {"batch_size": train_batch_size, "data_dir": FLAGS.data_dir}
    )
    ...
    with tf.variable_scope(tf.get_variable_scope()):
        mems = [
            tf.Variable(
                tf.zeros([FLAGS.mem_len, per_core_bsz, FLAGS.d_model], tf.float32),
                trainable=False,
            )
    ...
    if FLAGS.warmup_steps > 0:
        warmup_lr = (
            ...
            / tf.to_float(FLAGS.warmup_steps)
            * FLAGS.learning_rate
        )
    else:
        warmup_lr = 0.0
    ...
    acc_train_op = optimizer.apply_gradients(
        list(zip(acc_clipped, all_vars)), global_step
    )
    ...
    acc_final_op = tf.group(
        acc_train_op, assign_mems, in_progress.assign(tf.zeros_like(in_progress))
    )
    ##### Training loop
    ...
    with tf.Session(
        ...
        fetches = [
            ...
            target_tokens,
            learning_rate,
            final_op if FLAGS.batch_chunk == 1 else acc_final_op,
        ]
        total_loss, prev_step, target_tokens = 0.0, -1, 0
        ...
        while True:
            ...
            if curr_step > 0 and curr_step % FLAGS.log_interval == 0:
                ...
                throughput = (
                    target_tokens * num_core_per_host / (time.time() - start_time)
                )
                ...
                total_loss, prev_step, target_tokens = 0.0, curr_step, 0
                start_time = time.time()
        ...
        if rank == 0:
            tf.logging.info(
                "Training throughput: {:>6.0f} tok/s".format(
                    meters["train_throughput"].avg
                )
            )
            summary = {
...
def evaluate(n_token, cutoffs):
    ...
    with tf.variable_scope(tf.get_variable_scope()):
        mems = [
            tf.placeholder(tf.float32, [FLAGS.mem_len, bsz, FLAGS.d_model])
            for _ in range(FLAGS.n_layer)
        ]
    ...
    ##### Evaluation loop
    mems_np = [
        np.zeros([FLAGS.mem_len, bsz, FLAGS.d_model], dtype=np.float32)
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        tf.logging.info(
            "Evaluating with: bs {}, math {} ".format(
                FLAGS.eval_batch_size, "amp" if FLAGS.amp else "fp32"
            )
        )
        tf.logging.info(
            "| loss {:.2f} | pplx {:>7.2f}, bpc {:>7.4f}, tok/s {:>6.1f}, ms/batch {:>4.2f}".format(
                avg_loss,
                math.exp(avg_loss),
                avg_loss / math.log(2),
                meters["eval_throughput"].avg,
                meters["eval_latency"].avg,
            )
        )
        summary = {
            "eval_loss": avg_loss,
        ...
        for p in FLAGS.percentiles:
            p = int(p)
            tf.logging.info(
                "Latency {}%: {:>4.2f} ms".format(p, np.percentile(latency_data, p))
            )
