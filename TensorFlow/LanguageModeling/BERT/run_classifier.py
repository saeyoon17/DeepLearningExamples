...
flags.DEFINE_integer("display_loss_steps", 10, "How often to print loss from estimator")
...
def get_frozen_tftrt_model(
    bert_config, shape, num_labels, use_one_hot_embeddings, init_checkpoint
):
    tf_config = tf.compat.v1.ConfigProto()
    tf_config.gpu_options.allow_growth = True
    output_node_names = [
        "loss/cls_loss",
        "loss/cls_per_example_loss",
        "loss/cls_logits",
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        def metric_fn(per_example_loss, label_ids, logits):
            ...
            if task_name == "cola":
                ...
                MCC_op = tf.group(
                    FN_op, TN_op, TP_op, FP_op, tf.identity(MCC, name="MCC")
                )
...
def input_fn_builder(
    features, batch_size, seq_length, is_training, drop_remainder, hvd=None
):
    """Creates an `input_fn` closure to be passed to Estimator."""
...
def main(_):
    ...
    if FLAGS.horovod:
        ...
        global_batch_size = (
            FLAGS.train_batch_size * FLAGS.num_accumulation_steps * hvd.size()
        )
    ...
    if FLAGS.do_train:
        ...
        file_based_convert_examples_to_features(
            train_examples[start_index:end_index],
            label_list,
            FLAGS.max_seq_length,
            tokenizer,
            tmp_filenames[hvd_rank],
        )
        ...
        ss_sentences_per_second = (
            (training_hooks[-1].count - training_hooks[-1].skipped)
            * global_batch_size
            * 1.0
    ...
    if FLAGS.do_eval and master_process:
        ...
        with tf.io.gfile.GFile(output_eval_file, "w") as writer:
            ...
            for key in sorted(result.keys()):
                dllogging.logger.log(
                    step=(), data={key: float(result[key])}, verbosity=Verbosity.DEFAULT
                )
