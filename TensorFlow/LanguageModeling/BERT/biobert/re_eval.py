...
parser.add_argument(
    ...
    type=str,
    default="binary",
    help="default:binary, possible other options:{chemprot}",
)
...
if args.task == "binary":
    ...
    p, r, f, s = sklearn.metrics.precision_recall_fscore_support(
        y_pred=pred_class, y_true=testdf["label"]
    )
    results = dict()
...
if args.task == "chemprot":
    ...
    p, r, f, s = sklearn.metrics.precision_recall_fscore_support(
        y_pred=pred_class, y_true=test_answer, labels=[0, 1, 2, 3, 4], average="micro"
    )
    results = dict()
    results["f1 score"] = f
    results["recall"] = r
