...
def create_optimizer(
    ...
    if hvd is not None and (
        num_accumulation_steps == 1 or (not allreduce_post_accumulation)
    ):
        optimizer = hvd.DistributedOptimizer(
            optimizer,
            sparse_as_dense=True,
            compression=Compression.fp16
            if use_fp16 or manual_fp16
    ...
    if num_accumulation_steps > 1:
        ...
        reset_step = tf.cast(
            tf.math.equal(local_step % num_accumulation_steps, 0), dtype=tf.bool
        )
        local_step = tf.cond(
            reset_step,
        ...
        grads_and_vars_and_accums = [
            (gv[0], gv[1], accum_vars[i])
        ...
        all_are_finite = (
            tf.reduce_all([tf.reduce_all(tf.is_finite(g)) for g in grads])
        ...
        (clipped_grads, _) = tf.clip_by_global_norm(
            ...
            clip_norm=1.0,
            use_norm=tf.cond(
                all_are_finite, lambda: tf.global_norm(grads), lambda: tf.constant(1.0)
            ),
        )
        ...
        accum_vars = tf.cond(
            reset_step,
            lambda: [
                accum_vars[i].assign(grad) for i, grad in enumerate(clipped_grads)
            ],
            lambda: [
                accum_vars[i].assign_add(grad) for i, grad in enumerate(clipped_grads)
            ],
        )
        ...
        def update(accum_vars):
            ...
            return optimizer.apply_gradients(
                list(zip(accum_vars, tvars)), global_step=global_step
            )
        ...
        new_global_step = tf.cond(
            tf.math.logical_and(
                update_step,
                tf.cast(hvd.allreduce(tf.cast(batch_finite, tf.int32)), tf.bool)
                if hvd is not None
                else batch_finite,
            ),
        ...
        new_global_step = tf.cond(
            all_are_finite, lambda: global_step + 1, lambda: global_step
        )
        new_global_step = tf.identity(new_global_step, name="step_update")
...
class AdamWeightDecayOptimizer(tf.compat.v1.train.Optimizer):
    ...
    def apply_gradients(
        self, grads_and_vars, global_step=None, name=None, manual_fp16=False
    ):
        """See base class."""
        assignments = []
        for (grad, param) in grads_and_vars:
            ...
            assignments.extend(
                [param_fp32.assign(next_param), m.assign(next_m), v.assign(next_v)]
            )
...
class LAMBOptimizer(tf.compat.v1.train.Optimizer):
    ...
    def apply_gradients(
        self, grads_and_vars, global_step, name=None, manual_fp16=False
    ):
        """See base class."""
        assignments = []
