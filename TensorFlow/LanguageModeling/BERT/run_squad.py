...
def extract_run_squad_flags():
    ...
    flags.DEFINE_string(
        "dllog_path", "/results/bert_dllog.json", "filename where dllogger writes to"
    )
    ...
    flags.DEFINE_integer(
        "display_loss_steps", 10, "How often to print loss from estimator"
    )
    ...
    flags.DEFINE_bool(
        "export_triton",
        False,
        "Whether to export saved model or run inference with Triton",
    )
    flags.DEFINE_string(
    ...
    flags.DEFINE_string(
        "triton_server_url",
        "localhost:8001",
        "exports to appropriate directory for Triton",
    )
    flags.DEFINE_bool(
        "triton_model_overwrite",
        False,
        "If True, will overwrite an existing directory with the specified 'model_name' and 'version_name'",
    )
    flags.DEFINE_integer(
...
def model_fn_builder(
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if not is_training and FLAGS.use_trt:
            trt_graph = get_frozen_tftrt_model(
                bert_config, input_ids.shape, use_one_hot_embeddings, init_checkpoint
            )
        ...
        if FLAGS.verbose_logging:
            ...
            for var in tvars:
                ...
                tf.compat.v1.logging.info(
                    " %d name = %s, shape = %s%s",
                    0 if hvd is None else hvd.rank(),
                    var.name,
                    var.shape,
                    init_string,
                )
...
def validate_flags_or_throw(bert_config):
    ...
    if not FLAGS.do_train and not FLAGS.do_predict and not FLAGS.export_triton:
        raise ValueError(
            "At least one of `do_train` or `do_predict` or `export_SavedModel` must be True."
        )
...
def export_model(estimator, export_dir, init_checkpoint):
    ...
    if not os.path.exists(final_model_folder):
        ...
        if FLAGS.triton_model_overwrite:
            ...
            os.rename(saved_dir, final_model_folder)
            print(
                "WARNING: Existing model was overwritten. Model dir: {}".format(
                    final_model_folder
                )
            )
        else:
...
def main(_):
    ...
    if FLAGS.horovod:
        ...
        tf.compat.v1.logging.info(
            "hvd.size() = %d hvd.rank() = %d", hvd.size(), hvd.rank()
        )
        global_batch_size = (
            FLAGS.train_batch_size * hvd.size() * FLAGS.num_accumulation_steps
        )
    ...
    training_hooks.append(
        LogTrainRunHook(global_batch_size, hvd_rank, FLAGS.save_checkpoints_steps)
    )
    ...
    if FLAGS.do_train:
        ...
        # in memory.
        train_writer = FeatureWriter(filename=tmp_filenames[hvd_rank], is_training=True)
        ...
        tf.compat.v1.logging.info(
            "  Num split examples = %d", train_writer.num_features
        )
        ...
        estimator.train(
            input_fn=train_input_fn, hooks=training_hooks, max_steps=num_train_steps
        )
        train_time_elapsed = time.time() - train_start_time
        train_time_wo_overhead = training_hooks[-1].total_time
        avg_sentences_per_second = (
            num_train_steps * global_batch_size * 1.0 / train_time_elapsed
        )
    ...
    if FLAGS.do_predict and master_process:
        ...
        if FLAGS.num_eval_iterations is not None:
            eval_examples = eval_examples[
                : FLAGS.num_eval_iterations * FLAGS.predict_batch_size
            ]
        ...
        if FLAGS.eval_script:
            ...
            dllogging.logger.log(
                step=(), data={"exact_match": exact_match}, verbosity=Verbosity.DEFAULT
            )
            print(str(eval_out))
