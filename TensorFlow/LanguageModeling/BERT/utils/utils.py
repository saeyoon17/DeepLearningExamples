...
class LogTrainRunHook(tf.estimator.SessionRunHook):
    ...
    def before_run(self, run_context):
        self.t0 = time.time()
        return tf.estimator.SessionRunArgs(fetches=["step_update:0"])
    ...
    def after_run(self, run_context, run_values):
        ...
        if (self.global_step - self.init_global_step) <= self.num_steps_ignore_xla or (
