...
def read_squad_examples(
    input_file, is_training, version_2_with_negative=False, input_data=None
):
...
def main():
    ...
    train_writer = FeatureWriter(
        filename=os.path.join(
            FLAGS.squad_dir, "final_tfrecords_sharded/train.tf_record"
        ),
        is_training=True,
    )
    ...
    eval_writer = FeatureWriter(
        filename=os.path.join(
            FLAGS.squad_dir, "final_tfrecords_sharded/eval.tf_record"
        ),
        is_training=False,
    )
    eval_features = []
