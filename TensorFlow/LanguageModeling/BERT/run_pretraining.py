...
flags.DEFINE_string(
    "optimizer_type", "lamb", "Optimizer used for training - LAMB or ADAM"
)
...
flags.DEFINE_bool(
    "verbose_logging", False, "If true, all of the trainable parameters are printed"
)
...
flags.DEFINE_integer(
    "init_loss_scale",
    2**32,
    "Initial value of loss scale if mixed precision training",
)
...
class _LogSessionRunHook(tf.estimator.SessionRunHook):
    ...
    def after_create_session(self, session, coord):
        ...
        self.all_count = (
            0  # number of steps (including accumulation) between every print
        )
        self.loss = 0.0  # accumulation of loss in each step between every print

        self.total_time = (
            0.0  # total time taken to train (excluding warmup + ckpt saving steps)
        )
        self.step_time = 0.0  # time taken per step
        self.init_global_step = session.run(
            tf.train.get_global_step()
        )  # training starts at init_global_step
        self.skipped = 0
        self.final_loss = 0
        ...
        if update_step:
            ...
            if print_step == 1 or print_step % self.display_every == 0:
                ...
                if self.hvd_rank >= 0 and FLAGS.report_loss:
                    if FLAGS.manual_fp16 or FLAGS.amp:
                        self.dllogging.logger.log(
                            step=(print_step),
                            data={
                                "Rank": int(self.hvd_rank),
                                "throughput_train": float(sent_per_sec),
                                "mlm_loss": float(mlm_loss),
                                "nsp_loss": float(nsp_loss),
                                "total_loss": float(total_loss),
                                "avg_loss_step": float(avg_loss_step),
                                "learning_rate": str(lr),
                                "loss_scaler": int(loss_scaler),
                            },
...
def get_masked_lm_output(
    ...
    with tf.variable_scope("cls/predictions"):
        ...
        logits = tf.matmul(
            tf.cast(input_tensor, tf.float32), output_weights, transpose_b=True
        )
...
def main(_):
    ...
    model_fn = model_fn_builder(
        ...
        else FLAGS.learning_rate * hvd.size(),
    ...
    if FLAGS.do_train:
        ...
        if not FLAGS.horovod or hvd.rank() == 0:
            global_batch_size = (
                FLAGS.train_batch_size * FLAGS.num_accumulation_steps
                if not FLAGS.horovod
                else FLAGS.train_batch_size * FLAGS.num_accumulation_steps * hvd.size()
            )
            log_hook = _LogSessionRunHook(
                global_batch_size,
                FLAGS.num_accumulation_steps,
                dllogging,
        ...
        if not FLAGS.horovod or hvd.rank() == 0:
            ...
            avg_sentences_per_second = (
                FLAGS.num_train_steps * global_batch_size * 1.0 / train_time_elapsed
            )
            ...
            if log_hook.final_loss != 0:
                dllogging.logger.log(
                    step=(),
                    data={"total_loss": log_hook.final_loss},
                    verbosity=Verbosity.DEFAULT,
                )
            tf.compat.v1.logging.info("-----------------------------")
