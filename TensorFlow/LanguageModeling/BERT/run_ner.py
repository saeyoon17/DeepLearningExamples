...
def main(_):
    ...
    if FLAGS.do_predict and master_process:
        ...
        with tf.io.gfile.GFile(
            os.path.join(FLAGS.output_dir, "label2id.pkl"), "rb"
        ) as rf:
        ...
        with tf.io.gfile.GFile(output_predict_file, "w") as writer, tf.io.gfile.GFile(
            test_labels_file, "w"
        ) as tl, tf.io.gfile.GFile(test_labels_err_file, "w") as tle:
            print(id2label)
            i = 0
            for prediction in estimator.predict(
                input_fn=predict_input_fn, hooks=eval_hooks, yield_single_examples=True
            ):
                output_line = (
