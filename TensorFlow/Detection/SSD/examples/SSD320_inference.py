...
flags.DEFINE_integer(
    "warmup_iters", 100, "Number of iterations skipped during benchmark"
)
flags.DEFINE_integer(
    "benchmark_iters", 300, "Number of iterations measured by benchmark"
)
...
def build_benchmark_input_fn(input_fn):
    def benchmark_input_fn(params={}):
        ...
        return (
            input_fn(params).repeat().take(FLAGS.warmup_iters + FLAGS.benchmark_iters)
        )
...
class TimingHook(tf.train.SessionRunHook):
    ...
    def log_progress(self):
        if sys.stdout.isatty():
            print(
                len(self.times) - FLAGS.warmup_iters,
                "/",
                FLAGS.benchmark_iters,
                " " * 10,
                end="\r",
            )
    ...
    def end(self, *args, **kwargs):
        ...
        throughput = (
            sum([1 / x for x in self.times[FLAGS.warmup_iters :]])
            * FLAGS.batch_size
            / FLAGS.benchmark_iters
        )
        latency_avg = (
            1000 * sum(self.times[FLAGS.warmup_iters :]) / FLAGS.benchmark_iters
        )
