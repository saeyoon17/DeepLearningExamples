...
def nasnet_large_arg_scope(
    weight_decay=5e-5, batch_norm_decay=0.9997, batch_norm_epsilon=1e-3
):
...
def _imagenet_stem(inputs, hparams, stem_cell, current_step=None):
    ...
    net = slim.conv2d(
        inputs, num_stem_filters, [3, 3], stride=2, scope="conv0", padding="VALID"
    )
...
def build_nasnet_cifar(
    images, num_classes, is_training=True, config=None, current_step=None
):
