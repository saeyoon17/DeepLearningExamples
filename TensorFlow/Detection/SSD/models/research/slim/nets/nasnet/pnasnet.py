...
def pnasnet_large_arg_scope(
    weight_decay=4e-5, batch_norm_decay=0.9997, batch_norm_epsilon=0.001
):
...
def pnasnet_mobile_arg_scope(
    weight_decay=4e-5, batch_norm_decay=0.9997, batch_norm_epsilon=0.001
):
    """Default arg scope for the PNASNet Mobile ImageNet model."""
    return nasnet.nasnet_mobile_arg_scope(
        weight_decay, batch_norm_decay, batch_norm_epsilon
    )


def _build_pnasnet_base(
    images, normal_cell, num_classes, hparams, is_training, final_endpoint=None
):
    ...
    for cell_num in range(hparams.num_cells):
        ...
        if (
            hparams.use_aux_head
            and cell_num in aux_head_cell_idxes
            and num_classes
            and is_training
        ):
            aux_net = activation_fn(net)
            # pylint: disable=protected-access
            nasnet._build_aux_head(
                aux_net,
                end_points,
                num_classes,
                hparams,
...
def build_pnasnet_large(
    images, num_classes, is_training=True, final_endpoint=None, config=None
):
    ...
    with arg_scope(
        [slim.dropout, nasnet_utils.drop_path, slim.batch_norm], is_training=is_training
    ):
        with arg_scope(
            [
                slim.avg_pool2d,
