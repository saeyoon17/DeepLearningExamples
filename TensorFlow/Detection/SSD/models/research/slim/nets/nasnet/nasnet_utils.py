...
def _operation_to_filter_shape(operation):
    ...
    assert filter_shape == int(
        splitted_operation[1][0]
    ), "Rectangular filters not supported."
    return filter_shape
...
def _stacked_separable_conv(
    net, stride, operation, filter_size, use_bounded_activation
):
...
class NasNetABaseCell(object):
    ...
    def _apply_conv_operation(
        self, net, operation, stride, is_from_original_input, current_step
    ):
        ...
        if "separable" in operation:
            net = _stacked_separable_conv(
                net, stride, operation, filter_size, self._use_bounded_activation
            )
            if self._use_bounded_activation:
    ...
    def _apply_drop_path(
        self, net, current_step=None, use_summaries=False, drop_connect_version="v3"
    ):
