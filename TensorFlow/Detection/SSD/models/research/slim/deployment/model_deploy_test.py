...
class DeploymentConfigTest(tf.test.TestCase):
    ...
    def testPS(self):
        ...
        self.assertEqual(deploy_config.clone_scope(0), "")
        self.assertDeviceEqual(
            deploy_config.optimizer_device(), "/job:worker/device:CPU:0"
        )
        self.assertDeviceEqual(
            deploy_config.inputs_device(), "/job:worker/device:CPU:0"
        )
...
class CreatecloneTest(tf.test.TestCase):
    ...
    def testCreateLogisticClassifier(self):
        ...
        with g.as_default():
            ...
            self.assertEqual(
                clone.outputs.op.name, "LogisticClassifier/fully_connected/Sigmoid"
            )
    ...
    def testCreateSingleclone(self):
        ...
        with g.as_default():
            ...
            for v in slim.get_variables():
                ...
                self.assertDeviceEqual(v.value().device, "CPU:0")
            self.assertEqual(
                clone.outputs.op.name, "BatchNormClassifier/fully_connected/Sigmoid"
            )
            self.assertEqual(clone.scope, "")
            self.assertDeviceEqual(clone.device, "GPU:0")
            self.assertEqual(len(slim.losses.get_losses()), 1)
...
class OptimizeclonesTest(tf.test.TestCase):
    ...
    def testCreateMulticloneCPU(self):
        ...
        with g.as_default():
            ...
            num_clones = 4
            deploy_config = model_deploy.DeploymentConfig(
                num_clones=num_clones, clone_on_cpu=True
            )
...
class DeployTest(tf.test.TestCase):
    ...
    def testLocalTrainOp(self):
        ...
        with g.as_default():
            ...
            model = model_deploy.deploy(
                deploy_config, model_fn, model_args, optimizer=optimizer
            )
    ...
    def testNoSummariesOnGPU(self):
        with tf.Graph().as_default():
            ...
            model = model_deploy.deploy(
                deploy_config, ModelFn, optimizer=tf.train.GradientDescentOptimizer(1.0)
            )
