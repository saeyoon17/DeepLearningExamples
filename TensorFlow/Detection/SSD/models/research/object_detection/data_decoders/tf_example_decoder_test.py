...
class TfExampleDecoderTest(tf.test.TestCase):
    ...
    def testDecodeAdditionalChannels(self):
        ...
        additional_channel_tensor = np.random.randint(256, size=(4, 5, 1)).astype(
            np.uint8
        )
        ...
        example = tf.train.Example(
            features=tf.train.Features(
                feature={
                    "image/encoded": dataset_util.bytes_feature(encoded_jpeg),
                    "image/format": dataset_util.bytes_feature("jpeg"),
                    "image/object/area": dataset_util.float_list_feature(object_area),
                }
            )
        ).SerializeToString()
        ...
        example = tf.train.Example(
            features=tf.train.Features(
                feature={
                    "image/encoded": dataset_util.bytes_feature(encoded_jpeg),
                    "image/format": dataset_util.bytes_feature("jpeg"),
                    "image/class/text": dataset_util.bytes_list_feature(["dog", "cat"]),
                }
            )
        ).SerializeToString()
        label_map_string = """
