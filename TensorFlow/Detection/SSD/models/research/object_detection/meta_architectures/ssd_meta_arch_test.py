...
class SsdMetaArchTest(
        ...
        for input_shape in input_shapes:
            ...
            with self.test_session(graph=tf_graph) as sess:
                ...
                prediction_out = sess.run(
                    prediction_dict,
                    feed_dict={
                        preprocessed_input_placeholder: np.random.uniform(
            ...
            self.assertAllEqual(
                prediction_out["box_encodings"].shape, expected_box_encodings_shape_out
            )
            self.assertAllEqual(
    ...
    def test_predict_result_shapes_on_image_with_static_shape(self, use_keras):
        ...
        (box_encodings, class_predictions, _, _) = self.execute(graph_fn, [input_image])
        ...
        for input_shape in input_shapes:
            ...
            with self.test_session(graph=tf_graph) as sess:
                ...
                detections_out = sess.run(
                    detections,
                    feed_dict={
                        input_placeholder: np.random.uniform(size=(batch_size, 2, 2, 3))
                    },
                )
            ...
            self.assertAllClose(
                detections_out["num_detections"], expected_num_detections
            )
    ...
    def test_loss_results_are_correct_without_add_background_class(self, use_keras):
        ...
        def graph_fn(
            ...
            groundtruth_classes_list = [groundtruth_classes1, groundtruth_classes2]
            model, _, _, _ = self._create_model(
                apply_hard_mining=False, add_background_class=False, use_keras=use_keras
            )
            model.provide_groundtruth(groundtruth_boxes_list, groundtruth_classes_list)
            prediction_dict = model.predict(preprocessed_tensor, true_image_shapes=None)
            loss_dict = model.loss(prediction_dict, true_image_shapes=None)
