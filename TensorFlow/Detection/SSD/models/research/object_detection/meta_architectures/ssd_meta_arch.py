...
from object_detection.utils import ops, shape_utils, visualization_utils
...
class SSDMetaArch(model.DetectionModel):
    ...
    def predict(self, preprocessed_inputs, true_image_shapes):
        ...
        batchnorm_updates_collections = (
            None if self._inplace_batchnorm_update else tf.GraphKeys.UPDATE_OPS
        )
        if self._feature_extractor.is_keras_model:
    ...
    def _summarize_target_assignment(self, groundtruth_boxes_list, match_list):
        ...
        tf.summary.scalar(
            "AvgNumPositiveAnchorsPerImage",
            tf.reduce_mean(tf.to_float(pos_anchors_per_image)),
            family="TargetAssignment",
        )
        tf.summary.scalar(
            "AvgNumNegativeAnchorsPerImage",
