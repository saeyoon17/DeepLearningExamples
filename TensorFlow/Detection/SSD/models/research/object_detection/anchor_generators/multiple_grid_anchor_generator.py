...
from object_detection.core import anchor_generator, box_list_ops
...
class MultipleGridAnchorGenerator(anchor_generator.AnchorGenerator):
    ...
    def _generate(self, feature_map_shape_list, im_height=1, im_width=1):
        ...
        if not self._anchor_strides:
            anchor_strides = [
                (1.0 / tf.to_float(pair[0]), 1.0 / tf.to_float(pair[1]))
                for pair in feature_map_shape_list
            ]
        else:
            anchor_strides = [
                (tf.to_float(stride[0]) / im_height, tf.to_float(stride[1]) / im_width)
        ...
        if not self._anchor_offsets:
            ...
            anchor_offsets = [
                (tf.to_float(offset[0]) / im_height, tf.to_float(offset[1]) / im_width)
