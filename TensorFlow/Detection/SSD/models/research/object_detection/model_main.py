...
def main(unused_argv):
    ...
    if FLAGS.checkpoint_dir:
        ...
        train_hooks = [
            ...
            DLLoggerHook(
                hvd.size() * train_and_eval_dict["train_batch_size"], hvd.rank()
            ),
        ]
        eval_hooks = []
        ...
        for x in range(FLAGS.eval_count):
            estimator.train(
                train_input_fn, hooks=train_hooks, steps=train_steps // FLAGS.eval_count
            )

            if hvd.rank() == 0:
                eval_input_fn = eval_input_fns[0]
                results = estimator.evaluate(
                    eval_input_fn, steps=None, hooks=eval_hooks
                )
