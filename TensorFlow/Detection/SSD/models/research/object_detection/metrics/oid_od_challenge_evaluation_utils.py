...
def build_groundtruth_boxes_dictionary(data, class_label_map):
    ...
    return {
        standard_fields.InputDataFields.groundtruth_boxes: data_boxes[
            ["YMin", "XMin", "YMax", "XMax"]
        ].as_matrix(),
        standard_fields.InputDataFields.groundtruth_classes: data_boxes["LabelName"]
        .map(lambda x: class_label_map[x])
        .as_matrix(),
        standard_fields.InputDataFields.groundtruth_group_of: data_boxes["IsGroupOf"]
        .as_matrix()
        .astype(int),
...
def build_predictions_dictionary(data, class_label_map):
    ...
    """
    return {
        standard_fields.DetectionResultFields.detection_boxes: data[
            ["YMin", "XMin", "YMax", "XMax"]
        ].as_matrix(),
        standard_fields.DetectionResultFields.detection_classes: data["LabelName"]
        .map(lambda x: class_label_map[x])
        .as_matrix(),
        standard_fields.DetectionResultFields.detection_scores: data[
            "Score"
        ].as_matrix(),
    }
