...
from object_detection.utils import json_utils, object_detection_evaluation
...
def _check_mask_type_and_value(array_name, masks):
    ...
    if masks.dtype != np.uint8:
        raise ValueError(
            "{} must be of type np.uint8. Found {}.".format(array_name, masks.dtype)
        )
...
class CocoMaskEvaluator(object_detection_evaluation.DetectionEvaluator):
    ...
    def add_single_detected_image_info(self, image_id, detections_dict):
        ...
        _check_mask_type_and_value(
            standard_fields.DetectionResultFields.detection_masks, detection_masks
        )
    ...
    def evaluate(self):
        ...
        groundtruth_dict = {
            ...
            "images": [
                ...
                for image_id, shape in self._image_id_to_mask_shape_map.iteritems()
            ],
            "categories": self._categories,
        }
        ...
        mask_metrics = {
            "DetectionMasks_" + key: value for key, value in mask_metrics.iteritems()
        }
        return mask_metrics
