...
class COCOEvalWrapper(cocoeval.COCOeval):
    ...
    def __init__(
        self, groundtruth=None, detections=None, agnostic_mode=False, iou_type="bbox"
    ):
    ...
    def ComputeMetrics(
        self, include_metrics_per_category=False, all_metrics_per_category=False
    ):
        ...
        for category_index, category_id in enumerate(self.GetCategoryIdList()):
            ...
            if all_metrics_per_category:
                per_category_ap[
                    "Precision mAP ByCategory/{}".format(category)
                ] = self.category_stats[0][category_index]
                per_category_ap[
                ...
                per_category_ap[
                    "Recall AR@1 ByCategory/{}".format(category)
                ] = self.category_stats[6][category_index]
                per_category_ap[
                    "Recall AR@10 ByCategory/{}".format(category)
                ] = self.category_stats[7][category_index]
                per_category_ap[
                    "Recall AR@100 ByCategory/{}".format(category)
...
def ExportSingleImageDetectionBoxesToCoco(
    image_id, category_id_set, detection_boxes, detection_scores, detection_classes
):
