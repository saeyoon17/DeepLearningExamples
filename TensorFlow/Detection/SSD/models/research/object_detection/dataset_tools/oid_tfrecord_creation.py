...
def tf_example_from_annotations_data_frame(
    ...
    feature_map = {
        ...
        standard_fields.TfExampleFields.object_class_label: dataset_util.int64_list_feature(
            filtered_data_frame_boxes.LabelName.map(lambda x: label_map[x]).as_matrix()
        ),
        standard_fields.TfExampleFields.filename: dataset_util.bytes_feature(
            "{}.jpg".format(image_id)
        ),
        standard_fields.TfExampleFields.source_id: dataset_util.bytes_feature(image_id),
    ...
    if "ConfidenceImageLabel" in filtered_data_frame_labels.columns:
        feature_map[
            ...
            filtered_data_frame_labels.LabelName.map(lambda x: label_map[x]).as_matrix()
        )
