...
def merge_boxes_with_multiple_labels(
    ...
    with tf.control_dependencies(
        ...
        def map_box_encodings(i):
            ...
            box_mask = tf.equal(unique_indices, i * tf.ones(num_boxes, dtype=tf.int32))
...
def matmul_crop_and_resize(image, boxes, crop_size, scope=None):
    ...
    if len(img_shape) != 4:
        raise ValueError(
            "image should have shape " "`[batch, image_height, image_width, depth]`"
        )
    ...
    def _lin_space_weights(num, img_size):
        if num > 1:
            ...
            start_weights = tf.constant(num * [0.5 * (img_size - 1)], dtype=tf.float32)
            stop_weights = tf.constant(num * [0.5 * (img_size - 1)], dtype=tf.float32)
    ...
    with tf.name_scope(scope, "MatMulCropAndResize"):
        ...
        image_idx_h = tf.constant(
            np.reshape(np.arange(img_height), (1, 1, 1, img_height)), dtype=boxes.dtype
        )
        image_idx_w = tf.constant(
            np.reshape(np.arange(img_width), (1, 1, 1, img_width)), dtype=boxes.dtype
        )
