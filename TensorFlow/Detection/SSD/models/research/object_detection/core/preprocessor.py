...
def random_rotation90(
    image, boxes=None, masks=None, keypoints=None, seed=None, preprocess_vars_cache=None
):
...
def _strict_random_crop_image(
    ...
    with tf.name_scope("RandomCropImage", values=[image, boxes]):
        ...
        if masks is not None:
            ...
            new_masks = tf.slice(
                masks_of_boxes_completely_inside_window, masks_box_begin, masks_box_size
            )
...
def random_crop_pad_image(
    ...
    min_image_size = tf.to_int32(
        tf.to_float(tf.stack([image_height, image_width])) * min_padded_size_ratio
    )
    max_image_size = tf.to_int32(
        tf.to_float(tf.stack([image_height, image_width])) * max_padded_size_ratio
    )
    ...
    with tf.name_scope("RandomPadToAspectRatio", values=[image]):
        ...
        if masks is not None:
            ...
            new_masks = tf.image.pad_to_bounding_box(
                new_masks, 0, 0, tf.to_int32(target_height), tf.to_int32(target_width)
            )
...
def preprocess(
    ...
    for option in preprocess_options:
        ...
        if (
            preprocess_vars_cache is not None
            and "preprocess_vars_cache" in inspect.getargspec(func).args
        ):
