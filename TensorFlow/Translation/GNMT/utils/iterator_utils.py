...
def get_effective_epoch_size(hparams, train=True):
    ...
    for srcline, tgtline in zip(srcf, tgtf):
        ...
        if len_srcline < src_max_len and len_tgtline < tgt_max_len:
            epoch_size += 1
...
def get_iterator(
    ...
    if filter_oversized_sequences:
        src_tgt_dataset = src_tgt_dataset.filter(
            lambda src, tgt: tf.logical_and(
                tf.size(src) < src_max_len, tf.size(tgt) < tgt_max_len
            )
        )
    ...
    def batching_func(x):
        return x.padded_batch(
            ...
            padded_shapes=(
                tf.TensorShape(
                    [src_max_len if use_static_input_shape else None]
                ),  # src
                tf.TensorShape(
...
def get_infer_iterator(
    ...
    batched_dataset = batched_dataset.map(
        lambda src_ids, src_seq_len: (
            {"source": src_ids, "source_sequence_length": src_seq_len}
        )
    )
    return batched_dataset
