...
def sparse_softmax_crossent_with_logits(logits=None, labels=None, name=None):
    ...
    with tf.name_scope(name, "SparseSoftmaxCrossEntropyWithLogits", [labels, logits]):
        labels = tf.convert_to_tensor(labels)
        logits = tf.convert_to_tensor(logits)
        precise_logits = (
            tf.cast(logits, tf.float32)
            if (tf.as_dtype(logits.dtype) == tf.float16)
            else logits
        )
        ...
        if not static_shapes_fully_defined:
            ...
            if not (xla_compile or use_xla):
                ...
                shape_checks.append(
                    tf.assert_equal(tf.shape(labels), tf.shape(logits)[:-1])
                )
...
def clip_by_global_norm(t_list, clip_norm, use_norm=None, name=None):
    ...
    with tf.name_scope(name, "clip_by_global_norm", t_list + [clip_norm]) as name:
        ...
        scale = clip_norm * tf.minimum(
            1.0 / use_norm, tf.constant(1.0, dtype=use_norm.dtype) / clip_norm
        )
