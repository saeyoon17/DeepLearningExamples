...
from utils import evaluation_utils, iterator_utils
...
def add_arguments(parser):
    ...
    # network
    parser.add_argument("--num_units", type=int, default=1024, help="Network size.")
    ...
    parser.add_argument("--target_bleu", type=float, default=None, help="Target bleu.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help=("If nonzero, smooth the labels towards " "1/num_classes."),
    )
    ...
    parser.add_argument(
        "--output_dir", type=str, default="results", help="Store log/model files."
    )
    ...
    parser.add_argument(
        "--forget_bias", type=float, default=0.0, help="Forget bias for BasicLSTMCell."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            """Detokenizer script file. Default: DATA_DIR/mosesdecoder/scripts/tokenizer/detokenizer.perl"""
        ),
    )
    parser.add_argument(
        "--tokenizer_file",
        ...
        default=None,
        help=(
            """Tokenizer script file. Default: DATA_DIR/mosesdecoder/scripts/tokenizer/tokenizer.perl"""
        ),
    )
    ...
    parser.add_argument(
        "--amp", action="store_true", help="use amp for training and inference"
    )
    parser.add_argument(
        "--use_fastmath",
        type="bool",
    ...
    parser.add_argument(
        "--use_autojit_xla", type="bool", default=False, help="Use auto jit xla."
    )
    # GPU knobs
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_block_lstm", type="bool", default=False, help="whether to use block lstm"
    )
    parser.add_argument(
        "--use_defun", type="bool", default=False, help="whether to use Defun"
    )
    ...
    parser.add_argument(
        "--debug", type="bool", default=False, help="Debug train and eval"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--clip_grads", type="bool", default=True, help="whether to clip gradients"
    )
    parser.add_argument(
        "--profile", type="bool", default=False, help="If generate profile"
    )
    parser.add_argument(
        "--profile_save_steps",
        type=int,
...
def create_hparams(flags):
    ...
    return tf.contrib.training.HParams(
        ...
        if flags.detokenizer_file is not None
        else os.path.join(
            flags.data_dir, "mosesdecoder/scripts/tokenizer/detokenizer.perl"
        ),
        tokenizer_file=flags.tokenizer_file
        if flags.tokenizer_file is not None
        else os.path.join(
            flags.data_dir, "mosesdecoder/scripts/tokenizer/tokenizer.perl"
        ),
        # Advanced inference arguments
        ...
        # Other knobs
        local_parameter_device=(
            "cpu" if flags.num_gpus == 0 else flags.local_parameter_device
        ),
...
def tokenize(hparams, file, tokenized_file):
    ...
    with open(file, "rb") as input_file:
        with open(tokenized_file, "wb") as output_file:
            subprocess.run(
                [hparams.tokenizer_file, "-l", hparams.src],
                stdin=input_file,
                stdout=output_file,
            )
...
def detokenize(hparams, file, detokenized_file):
    ...
    with open(file, "rb") as input_file:
        with open(detokenized_file, "wb") as output_file:
            subprocess.run(
                [hparams.detokenizer_file, "-l", hparams.tgt],
                stdin=input_file,
                stdout=output_file,
            )
...
def main(unused_argv):
    ...
    if active_xla_option_nums > 1:
        raise ValueError("Only one of use_xla, xla_compile, use_autojit_xla can be set")
    ...
    if FLAGS.mode == "train":
        ...
        if FLAGS.mode == "infer":
            ...
            utils.print_out(
                "Running translate mode on file {}.".format(FLAGS.translate_file)
            )
            translate_mode = True
        ...
        dllogger.init(
            backends=[
                ...
                dllogger.JSONStreamBackend(
                    ...
                    os.path.join(FLAGS.output_dir, FLAGS.mode + "-report.json"),
                ),
            ]
        )
        ...
        _, (eval_speed, eval_latencies), eval_output_tokens = estimator.eval_fn(
        ...
        utils.print_out(
            ...
            % (
                eval_delta / 60.0,
                eval_speed,
                eval_speed * (eval_src_tokens + eval_output_tokens) / eval_sentences,
            ),
            f=sys.stderr,
        )
        logging_data = {
            "infer_speed_sent": eval_speed,
            "infer_speed_toks": eval_speed
            * (eval_src_tokens + eval_output_tokens)
            / eval_sentences,
        }
        for lat in sorted(eval_latencies):
            utils.print_out(
                "eval latency_%s for ckpt: %.2f ms" % (lat, eval_latencies[lat] * 1000)
            )
        ...
        if translate_mode:
            detokenize(
                hparams,
                hparams.translate_file + ".trans.tok",
        ...
        (
            ...
            train_src_tokens,
            train_tgt_tokens,
        ) = iterator_utils.get_effective_epoch_size(hparams)
        eval_sentences, eval_src_tokens, _ = iterator_utils.get_effective_epoch_size(
            hparams, train=False
        )
        ...
        while not should_stop:
            ...
            for lat in sorted(eval_latencies):
                utils.print_out(
                    "eval latency_%s for epoch %d: %.2f ms"
            ...
            if FLAGS.debug or (
                FLAGS.target_bleu is not None and bleu_score > FLAGS.target_bleu
            ):
                should_stop = True
                utils.print_out(
            ...
            if epochs >= FLAGS.max_train_epochs:
                should_stop = True
                utils.print_out(
                    "Stop job since max_train_epochs is reached.", f=sys.stderr
                )
    ...
    utils.print_out(
        "Experiment took {} min".format((experiment_end - experiment_start) / 60)
    )
