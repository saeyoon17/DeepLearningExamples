...
def build_collective_reduce(
    input_tensors, num_workers, num_shards, red_op="Add", un_op="Id"
):
...
def broadcast_send(t, shape, dtype, group_size, group_key, instance_key):
    return collective_ops.broadcast_send(
        t, shape, dtype, group_size, group_key, instance_key
    )


def broadcast_recv(shape, dtype, group_size, group_key, instance_key):
    return collective_ops.broadcast_recv(
        shape, dtype, group_size, group_key, instance_key
    )
...
def sum_gradients_all_reduce(
    ...
    aux_device_groups = group_device_names(
        aux_devices, num_shards if (alg != "collective" and alg_contains_shuffle) else 1
    )
    group_index = 0
    ...
    chunked_gv = [gv[x : x + merge_scope] for x in xrange(0, len(gv), merge_scope)]
...
def pack_range(key, packing, grad_vars, rng):
    ...
    with tf.name_scope("pack"):
        ...
        packing[key] = GradPackTuple(
            indices=range(rng[0], rng[1] + 1), vars=variables, shapes=restore_shapes
        )
    ...
    if small_ranges:
        ...
        for dev_idx, gv_list in enumerate(tower_grads):
            ...
            for r in small_ranges:
                ...
                new_gv_list.append(
                    (pack_range(key, packing, gv_list, r), "packing_var_placeholder")
                )
