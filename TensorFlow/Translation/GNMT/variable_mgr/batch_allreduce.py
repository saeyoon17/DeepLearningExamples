...
from variable_mgr import allreduce, constants
...
def _all_reduce_using_copy(tensors_across_devices, use_mean):
    ...
    if isinstance(tensors_across_devices[0], tf.IndexedSlices):
        ...
        if use_mean:
            val = tf.multiply(
                reduced_tensor.values, float(1.0 / len(tensors_across_devices))
            )
            reduced_tensor = tf.IndexedSlices(
                val, reduced_tensor.indices, reduced_tensor.dense_shape
            )
    else:
...
class BatchAllReduceAlgorithm(object):
    ...
    def batch_all_reduce(
        self, all_device_tensors, num_splits, compact_tensors, defer_tensors
    ):
        ...
        if defer_tensors:
            all_device_tensors = _add_put_op_control_deps(
                all_device_tensors, num_splits, put_ops
            )
    ...
    def _do_batch_all_reduce(self, all_device_tensors):
        ...
        for i, tensors_across_devices in enumerate(zip(*all_device_tensors)):
            with tf.device(self._devices[i % len(self._devices)]):
                reduced_tensor = _all_reduce_using_copy(
                    tensors_across_devices, self._use_mean
                )
...
class HierarchicalCopyAlgorithm(BatchAllReduceAlgorithm):
    ...
    def _do_batch_all_reduce(self, all_device_tensors):
        avail_devices = [
            device_tensors[0].device for device_tensors in all_device_tensors
        ]
        reduced_tensors = []
        ...
        for i, tensors_across_devices in enumerate(zip(*all_device_tensors)):
            ...
            group_0_tensors = tensors_across_devices[
                group_0_begin : group_0_begin + group_size
            ]
            ...
            group_1_tensors = tensors_across_devices[
                group_1_begin : group_1_begin + group_size
            ]
    ...
    def __get_main_devices(self, tensor_index, num_devices):
        ...
        if self._network_topology == constants.NetworkTopology.DGX1:
            return (
                tensor_index % num_devices,
                (tensor_index + (num_devices // 2)) % num_devices,
            )
...
class AllReduceSpecAlgorithm(BatchAllReduceAlgorithm):
    ...
    def _do_batch_all_reduce(self, all_device_tensors):
        ...
        # tower_grads.
        tower_grads = [
            [(t, None) for t in device_tensors] for device_tensors in all_device_tensors
        ]
...
def algorithm_from_params(params):
    ...
    if params.all_reduce_spec:
        ...
        return AllReduceSpecAlgorithm(
            params.all_reduce_spec,
            gpu_indices,
...
def _defer_all_device_tensors(all_device_tensors):
    ...
    new_all_device_tensors = _apply_to_all_device_tensors(
        all_device_tensors, apply_func
    )
...
def _undo_compact_all_device_tensors(all_device_tensors, orig_all_device_tensors):
    ...
    return _apply_to_all_device_tensors(all_device_tensors, apply_func, colocate=False)
...
class _TensorPacker(object):
    ...
    _concat_tensor_state = namedtuple(
        "_concat_tensor_state", ["orig_shapes", "orig_sizes"]
    )
    ...
    def _undo_concat_tensors(self, concatenated_tensor, concat_tensor_state):
        ...
        tensors_with_sizes = tf.split(
            concatenated_tensor, concat_tensor_state.orig_sizes
        )
        tensors_with_shapes = [
            tf.reshape(grad, shape)
            for grad, shape in zip(tensors_with_sizes, concat_tensor_state.orig_shapes)
        ]
    ...
    def undo_concat_all_device_tensors(self, all_device_tensors):
        ...
        new_all_device_tensors = []
        for [concat_tensor], tensor_state in zip(
            all_device_tensors, self._tensor_states
        ):
            with tf.colocate_with(concat_tensor):
                new_all_device_tensors.append(
                    self._undo_concat_tensors(concat_tensor, tensor_state)
                )
