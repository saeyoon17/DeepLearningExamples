...
def get_loss_scale_update_op(
    ...
    def increase_loss_scale_func():
        return tf.group(
            tf.assign(loss_scale_normal_steps, 0), tf.assign(loss_scale, loss_scale * 2)
        )
...
def append_gradients_with_loss_scale(
    training_ops, get_apply_gradients_ops_func, loss_scale_params, grad_has_inf_nan
):
    ...
    if loss_scale is None or not enable_auto_loss_scale or not is_chief:
        ...
        def update_op_if_nan_or_inf():
            ...
            return tf.group(
                tf.assign(loss_scale, loss_scale / 2.0),
...
class OverrideCachingDevice(object):
    ...
    def __init__(
        self, devices, device_for_small_variables, small_variable_size_threshold
    ):
...
ops.register_tensor_conversion_function(
    StagedModelVariable, StagedModelVariable._TensorConversionFunction
...
class StagedVariableGetter(object):
    ...
    def trainable_variables_on_device(self, rel_device_num, abs_device_num, writable):
...
def aggregate_single_gradient_using_copy(grad_and_vars, use_mean, check_inf_nan):
    ...
    if any(isinstance(g, tf.IndexedSlices) for g in grads):
        ...
        grad = gradients_impl._AggregateIndexedSlicesGradients(
            grads
        )  # pylint: disable=protected-access
    else:
        grad = tf.add_n(grads)
