...
from variable_mgr import allreduce, batch_allreduce, variable_mgr_util
...
class VariableMgr(object):
    ...
    def append_apply_gradients_ops(
        self, gradient_state, opt, grads, training_ops, loss_scale_params
    ):
    ...
    def trainable_variables_on_device(
        self, rel_device_num, abs_device_num, writable=False
    ):
...
class VariableMgrLocalReplicated(VariableMgr):
    ...
    def preprocess_device_grads(self, device_grads):
        compact_grads = (
            self.benchmark_cnn.params.use_fp16
            and self.benchmark_cnn.params.compact_gradient_transfer
        )
        ...
        reduced_grads, self._warmup_ops = algorithm.batch_all_reduce(
            grads_to_reduce,
            self.benchmark_cnn.params.gradient_repacking,
            compact_grads,
            defer_grads,
        )
        ...
        if (
            self.benchmark_cnn.params.use_fp16
            and self.benchmark_cnn.enable_auto_loss_scale
        ):
