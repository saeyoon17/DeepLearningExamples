...
from utils import iterator_utils, math_utils
...
def create_attention_mechanism(num_units, memory, source_sequence_length, dtype=None):
...
class BaseModel(object):
    ...
    def _set_params_initializer(self, hparams, mode, features, scope, extra_args=None):
    ...
    def _set_train_or_infer(self, res, hparams):
        ...
        if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
            ...
            ) + tf.reduce_sum(self.features["target_sequence_length"])
        ...
        if self.mode != tf.contrib.learn.ModeKeys.INFER:
            ...
            self.predict_count = tf.reduce_sum(self.features["target_sequence_length"])
        ...
        if hparams.use_dist_strategy and self.mode == tf.contrib.learn.ModeKeys.TRAIN:
            # Gradients
    ...
    def _get_decay_info(self, hparams):
        ...
        if hparams.decay_scheme in ["luong5", "luong10", "luong234", "jamesqin1616"]:
            epoch_size, _, _ = iterator_utils.get_effective_epoch_size(hparams)
            num_train_steps = int(
                hparams.max_train_epochs * epoch_size / hparams.batch_size
            )
            decay_factor = 0.5
            if hparams.decay_scheme == "luong5":
                ...
                decay_times = 16
                start_decay_step = int(num_train_steps / 16.0)
                remain_steps = num_train_steps - start_decay_step
    ...
    def build_graph(self, hparams, scope=None):
        ...
        with tf.variable_scope(scope or "dynamic_seq2seq", dtype=self.dtype):
            ...
            ## Decoder
            logits, sample_id = self._build_decoder(
    ...
    def _softmax_cross_entropy_loss(self, logits, labels, label_smoothing):
        ...
        if use_defun:
            ReduceSum = function.Defun(
                tf.float32, compiled=use_xla, noinline=True, grad_func=ReduceSumGrad
            )(ReduceSum)
        ...
        if abs(label_smoothing) > 1e-3:
            ...
            if use_defun:
                ComputeNegativeCrossent = function.Defun(compiled=use_xla)(
                    ComputeNegativeCrossent
                )
