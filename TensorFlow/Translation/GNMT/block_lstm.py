...
class LSTMBlockWrapper(base_layer.Layer):
    ...
    @abc.abstractmethod
    def _call_cell(
        self, inputs, initial_cell_state, initial_output, dtype, sequence_length
    ):
    ...
    def call(
        ...
        if initial_state is None:
            ...
            z = tf.zeros(tf.stack([batch_size, self.num_units]), dtype=dtype)
            initial_state = z, z
        else:
        ...
        (
            initial_cell_state,
            initial_output,
        ) = initial_state  # pylint: disable=unpacking-non-sequence
        ...
        if sequence_length is not None:
            if mask_output:
                ...
                mask = tf.transpose(
                    tf.sequence_mask(sequence_length, time_len, dtype=dtype), [1, 0]
                )
                mask = tf.tile(tf.expand_dims(mask, axis=-1), [1, 1, self.num_units])
                outputs *= mask
            """
      ...
      """
            ...
            mod_outputs = outputs
            final_cell_state = self._gather_states(
                mod_cell_states, sequence_length - 1, batch_size
            )
            final_output = self._gather_states(
                mod_outputs, sequence_length - 1, batch_size
            )
        else:
...
class LSTMBlockFusedCell(LSTMBlockWrapper):
    ...
    def __init__(
        ...
        """
        super(LSTMBlockFusedCell, self).__init__(_reuse=reuse, name=name, dtype=dtype)
    ...
    def build(self, input_shape):
        ...
        self._bias = self.add_variable(
            "bias", [self._num_units * 4], initializer=tf.constant_initializer(0.0)
        )
