...
from utils import iterator_utils, misc_utils, nmt_utils, vocab_utils
from variable_mgr import variable_mgr, variable_mgr_util
    ...
    def build_graph(self, features, labels, mode, params):
        ...
        if mode == tf.contrib.learn.ModeKeys.INFER:
            ...
            with mixed_precision_scope(), tf.device("gpu:0"), tf.name_scope(
                "tower_0"
            ), var_mgr.create_outer_variable_scope(0):
                    ...
                    with tf.device(devices[tid % len(devices)]), tf.name_scope(
                        ...
                        with var_mgr.create_outer_variable_scope(tid) as var_scope:
                            ...
                            (
                                ...
                                batch_size,
                                grads,
                            ) = unpack_fprop_and_bprop_output(outputs)
            ...
            misc_utils.print_out("All global vars(%d): " % len(tf.global_variables()))
            for gv in tf.global_variables():
                misc_utils.print_out(gv.name)

            misc_utils.print_out("master backproped params(%d): " % len(master_params))
            ...
            scaffold = tf.train.Scaffold(
                ...
                saver=tf.train.Saver(
                    saveable_vars + saveable_objects, save_relative_paths=True
                ),
            )
...
def make_model_fn(hparams):
    ...
    if hparams.use_dist_strategy:
        ...
        def fn(features, labels, mode, params):
            """docstring."""
            (loss, _, _, predictions, train_op, _) = factory.build_graph_dist_strategy(
        ...
        def fn(features, labels, mode, params):
            ...
            if mode == tf.contrib.learn.ModeKeys.INFER:
                ...
                return tf.estimator.EstimatorSpec(
                    mode=mode, loss=loss, scaffold=scaffold, train_op=train_op
                )
...
def make_input_fn(hparams, mode):
    ...
    def _synthetic_input_fn(params):
        ...
        features = {
            ...
            "target_input": tf.random_uniform(
                dtype=tf.int32,
                minval=1,
                maxval=10,
                seed=2,
            ...
            "target_output": tf.random_uniform(
                dtype=tf.int32,
                minval=1,
                maxval=10,
                seed=3,
            ...
            "source_sequence_length": tf.constant([src_max_len] * batch_size),
            "target_sequence_length": tf.constant([tgt_max_len] * batch_size),
        }
        return features
...
def get_sacrebleu(trans_file, detokenizer_file):
    ...
    # run sacrebleu
    cmd = (
        "cat %s | sacrebleu -t wmt14/full -l en-de --score-only -lc --tokenize" " intl"
    ) % (detok_trans_path)
...
def get_metrics(hparams, model_fn, ckpt=None, only_translate=False):
    ...
    if only_translate:
        ...
        trans_file = os.path.join(
            hparams.output_dir,
            "newstest2014_out_{}.tok.de".format(
    ...
    if only_translate:
        return (
            None,
            benchmark_hook.get_average_speed_and_latencies(),
            sum(output_tokens),
        )
