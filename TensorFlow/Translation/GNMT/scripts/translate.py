...
parser.add_argument(
    ...
    metavar="W1,[W2,...]",
    default="5",
    help="beam widths separated by comma",
)
...
for batch_size in batch_sizes:
    for beam_width in beam_widths:
        ...
        speed_search_res = re.search(
            rb"\neval time for ckpt: ((\d|.)+) mins \(((\d|.)+) sent/sec, ((\d|.)+) tokens/sec\)",
            out,
        )
        ...
        for lat in ["avg", "50", "90", "95", "99", "100"]:
            latencies.append(
                re.search(
                    r"\neval latency_{} for ckpt: ((\d|.)+) ms".format(lat).encode(),
                    out,
                )
            )
        if (
            bleu_search_res is None
            or speed_search_res is None
            or any(filter(lambda x: x is None, latencies))
        ):
