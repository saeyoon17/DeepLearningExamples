...
from tensorflow.python.ops import resource_variable_ops, rnn_cell_impl
from utils import math_utils, misc_utils
...
def _prepare_memory(memory, memory_sequence_length, check_inner_dims_defined):
    ...
    if memory_sequence_length is None:
        ...
        seq_len_batch_size = (
            memory_sequence_length.shape[0].value or tf.shape(memory_sequence_length)[0]
        )
...
class _BaseAttentionMechanism(AttentionMechanism):
    ...
    def __init__(
        ...
        """
        if query_layer is not None and not isinstance(query_layer, tf.layers.Layer):
            raise TypeError(
        ...
        self._probability_fn = lambda score, prev: (  # pylint:disable=g-long-lambda
            probability_fn(
                _maybe_mask_score(score, memory_sequence_length, score_mask_value), prev
            )
        )
        with tf.name_scope(name, "BaseAttentionMechanismInit", nest.flatten(memory)):
            ...
            self._keys = (
                self.memory_layer(self._values)
                if self.memory_layer  # pylint: disable=not-callable
                else self._values
            )
    ...
    def __init__(
        ...
        if self._normalize:
            ...
            self.g = tf.get_variable(
                "attention_g",
                dtype=dtype,
                initializer=tf.constant_initializer(math.sqrt((1.0 / num_units))),
                shape=(),
            )
        ...
        def _compute_alignments(query, state):
            with tf.variable_scope(None, "bahdanau_attention", [query]):
                ...
                score = _bahdanau_score(
                    processed_query, self._keys, self._normalize, self.v, self.g, self.b
                )
...
class AttentionWrapperState(
    ...
    def clone(self, **kwargs):
        ...
        return nest.map_structure(
            with_same_shape, self, super(AttentionWrapperState, self)._replace(**kwargs)
        )
...
def _compute_attention(
    ...
    context = math_utils.BatchMatMul(expanded_alignments, attention_mechanism.values)
...
class AttentionWrapper(tf.nn.rnn_cell.RNNCell):
        ...
        **NOTE** If you are using the `BeamSearchDecoder` with a cell wrapped in
        ...
        if attention_layer_size is not None:
            ...
            self._attention_layer_size = sum(
                layer.compute_output_shape(
                    [None, cell.output_size + mechanism.values.shape[-1].value]
                )[-1].value
        ...
        with tf.name_scope(name, "AttentionWrapperInit"):
            if initial_cell_state is None:
                ...
                state_batch_size = (
                    final_state_tensor.shape[0].value or tf.shape(final_state_tensor)[0]
                )
                error_message = (
    ...
    def force_build(self, cell_input_shape, query_shape):
        ...
        _build_cell(self._cell, (cell_input_shape[-1] + self._attention_layer_size))
...
class BahdanauAttentionFusedLayer(object):
    ...
    def __init__(
        ...
        # Bias added prior to the nonlinearity
        self.b = tf.get_variable(
            "attention_b", [num_units], dtype=dtype, initializer=tf.zeros_initializer()
        )
        if isinstance(self.b, resource_variable_ops.ResourceVariable):
            self.b = self.b.read_value()
    ...
    def __call__(self, queries):
        ...
        with tf.variable_scope(
            "multi_rnn_cell/cell_0_attention/attention/bahdanau_attention", [queries]
        ):
