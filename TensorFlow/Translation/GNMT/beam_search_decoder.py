...
def gather_tree_from_array(t, parent_ids, sequence_length):
    ...
    time_ind = tf.tile(
        tf.reshape(tf.range(max_time), [-1, 1, 1]), [1, batch_size, beam_width]
    )
    batch_ind = tf.tile(
        tf.reshape(tf.range(batch_size), [-1, 1, 1]), [1, max_time, beam_width]
    )
...
def _check_static_batch_beam_maybe(shape, batch_size, beam_width):
    ...
    if (
        ...
        and shape[0].value is not None
        and (
            shape[0] != batch_size * beam_width
            or (
                shape.ndims >= 2
                and shape[1].value is not None
...
class BeamSearchDecoder(tf.contrib.seq2seq.Decoder):
    ...
    **NOTE** If you are using the `BeamSearchDecoder` with a cell wrapped in
    ...
    def __init__(
        ...
        """
        if output_layer is not None and not isinstance(output_layer, tf.layers.Layer):
            raise TypeError(
    ...
    def finalize(self, outputs, final_state, sequence_lengths):
        ...
        max_sequence_lengths = tf.to_int32(tf.reduce_max(final_state.lengths, axis=1))
    ...
    def _maybe_sort_array_beams(self, t, parent_ids, sequence_length):
        ...
        if (
            ...
            shape = (
                t._element_shape[0]
                if t._infer_shape and t._element_shape
        ...
        if not _check_static_batch_beam_maybe(
            shape, tf.contrib.util.constant_value(self._batch_size), self._beam_width
        ):
            return t
        t = t.stack()
...
def _beam_search_step(
    ...
    batch_finished = tf.tile(
        tf.expand_dims(batch_finished, 2), [1, beam_width, vocab_size]
    )
    ...
    raw_next_word_ids = tf.mod(word_indices, vocab_size, name="next_beam_word_ids")
    next_word_ids = tf.to_int32(raw_next_word_ids)
    next_beam_ids = tf.to_int32(word_indices / vocab_size, name="next_beam_parent_ids")
...
def _get_scores(
    ...
    return tf.where(
        finished, log_probs / length_penalty_ + weighted_coverage_penalty, log_probs
    )
...
def _length_penalty(sequence_lengths, penalty_factor):
    ...
    if static_penalty is not None and static_penalty == 0:
        return 1.0
    return tf.div(
        (5.0 + tf.to_float(sequence_lengths)) ** penalty_factor,
