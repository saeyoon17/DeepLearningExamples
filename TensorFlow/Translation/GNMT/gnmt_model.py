...
class GNMTModel(model.BaseModel):
    ...
    def _build_encoder(self, hparams):
        ...
        with tf.variable_scope("encoder"):
            ...
            bi_encoder_outputs, bi_encoder_state = self._build_encoder_layers_bidi(
                encoder_emb_inp, self.features["source_sequence_length"], hparams, dtype
            )
    ...
    def _build_bidi_rnn_fused(self, inputs, sequence_length, hparams, dtype):
        if (
            not np.isclose(hparams.dropout, 0.0)
    ...
    def _build_unidi_rnn_fused(self, inputs, state, sequence_length, hparams, dtype):
        if (
            not np.isclose(hparams.dropout, 0.0)
            and self.mode == tf.contrib.learn.ModeKeys.TRAIN
        ):
        ...
        outputs, (final_c, final_h) = cell(
            inputs, state, dtype=dtype, sequence_length=sequence_length
        )
    ...
    def _build_unidi_rnn_cudnn(
        self, inputs, state, sequence_length, dtype, hparams, num_layers, is_fwd
    ):
        # cudnn inputs only support time-major
        if not self.time_major:
            inputs = tf.transpose(inputs, axis=[1, 0, 2])

        if num_layers == 1 and not np.isclose(hparams.dropout, 0.0):
    ...
    """
        ...
        states = tuple(tf.nn.rnn_cell.LSTMStateTuple(c=c, h=h) for h, c in zip(hs, cs))
        # No need to transpose back
        return outputs, states

    def _build_encoder_cell(self, hparams, num_layers, num_residual_layers, dtype=None):
    ...
    def _build_bidi_rnn_base(self, inputs, sequence_length, hparams, dtype):
        ...
        fw_cell = self._build_encoder_cell(
            hparams, 1, 0, dtype  # num_bi_layers,  # num_bi_residual_layers,
        )
    ...
    def _build_bidi_rnn_cudnn(self, inputs, sequence_length, hparams, dtype):
        ...
        # no dropout).
        if not np.isclose(hparams.dropout, 0.0):
            inputs = tf.nn.dropout(inputs, keep_prob=1 - hparams.dropout)
        if not hparams.use_loose_bidi_cudnn_lstm:
            ...
            bi_outputs, (bi_h, bi_c) = tf.contrib.cudnn_rnn.CudnnLSTM(
                ...
                dropout=0.0,  # one layer, dropout isn't applied anyway,
            ...
            # No need to transpose back
            return bi_outputs, (
                tf.nn.rnn_cell.LSTMStateTuple(c=fwd_c, h=fwd_h),
                tf.nn.rnn_cell.LSTMStateTuple(c=bak_c, h=bak_h),
            )

    def _build_encoder_layers_unidi(
        self, inputs, sequence_length, num_uni_layers, hparams, dtype
    ):
        ...
        if hparams.use_fused_lstm:
            for i in range(num_uni_layers):
                ...
                encoder_outputs, (final_c, final_h) = cell(
                    cell_inputs, dtype=dtype, sequence_length=sequence_length
                )
            ...
            for i in range(num_uni_layers):
                ...
                encoder_state += (
                    tf.nn.rnn_cell.LSTMStateTuple(encoder_states.c, encoder_states.h),
                )
                if i >= num_uni_layers - self.num_encoder_residual_layers:
                    encoder_outputs += inputs
                inputs = encoder_outputs
    ...
    def _build_decoder_cell(
        ...
        if hparams.pass_hidden_state:
            decoder_initial_state = tuple(
                ...
                if isinstance(zs, attention_wrapper.AttentionWrapperState)
                else es
    ...
    def _build_decoder_fused_for_training(
        self, encoder_outputs, initial_state, decoder_emb_inp, hparams
    ):
