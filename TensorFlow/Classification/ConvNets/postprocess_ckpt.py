...
def process_checkpoint(input_ckpt, output_ckpt_path, dense_layer):
    ...
    with tf.Session() as sess:
        ...
        tf.train.Saver(var_list=new_var_list).save(
            sess, output_ckpt_path, write_meta_graph=False, write_state=False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to pretrained RN50 checkpoint with dense layer",
    )
    parser.add_argument(
        "--dense_layer", type=str, default="resnet50/output/dense/kernel"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="output_dir",
        help="Output directory to store new checkpoint",
    )
