...
from utils import data_utils, hooks
...
class Runner(object):
    def __init__(
        ...
        self._model = resnet.ResnetModel(
            ...
            if "cardinality" in architecture
            else 1,
    ...
    def _get_session_config(
        mode, use_xla, use_dali, use_cpu, gpu_memory_fraction, gpu_id=0
    ):
    ...
    def _get_run_config(
        ...
        config = tf.estimator.RunConfig(
            ...
            else 1e9,  # disabled in benchmark mode
    ...
    def _get_estimator(
        self, mode, run_params, use_xla, use_dali, gpu_memory_fraction, gpu_id=0
    ):
    ...
    def train(
        ...
        if self.run_hparams.data_dir is not None:
            (
                ...
                num_epochs,
                num_decay_steps,
            ) = runner_utils.parse_tfrecords_dataset(
    ...
    def predict(
        ...
        try:
            ...
            for result in inference_results:
                print(
                    result["classes"], str(result["probabilities"][result["classes"]])
                )
