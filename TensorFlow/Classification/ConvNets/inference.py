...
from runtime import runner, runner_utils
...
def argument_parser() -> argparse.Namespace:
    ...
    exclusive_args.add_argument(
        ...
        type=str,
        default=None,
        help="Saved model location to use for inference",
    )
    exclusive_args.add_argument(
        "--architecture", type=str, choices=model_architectures.keys()
    )

    parser.add_argument(
        "--log-path", type=str, default="./log.json", help="Path to log file"
    )
    parser.add_argument(
        "--tf-trt", action="store_true", default=False, help="Use TF-TRT for inference"
    )
    parser.add_argument(
        "--amp", action="store_true", default=False, help="Use AMP for inference"
    )
    parser.add_argument(
        "--data-dir",
        type=str,
        required=False,
        default=None,
        help="Localization of validation data",
    )
    parser.add_argument(
...
def main(args: argparse.Namespace):
    ...
    if args.data_dir is not None:
        ...
        dataset = data_utils.get_tfrecords_input_fn(
            filenames=filenames,
    ...
    with tf.Session() as sess:
        ...
        tf.saved_model.loader.load(
            sess, [tf.saved_model.tag_constants.SERVING], str(saved_model_to_load)
        )
        ...
        try:
            ...
            for samples_processed in range(int(num_steps)):
                if args.data_dir is not None:
                    ...
                    if samples_processed == 0:
                        next_batch_image = np.random.normal(
                ...
                output = sess.run(
                    [output_tensor_name], feed_dict={"input_tensor:0": next_batch_image}
                )
                ...
                if samples_processed % LOG_FREQUENCY == 0 and samples_processed != 0:
                    ...
                    current_throughput = (
                        LOG_FREQUENCY * batch_size / (current_time - last_time)
                    )
                    dllogger.log(
                        step=(0, samples_processed),
            ...
            dllogger.log(
                step=tuple(),
                data={
                    "throughput": image_processed / (last_time - start_time),
