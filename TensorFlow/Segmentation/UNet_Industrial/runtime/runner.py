...
class Runner(object):
    def __init__(
        ...
        os.environ["TF_GPU_THREAD_COUNT"] = (
            "1" if not hvd_utils.is_using_hvd() else str(hvd.size())
        )
    ...
    def _get_run_config(mode, model_dir, xla, seed=None):
        ...
        if mode == "train":
            if hvd_utils.is_using_hvd():
                ...
                config = config.replace(
                    save_checkpoints_steps=1000, keep_checkpoint_every_n_hours=3
                )
    ...
    def train(
        ...
        if self.run_hparams.data_dir is not None:
            (
                ...
                num_steps,
                num_epochs,
            ) = self.dataset.get_dataset_runtime_specs(
                training=True,
                iter_unit=iter_unit,
                num_iter=num_iter,
        ...
        if not hvd_utils.is_using_hvd() or hvd.rank() == 0:
            ...
            Logger.log(
                step=("PARAMETER"),
                data={"Learning Rate Decay Factor": learning_rate_decay_factor},
                verbosity=Logger.Verbosity.DEFAULT,
            )
            Logger.log(
                step=("PARAMETER"),
            ...
            Logger.log(
                step=("PARAMETER"),
                data={"Total Files to be Processed": num_steps * global_batch_size},
                verbosity=Logger.Verbosity.DEFAULT,
            )
        ...
        model = self._get_estimator(
            mode="train", run_params=estimator_params, xla=self.xla
        )
    ...
    def evaluate(
        ...
        model = self._get_estimator(
            mode="validation", run_params=estimator_params, xla=self.xla
        )
        ...
        try:
            ...
            data_to_log = {
                "{prefix}.{key}".format(prefix=Logger._stage, key=key): float(val)
                for key, val in sorted(eval_results.items(), key=operator.itemgetter(0))
                if not any(
                    val in key for val in ["loss", "global_step", "Confusion_Matrix"]
                )
            }
            ...
            Logger.log(
                step=(num_steps,),
                data={
                    "{prefix}.false_positives".format(prefix=Logger._stage): str(fps)
                },
                verbosity=Logger.Verbosity.DEFAULT,
            )
            ...
            Logger.log(
                step=(num_steps,),
                data={
                    "{prefix}.false_negatives".format(prefix=Logger._stage): str(fns)
                },
                verbosity=Logger.Verbosity.DEFAULT,
            )
            ...
            Logger.log(
                step=(num_steps,),
                data={
                    "{prefix}.true_positive_rate".format(prefix=Logger._stage): str(
                        ["%.3f" % x for x in tpr]
                    )
                },
                verbosity=Logger.Verbosity.DEFAULT,
            ...
            Logger.log(
                step=(num_steps,),
                data={
                    "{prefix}.true_negative_rate".format(prefix=Logger._stage): str(
                        ["%.3f" % x for x in tnr]
                    )
                },
                verbosity=Logger.Verbosity.DEFAULT,
            ...
            if save_eval_results_to_json:
                ...
                with open(
                    os.path.join(self.run_hparams.model_dir, "..", "results.json"), "w"
                ) as f:
