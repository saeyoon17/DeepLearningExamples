...
class ProfilerHook(tf.train.SessionRunHook):
    ...
    def after_create_session(self, session, coord):
        ...
        params_count = tf.get_default_graph().get_tensor_by_name(
            "trainable_parameters_count_ref:0"
        )
        ...
        Logger.log(
            step=("PARAMETER"),
            data={"# Total Trainable Parameters": int(_params_count)},
    ...
    def before_run(self, run_context):
        ...
        if self._current_step % self._log_every == 0:
            ...
            additional_fetches = {
                "total_loss": tf.get_default_graph().get_tensor_by_name(
                    "losses/total_loss_ref:0"
                ),
                "iou_scores": dict(),
            ...
            if self._is_training:
                ...
                additional_fetches[
                    "weight_decay"
                ] = tf.get_default_graph().get_tensor_by_name("losses/l2_loss_ref:0")

                additional_fetches[
                    "reconstruction_loss"
                ] = tf.get_default_graph().get_tensor_by_name(
                    "losses/reconstruction_loss_ref:0"
                )

                additional_fetches[
                    "learning_rate"
                ] = tf.get_default_graph().get_tensor_by_name(
                    "optimizers/learning_rate_ref:0"
                )
            ...
            if self._sample_dir is not None and self._is_training:
                ...
                additional_fetches["samples"][
                    "input_image"
                ] = tf.get_default_graph().get_tensor_by_name("input_image_jpeg_ref:0")

                additional_fetches["samples"][
                    "mask"
                ] = tf.get_default_graph().get_tensor_by_name("mask_sample_ref:0")
            ...
            for threshold in [0.05, 0.125, 0.25, 0.5, 0.75, 0.85, 0.95, 0.99]:
                ...
                if threshold is not None:
                    additional_fetches["iou_scores"][
                        str(threshold)
                    ] = tf.get_default_graph().get_tensor_by_name(
                        "IoU_Metrics/iou_score_ths_%s_ref:0" % threshold
                    )

            additional_fetches["confusion_matrix"][
                "tp"
            ] = tf.get_default_graph().get_tensor_by_name(
    ...
    def after_run(self, run_context, run_values):
        ...
        if is_log_step:
            ...
            if self._current_step > self._warmup_steps:
                imgs_per_sec = float(
                    ProfilerHook.moving_average(self._processing_speed_arr, n=30)[-1]
                )

            Logger.log(
                step=(self._current_step,),
                data={
                    "{prefix}.avg_ips".format(prefix=Logger._stage): float(imgs_per_sec)
                },
                verbosity=Logger.Verbosity.DEFAULT,
            )
            ...
            if self._is_training:
                Logger.log(
                    step=(self._current_step,),
                    data={
                        "{prefix}.weight_decay".format(prefix=Logger._stage): float(
                            run_values.results["weight_decay"]
                        )
                    },
                    verbosity=Logger.Verbosity.DEFAULT,
                )
                Logger.log(
                    step=(self._current_step,),
                    data={
                        "{prefix}.reconstruction_loss".format(
                            prefix=Logger._stage
                ...
                Logger.log(
                    step=(self._current_step,),
                    data={
                        "{prefix}.total_loss".format(prefix=Logger._stage): float(
                            run_values.results["total_loss"]
                        )
                    },
                    verbosity=Logger.Verbosity.DEFAULT,
                )
                Logger.log(
                    step=(self._current_step,),
                    data={
                        "{prefix}.learning_rate".format(prefix=Logger._stage): float(
                            run_values.results["learning_rate"]
                        )
                    },
                    verbosity=Logger.Verbosity.DEFAULT,
                )
            ...
            for key, val in sorted(
                run_values.results["iou_scores"].items(), key=operator.itemgetter(0)
            ):
                Logger.log(
                    step=(self._current_step,),
                    data={
                        "{prefix}.IoU_THS_{ths}".format(
                            prefix=Logger._stage, ths=key
                        ): float(val)
                    },
                    verbosity=Logger.Verbosity.DEFAULT,
                )
            ...
            Logger.log(
                step=(self._current_step,),
                data={
                    "{prefix}.true_positives".format(prefix=Logger._stage): str(
            ...
            Logger.log(
                step=(self._current_step,),
                data={
                    "{prefix}.true_negatives".format(prefix=Logger._stage): str(
            ...
            Logger.log(
                step=(self._current_step,),
                data={
                    "{prefix}.false_positives".format(prefix=Logger._stage): str(
            ...
            Logger.log(
                step=(self._current_step,),
                data={
                    "{prefix}.false_negatives".format(prefix=Logger._stage): str(
            ...
            if self._sample_dir is not None and self._is_training:
                ...
                for key in sorted(
                    run_values.results["samples"].keys(), key=operator.itemgetter(0)
                ):

                    with open(
                        os.path.join(
                            self._sample_dir,
                            "sample_step_%04d_ths_%s.jpeg" % (self._current_step, key),
                        ),
                        "wb",
                    ) as fd:
                    ...
                    with open(
                        os.path.join(
                            self._sample_dir,
                            "sample_step_%04d_mask.jpeg" % self._current_step,
                        ),
                        "wb",
                    ) as fd:
    ...
    def end(self, session):
        ...
        try:
            avg_processing_speed = float(
                ProfilerHook.moving_average(self._processing_speed_arr, n=100)[-1]
            )
        except:
        ...
        Logger.log(
            step=(),
            data={
                "{prefix}.avg_ips".format(prefix=Logger._stage): avg_processing_speed
            },
            verbosity=Logger.Verbosity.DEFAULT,
        )
        ...
        perf_filename = "performances_%s.json" % (
