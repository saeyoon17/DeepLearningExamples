...
def _model_fn(features, labels, mode, params):
    ...
    learning_rate = tf.train.exponential_decay(
        lr_init, global_step, decay_steps=max_steps, decay_rate=0.96
    )
    ...
    with tf.device(device):
        ...
        with model_variable_scope(
            "UNet", reuse=tf.AUTO_REUSE, dtype=tf.float16, debug_mode=False
        ):
            ...
            dice_loss = tf.reduce_mean(
                1 - dice_coef(flat_logits, flat_labels), name="dice_loss_ref"
            )
...
class Runner:
    ...
    def predict(self):
        ...
        if hvd.rank() == 0:
            ...
            pickle.dump(
                predictions, open(os.path.join(output_dir, "predictions.pkl"), "wb")
            )
    ...
    def benchmark(self):
        if hvd.rank() == 0:
            self._classifier.evaluate(
                input_fn=self._dataset.synth_fn,
                steps=self._max_steps,
