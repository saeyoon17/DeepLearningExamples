...
def _ce(y_true, y_pred):
    ...
    return tf.reduce_sum(
        tf.reduce_mean(
            tf.keras.backend.binary_crossentropy(tf.cast(y_true, tf.float32), y_pred),
            axis=[0, 1, 2, 3],
        ),
...
def _dice(y_true, y_pred):
    ...
    """
    return tf.reduce_sum(
        dice_loss(predictions=y_pred, targets=y_true), name="dice_loss_ref"
    )
...
def dice_loss(predictions, targets, squared_pred=False, smooth=1e-5, top_smooth=0.0):
    """Dice
    ...
    reduce_axis = (
        list(range(2, n_len)) if is_channels_first else list(range(1, n_len - 1))
    )
...
def total_dice(predictions, targets, smooth=1e-5, top_smooth=0.0):
    """Total Dice
