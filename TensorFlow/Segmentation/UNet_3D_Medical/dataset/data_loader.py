...
from dataset.transforms import (Cast, CenterCrop, NormalizeImages,
                                OneHotLabels, PadXYZ,
                                RandomBrightnessCorrection, RandomCrop3D,
                                RandomHorizontalFlip, apply_test_transforms,
                                apply_transforms)
...
def cross_validation(arr: np.ndarray, fold_idx: int, n_folds: int):
    ...
    if fold_idx < 0 or fold_idx >= n_folds:
        raise ValueError(
            "Fold index has to be [0, n_folds). Received index {} for {} folds".format(
                fold_idx, n_folds
            )
        )
    ...
    return (
        np.concatenate(_folders[:fold_idx] + _folders[fold_idx + 1 :]),
        _folders[fold_idx],
    )
...
class Dataset:  # pylint: disable=R0902
    ...
    def __init__(
        ...
        data_dir,
        batch_size=2,
        input_shape=(128, 128, 128),  # pylint: disable=R0913
        fold_idx=0,
        n_folds=5,
        seed=0,
        params=None,
        ...
        self._train, self._eval = cross_validation(
            self._folders, fold_idx=fold_idx, n_folds=n_folds
        )
    ...
    def train_fn(self):
        ...
        dataset = dataset.map(
            self.parse, num_parallel_calls=tf.data.experimental.AUTOTUNE
        )
    ...
    def eval_fn(self):
        ...
        assert (
            len(self._eval) > 0
        ), "Evaluation data not found. Did you specify --fold flag?"
    ...
    def test_fn(self):
        ...
        dataset = dataset.map(
            self.parse_x, num_parallel_calls=tf.data.experimental.AUTOTUNE
        )
        ...
        dataset = dataset.map(
            map_func=lambda x, mean, stdev: apply_test_transforms(
                x, mean, stdev, transforms=transforms
            ),
            num_parallel_calls=tf.data.experimental.AUTOTUNE,
        )
        dataset = dataset.batch(
            batch_size=self._batch_size, drop_remainder=self.params.benchmark
        )
        dataset = dataset.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
    ...
    def synth_train_fn(self):
        ...
        mean = tf.random.uniform(
            (4,), dtype=tf.float32, minval=0, maxval=255, seed=self._seed
        )
        stddev = tf.random.uniform(
            (4,), dtype=tf.float32, minval=0, maxval=1, seed=self._seed
        )
        ...
        dataset = dataset.map(
            map_func=lambda x, y: apply_transforms(x, y, mean, stddev, transforms),
            num_parallel_calls=tf.data.experimental.AUTOTUNE,
        )
