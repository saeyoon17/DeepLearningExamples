...
class RandomCrop3D:  # pylint: disable=R0903
    ...
    def __call__(self, samples, labels, mean, stdev):
        ...
        max_ = tf.constant(
            [
                shape[0].value - self.shape[0] - self.margins[0],
...
class NormalizeImages:  # pylint: disable=R0903
    ...
    def __call__(self, samples, labels, mean, stdev):
        ...
        samples = tf.where(
            mask,
            (samples - tf.cast(mean, samples.dtype))
            / (tf.cast(stdev + 1e-8, samples.dtype)),
            samples,
        )
...
class RandomHorizontalFlip:  # pylint: disable=R0903
    ...
    def __call__(self, samples, labels, mean, stdev):
        ...
        samples = tf.cond(
            h_flip, lambda: tf.reverse(samples, axis=[1]), lambda: samples
        )
...
class RandomVerticalFlip:  # pylint: disable=R0903
    ...
    def __call__(self, samples, labels, mean, stdev):
        ...
        samples = tf.cond(
            h_flip, lambda: tf.reverse(samples, axis=[0]), lambda: samples
        )
...
class RandomGammaCorrection:  # pylint: disable=R0903
    ...
    def __init__(
        self, gamma_range=(0.8, 1.5), keep_stats=False, threshold=0.5, epsilon=1e-8
    ):
    ...
    def __call__(self, samples, labels, mean, stdev):
        ...
        gamma = tf.random_uniform(
            [], minval=self._gamma_range[0], maxval=self._gamma_range[1]
        )
        ...
        samples = tf.cond(
            augment,
            lambda: tf.math.pow(((samples - x_min) / float(x_range + self._eps)), gamma)
            * x_range
            + x_min,
            lambda: samples,
        )
