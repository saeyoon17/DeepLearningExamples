...
import numpy as np
from dataset.data_loader import CLASSES, Dataset
from model.model_fn import unet_3d
from runtime.arguments import PARSER
from runtime.hooks import get_hooks
from runtime.setup import build_estimator, get_logger, set_flags
...
def parse_evaluation_results(result, logger, step=()):
    ...
    data["mean_dice"] = sum([result[CLASSES[i]] for i in range(len(CLASSES))]) / len(
        CLASSES
    )
...
def main():
    ...
    if "train" in params.exec_mode:
        ...
        estimator.train(input_fn=dataset.train_fn, steps=max_steps, hooks=hooks)
    ...
    if params.exec_mode == "predict":
        if hvd.rank() == 0:
            predictions = estimator.predict(input_fn=dataset.test_fn, hooks=hooks)
