...
def parse_convergence_results(path, environment):
    ...
    for logfile in logfiles:
        ...
        tumor_core.append(
            float(
                [val for val in content.split("  ") if "tumor_core" in val][0].split()[
                    -1
                ]
            )
        )
        peritumoral_edema.append(
            float(
                [val for val in content.split("  ") if "peritumoral_edema" in val][
                    0
                ].split()[-1]
            )
        )
        enhancing_tumor.append(
            float(
                [val for val in content.split("  ") if "enhancing_tumor" in val][
                    0
                ].split()[-1]
            )
        )
        mean_dice.append(
            float(
                [val for val in content.split("  ") if "mean_dice" in val][0].split()[
                    -1
                ]
            )
        )
    ...
    if whole_tumor:
        ...
        print(
            "Evaluation peritumoral edema dice score:",
            sum(peritumoral_edema) / len(peritumoral_edema),
        )
        print(
            "Evaluation enhancing tumor dice score:",
            sum(enhancing_tumor) / len(enhancing_tumor),
        )
    else:
