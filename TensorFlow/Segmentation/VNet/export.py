...
from utils.tf_export import to_onnx, to_savedmodel, to_tf_trt

PARSER = argparse.ArgumentParser(description="V-Net")

PARSER.add_argument(
    "--to", dest="to", choices=["savedmodel", "tftrt", "onnx"], required=True
)
...
PARSER.add_argument(
    "--input_shape", nargs="+", type=int, help="""Model's input shape"""
)

PARSER.add_argument(
    "--data_dir", type=str, help="""Directory where the dataset is located"""
)

PARSER.add_argument(
    "--checkpoint_dir", type=str, help="""Directory where the checkpoint is located"""
)

PARSER.add_argument(
    "--savedmodel_dir", type=str, help="""Directory where the savedModel is located"""
)
...
def main():
    ...
    if flags.to == "tftrt":
        ds = MSDDataset(
            json_path=flags.data_dir + "/dataset.json", interpolator="linear"
        )
        ...
        to_tf_trt(
            savedmodel_dir=flags.savedmodel_dir,
