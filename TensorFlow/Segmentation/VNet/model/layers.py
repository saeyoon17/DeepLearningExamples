...
def convolution_layer(
    inputs, filters, kernel_size, stride, normalization, activation, mode
):
    x = tf.layers.conv3d(
        inputs=inputs,
        filters=filters,
...
def residual_block(
    input_0, input_1, kernel_size, depth, normalization, activation, mode
):
...
def downsample_block(
    inputs, depth, kernel_size, pooling, normalization, activation, mode
):
