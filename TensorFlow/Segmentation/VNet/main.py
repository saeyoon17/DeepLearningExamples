...
def main(_):
    ...
    run_config = tf.estimator.RunConfig(
        ...
        if FLAGS.benchmark
        else dataset.train_steps * FLAGS.train_epochs,
    ...
    if "train" in FLAGS.exec_mode:
        ...
        if FLAGS.benchmark:
            ...
            if hvd.rank() == 0:
                train_hooks += [
                    ProfilingHook(
                        FLAGS.warmup_steps, FLAGS.batch_size * hvd.size(), DLLogger
                    )
                ]
        else:
            if hvd.rank() == 0:
    ...
    if "evaluate" in FLAGS.exec_mode:
        if hvd.rank() == 0:
            ...
            result = estimator.evaluate(
                input_fn=dataset.eval_fn, steps=dataset.eval_steps, hooks=[]
            )
