...
def vnet_v2(features, labels, mode, params):
    ...
    with model_variable_scope(
        "vnet", reuse=tf.AUTO_REUSE, dtype=tf.float16, debug_mode=False
    ):
        features = tf.reshape(features, [params.batch_size] + params.input_shape + [1])
        if labels is not None:
            labels = tf.reshape(labels, [params.batch_size] + params.input_shape + [1])

        logits = Builder(
            kernel_size=params.convolution_size,
        ...
        one_hot_labels = tf.one_hot(
            indices=flattened_labels, depth=num_classes, dtype=tf.float32
        )

        with tf.name_scope("loss"):
            if params.loss == "dice":
                loss = dice_coef(
                    predict=tf.cast(flattened_softmax, tf.float32),
                    target=one_hot_labels,
                    dice_type="sorensen",
                )
                total_loss = tf.identity(
                    tf.reduce_sum(1.0 - loss), name="total_loss_ref"
                )
            else:
        ...
        if is_training:
            ...
            with tf.control_dependencies(update_ops):
                ...
                try:
                    amp_envar_enabled = (
                        int(os.environ["TF_ENABLE_AUTO_MIXED_PRECISION"]) == 1
                    )
                except KeyError:
        ...
        if is_eval:
            ...
            for i in range(num_classes):
                eval_metric_ops["%s dice" % params.labels[str(i)]] = tf.metrics.mean(
                    eval_loss[i]
                )

    return tf.estimator.EstimatorSpec(
        mode=mode, loss=total_loss, train_op=train_op, eval_metric_ops=eval_metric_ops
    )
