...
PARSER.add_argument(
    "--data_dir", required=True, type=str, help="Directory where the dataset is stored"
)
...
PARSER.add_argument(
    "--gpus", choices=[1, 8], required=True, type=int, help="Number of GPUs"
)
...
PARSER.add_argument(
    "--base_lr", default=0.0001, type=float, help="Initial learning rate for RMSProp"
)


def build_horovod_prefix(gpus):
    return (
        "mpirun -np {} -H localhost:{} -bind-to none -map-by slot -x NCCL_DEBUG=INFO -x LD_LIBRARY_PATH -x PATH -mca "
