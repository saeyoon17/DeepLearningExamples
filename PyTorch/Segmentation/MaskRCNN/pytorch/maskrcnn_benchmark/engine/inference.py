...
def inference(
    ...
    dllogger.log(
        step="PARAMETER",
        data={"eval_dataset_name": dataset_name, "eval_num_samples": len(dataset)},
    )
    start_time = synchronized_timestamp()
    with torch.autograd.profiler.emit_nvtx(enabled=profile):
        predictions, latency = compute_on_dataset(
            model, data_loader, device, steps=steps
        )
    ...
    dllogger.log(
        ...
        data={
            ...
            "inference_perf_fps": len_dataset / total_time,
        },
    )
    stats = {
    ...
    if skip_eval:
        dllogger.log(
            ...
            data={
                "skip_eval": True,
                "predictions_saved_path": os.path.join(
                    output_folder, "predictions.pth"
                ),
            },
        )
        return
