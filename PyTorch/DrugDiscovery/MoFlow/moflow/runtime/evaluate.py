...
from moflow.utils import (check_novelty, check_validity,
                          convert_predictions_to_mols, predictions_to_smiles)
    ...
    with autocast(enabled=args.amp):
        for i in range(args.steps):
            ...
            novel_r, abs_novel_r = check_novelty(
                validity_info["valid_smiles"], train_smiles, len(mols_batch)
            )
            _, nuv = check_novelty(
                list(set(validity_info["valid_smiles"])), train_smiles, len(mols_batch)
            )
            metrics = {
