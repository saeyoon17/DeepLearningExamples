...
def update_argparser(parser):
    parser.add_argument("--config", type=str, required=True, help="Network to deploy")
    parser.add_argument(
        "--val-path", type=str, help="Path to dataset to be used", required=True
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        required=True,
        help="Bool on whether network is a prunet",
    )
...
def get_dataloader_fn(config, val_path, batch_size, precision, is_prunet):
    ...
    def _get_dataloader():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            y_real = {
                "OUTPUT__0": np.tile(
                    target.to(dtype).cpu().numpy()[:, np.newaxis], (1, 1000)
                )
            }
