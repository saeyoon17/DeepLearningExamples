...
from ..core import (BaseLoader, BaseRunner, BaseRunnerSession, Format, Model,
                    TensorSpec, TimeMeasurement)
    ...
    def _prepare_buffers_if_needed(self, x_host: Dict[str, object]):
        ...
        if self._has_dynamic_shapes or new_batch_size != current_batch_size:
            ...
            for name in self._output_names:
                ...
                binding_idx: int = self._engine[name]
                dtype_from_trt_binding = np.dtype(
                    trt.nptype(self._engine.get_binding_dtype(binding_idx))
                )
            ...
            x_host = {
                name: _cast_input(name, host_input)
