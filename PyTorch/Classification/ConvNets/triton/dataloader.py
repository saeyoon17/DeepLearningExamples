    ...
    *,
    ...
    def _dataloader():
        ...
        for image_path in image_paths:
            ...
            img = img / np.array([0.229, 0.224, 0.225], dtype=np.float32).reshape(
                1, 1, 3
            )
            ...
            if len(batch) >= batch_size:
                ...
                x = {
                    "INPUT__0": np.ascontiguousarray(
                        np.transpose(
                            np.concatenate([img for img, *_ in batch]), (0, 3, 1, 2)
                        ).astype(np.float32 if precision == "fp32" else np.float16)
                    )
                }
                y_real = {
                    "OUTPUT__0": onehot[
                        np.concatenate([class_ for *_, class_ in batch])
                    ].astype(np.float32 if precision == "fp32" else np.float16)
                }
                batch = []
                yield ids, x, y_real
