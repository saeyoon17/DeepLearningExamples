...
def parse_args(parser):
    ...
    parser.add_argument(
        "--log-file", type=str, default="split_log.json", help="Filename for logging"
    )
    parser.add_argument(
        "--subsets",
        type=str,
    ...
    parser.add_argument(
        "--add-transcript", action="store_true", help="Add text columns to filelists"
    )
    parser.add_argument(
        "--add-pitch", action="store_true", help="Add pitch columns to filelists"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Increase verbosity"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Do not create actual files"
    )
    return parser
...
def generate_filelist(subset, meta, base_fname, cols_to_dump, args):
    ...
    if subset == "all":
        subset_meta = meta[
            meta.index.map(lambda fname: fname not in discard_train_ids_v3)
        ]
    elif subset == "val":
        ...
        subset_meta = meta[
            meta.index.map(
                lambda fname: (
                    fname not in val_ids
                    and fname not in test_ids
    ...
    if not args.dry_run:
        ...
        subset_meta.to_csv(
            ...
            quoting=QUOTE_NONE,
            index=None,
            columns=cols_to_dump,
        )


def main():
    parser = argparse.ArgumentParser(description="LJSpeech filelists generation")
