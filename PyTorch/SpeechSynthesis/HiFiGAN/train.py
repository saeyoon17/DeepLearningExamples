...
from common.utils import (Checkpointer, freeze, init_distributed, l2_promote,
                          print_once, reduce_tensor, unfreeze)
from hifigan.data_function import get_data_loader, mel_spectrogram
from hifigan.logging import Metrics, init_logger
...
def parse_args(parser):
    parser.add_argument(
        "-o", "--output", type=str, required=True, help="Directory to save checkpoints"
    )
    parser.add_argument(
        "--log_file", type=str, default=None, help="Path to a DLLogger log file"
    )
    ...
    train.add_argument(
        "--autocast_spectrogram",
        action="store_true",
        help="Enable autocast while computing spectrograms",
    )
    train.add_argument("--cuda", action="store_true", help="Run on GPU using CUDA")
    train.add_argument(
    ...
    train.add_argument(
        "--num_workers", type=int, default=1, help="Data loader workers number"
    )
    train.add_argument("--fine_tuning", action="store_true", help="Enable fine-tuning")
    train.add_argument(
        "--input_mels_dir",
        type=str,
        default=None,
    ...
    opt.add_argument(
        "--optimizer", type=str, default="adamw", help="Optimization algorithm"
    )
    opt.add_argument(
        "--lr_decay", type=float, default=0.9998, help="Learning rate decay"
    )
    opt.add_argument(
    ...
    opt.add_argument(
        ...
        required=True,
        help=(
            "Batch size per training iter. "
            "May be split into grad accumulation steps."
        ),
    )
    opt.add_argument(
        "--warmup_steps", type=int, default=1000, help="Number of steps for lr warmup"
    )
    ...
    audio.add_argument(
        "--max_wav_value", default=32768.0, type=float, help="Maximum audiowave value"
    )
    audio.add_argument("--sampling_rate", default=22050, type=int, help="Sampling rate")
    audio.add_argument("--filter_length", default=1024, type=int, help="Filter length")
    audio.add_argument("--num_mels", default=80, type=int, help="number of Mel bands")
    audio.add_argument(
        "--hop_length", default=256, type=int, help="Hop (stride) length"
    )
    audio.add_argument("--win_length", default=1024, type=int, help="Window length")
    audio.add_argument(
        "--mel_fmin", default=0.0, type=float, help="Minimum mel frequency"
    )
    audio.add_argument(
        "--mel_fmax", default=8000.0, type=float, help="Maximum mel frequency"
    )
    audio.add_argument(
        "--mel_fmax_loss",
        default=None,
    ...
    audio.add_argument(
        "--segment_size", default=8192, type=int, help="Training segment size"
    )
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            with autocast(enabled=args.amp and args.autocast_spectrogram):
                y_g_hat_mel = mel_spec(
                    y_g_hat.float().squeeze(1), fmax=args.mel_fmax_loss
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="PyTorch HiFi-GAN Training", allow_abbrev=False
    )
    ...
    if args.affinity != "disabled":
        ...
        affinity = gpu_affinity.set_affinity(
            args.local_rank, nproc_per_node, args.affinity
        )
    ...
    mpd = MultiPeriodDiscriminator(
        periods=args.mpd_periods, concat_fwd=args.concat_fwd
    ).cuda()

    assert (
        args.amp or not args.no_amp_grouped_conv
    ), "--no-amp-grouped-conv is applicable only when AMP is enabled"
    msd = MultiScaleDiscriminator(
        concat_fwd=args.concat_fwd, no_amp_grouped_conv=args.no_amp_grouped_conv
    )
    msd = msd.cuda()
    ...
    proto = {"adam": FusedAdam, "lamb": FusedLAMB, "adamw": AdamW}[args.optimizer]
    ...
    # setup DDP
    if distributed_run:
        kw = {"device_ids": [args.local_rank], "output_device": args.local_rank}
        gen = DDP(gen, **kw)
        msd = DDP(msd, **kw)
    ...
    val_loader = get_data_loader(
        args, distributed_run, train=False, val_kwargs=dict(repeat=5, split=True)
    )
    val_samples_loader = get_data_loader(
        args, False, train=False, val_kwargs=dict(split=False), batch_size=1
    )
    ...
    for epoch in range(max(1, last_epoch), args.epochs + 1):
        ...
        for step, batch in enumerate(train_loader):
            ...
            assert args.grad_accumulation > 1 or (
                is_first_accum_step and is_last_accum_step
            )
            ...
            with torch.set_grad_enabled(is_last_accum_step), autocast(enabled=args.amp):
                y_g_hat = gen(x)
        ...
        if epoch % args.validation_interval == 0:
            ...
            logger.log(
                (epoch,), val_metrics, scope="val", tb_iter=iters_all, flush_log=True
            )
            ...
            with torch.no_grad():
                for i, batch in enumerate(islice(val_samples_loader, 5)):
                    ...
                    with autocast(enabled=args.amp and args.autocast_spectrogram):
                        ...
                        y_hat_spec = mel_spec(
                            y_g_hat.float().squeeze(1), fmax=args.mel_fmax
                        )

                    logger.log_samples_tb(
                        iters_all, i, y_g_hat, y_hat_spec, args.sampling_rate
                    )
        ...
        epochs_done += 1
        if args.epochs_this_job is not None and epochs_done == args.epochs_this_job:
            break
