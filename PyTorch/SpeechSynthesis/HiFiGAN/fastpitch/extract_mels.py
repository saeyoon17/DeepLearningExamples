...
def parse_args(parser):
    ...
    general.add_argument(
        "--resume", action="store_true", help="Load last checkpoint from training"
    )
    general.add_argument("--amp", action="store_true", help="Enable AMP")
    general.add_argument("--cuda", action="store_true", help="Run on GPU using CUDA")
    general.add_argument(
    ...
    data.add_argument(
        "--heteronyms-path",
        type=str,
        default="data/cmudict/heteronyms",
        help="Path to the list of heteronyms",
    )
    data.add_argument(
        "--cmudict-path",
        type=str,
        default="data/cmudict/cmudict-0.7b",
...
def main():
    ...
    for k in CHECKPOINT_SPECIFIC_ARGS:
        ...
        if k in train_setup and getattr(args, k) != train_setup[k]:
            v = train_setup[k]
            print(
                f"Overwriting args.{k}={getattr(args, k)} with {v} "
    ...
    trainset = TTSDataset(
        audiopaths_and_text=args.dataset_files, load_mel_from_disk=False, **vars(args)
    )
    ...
    with torch.no_grad():
        for batch in tqdm(dataset_loader, "Extracting mels"):
            ...
            for apath, mel, len_ in zip(audiopaths, mel_out, mel_lens):
                np.save(
                    Path(args.output, Path(apath).stem + ".npy"),
