...
def parse_args(parser):
    ...
    parser.add_argument("input_ckpt", type=str, help="Path to the input checkpoint")
    parser.add_argument(
        "output_ckpt", default=None, help="Path to save the output checkpoint to"
    )
    parser.add_argument(
        "--cuda", action="store_true", help="Move model weights to GPU before export"
    )
    parser.add_argument(
        "--amp", action="store_true", help="Convert model to FP16 prior to saving"
    )
    parser.add_argument(
        "--load-from",
        type=str,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PyTorch model export", allow_abbrev=False
    )
    ...
    assert (
        args.load_from != args.convert_to
    ), "Load and convert formats must be different"

    print(
        f'Converting {args.model_name} from "{args.load_from}"'
        f' to "{args.convert_to}" ({device}).'
    )

    if args.load_from == "ts":
        ts_model, _ = models.load_and_setup_ts_model(
            args.model_name, args.input_ckpt, args.amp, device
        )
    else:
    ...
    if args.convert_to == "ts":
        ...
        trt_model = models.convert_ts_to_trt(
            "HiFi-GAN", ts_model, parser, args.amp, unk_args
        )
