...
def main():
    ...
    with open(args.transcripts) as f:
        ...
        transcripts = dict(
            line.replace("\ufeff", "").replace("－", "-").strip().split(" ", 1)
            for line in f
        )
    transcripts = {
        id_.replace("com_DL", "com_SF"): text.lower()
    ...
    generate(
        Path(args.output_dir, "sf_audio_text.txt"),
        {**val_ids_text, **test_ids_text, **train_ids_text},
        False,
        True,
    )
