...
def init(log_fpath, log_dir, enabled=True, tb_subsets=[], **tb_kw):
    ...
    for id_, pref in [
        ...
        dllogger.metadata(
            f"{id_}_loss", {"name": f"{pref}loss", "unit": None, "format": ":>5.2f"}
        )
        dllogger.metadata(
            f"{id_}_mel_loss",
        ...
        dllogger.metadata(
            f"{id_}_frames/s", {"name": None, "unit": "frames/s", "format": ":>10.2f"}
        )
        dllogger.metadata(
    ...
    global tb_loggers
    tb_loggers = {s: TBLogger(enabled, log_dir, name=s, **tb_kw) for s in tb_subsets}
