...
from .chinese import chinese_text_to_symbols, is_chinese, split_text
        ...
        for symbol_id in sequence:
            if symbol_id in self.id_to_symbol:
                ...
                if len(s) > 1 and s[0] == "@":
                    ...
                    len(s) > 1 and s[0] == "#" and (s[1].isalpha() or s[1] == "^")
                ):  # mandarin phoneme
                    if tmp == "":
                        tmp += " {" + s[1:] + " "
                    else:
    ...
    def encode_text(self, text, return_all=False):
        ...
        for segment in segments:
            if is_chinese(segment[0]):  # process the Chinese segment
                ...
                segment_encoded, segment_clean, segment_arpabet = super().encode_text(
                    segment, return_all=True
                )
