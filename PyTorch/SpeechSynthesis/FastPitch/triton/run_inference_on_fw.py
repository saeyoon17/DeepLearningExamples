...
from .deployment_toolkit.core import (DATALOADER_FN_NAME, BaseLoader,
                                      BaseRunner, Format, load_from_file)
...
def _parse_and_validate_args():
    ...
    if args.input_type in types_requiring_io_params and not all(
        p for p in [args.inputs, args.outputs]
    ):
        parser.error(
...
def main():
    ...
    with runner.init_inference(model=model) as runner_session, NpzWriter(
        args.output_dir
    ) as writer:
        get_dataloader_fn = load_from_file(
            args.dataloader, label="dataloader", target=DATALOADER_FN_NAME
