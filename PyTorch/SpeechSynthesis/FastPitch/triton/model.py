...
def update_argparser(parser):
    ...
    symbols.add_argument(
        "--n-symbols", default=148, type=int, help="Number of symbols in dictionary"
    )
    symbols.add_argument(
        "--padding-idx",
        default=0,
    ...
    in_fft.add_argument(
        "--in-fft-n-layers", default=6, type=int, help="Number of FFT blocks"
    )
    in_fft.add_argument(
        "--in-fft-n-heads", default=1, type=int, help="Number of attention heads"
    )
    in_fft.add_argument(
        "--in-fft-d-head", default=64, type=int, help="Dim of attention heads"
    )
    in_fft.add_argument(
        "--in-fft-conv1d-kernel-size", default=3, type=int, help="Conv-1D kernel size"
    )
    in_fft.add_argument(
        "--in-fft-conv1d-filter-size",
        default=1536,
        type=int,
        help="Conv-1D filter size",
    )
    in_fft.add_argument(
        "--in-fft-output-size", default=384, type=int, help="Output dim"
    )
    in_fft.add_argument(
        "--p-in-fft-dropout", default=0.1, type=float, help="Dropout probability"
    )
    in_fft.add_argument(
        "--p-in-fft-dropatt",
        default=0.1,
    ...
    out_fft.add_argument(
        "--out-fft-n-layers", default=6, type=int, help="Number of FFT blocks"
    )
    out_fft.add_argument(
        "--out-fft-n-heads", default=1, type=int, help="Number of attention heads"
    )
    out_fft.add_argument(
        "--out-fft-d-head", default=64, type=int, help="Dim of attention head"
    )
    out_fft.add_argument(
        "--out-fft-conv1d-kernel-size", default=3, type=int, help="Conv-1D kernel size"
    )
    out_fft.add_argument(
        "--out-fft-conv1d-filter-size",
        default=1536,
        type=int,
        help="Conv-1D filter size",
    )
    out_fft.add_argument(
        "--out-fft-output-size", default=384, type=int, help="Output dim"
    )
    out_fft.add_argument(
        "--p-out-fft-dropout",
        default=0.1,
    ...
    energy_pred.add_argument(
        "--energy-predictor-kernel-size",
        default=3,
        type=int,
        help="Pitch predictor conv-1D kernel size",
    )
    energy_pred.add_argument(
        "--energy-predictor-filter-size",
        default=256,
        type=int,
        help="Pitch predictor conv-1D filter size",
    )
    energy_pred.add_argument(
        "--p-energy-predictor-dropout",
        default=0.1,
        type=float,
        help="Pitch probability for energy predictor",
    )
    energy_pred.add_argument(
        "--energy-predictor-n-layers",
        default=2,
        type=int,
        help="Number of conv-1D layers",
    )
    ...
    parser.add_argument(
        "--checkpoint", type=str, help="Full path to the FastPitch checkpoint file"
    )
...
(if saved in checkpoints)",
    ...
    cond.add_argument(
        "--energy-embedding-kernel-size",
        default=3,
        type=int,
        help="Pitch embedding conv-1D kernel size",
    )
    cond.add_argument(
        "--speaker-emb-weight", type=float, default=1.0, help="Scale speaker embedding"
    )
    cond.add_argument(
        "--n-speakers", type=int, default=1, help="Number of speakers in the model."
    )
    cond.add_argument(
        "--pitch-conditioning-formants",
        default=1,
        type=int,
    ...
    parser.add_argument(
        "--output-format", type=str, required=True, help="Output format"
    )
...
def get_model(**model_args):
    ...
    tensor_names = {
        "inputs": ["INPUT__0"],
        "outputs": ["OUTPUT__0", "OUTPUT__1", "OUTPUT__2", "OUTPUT__3", "OUTPUT__4"],
    }
