...
def parse_args(parser):
    ...
    parser.add_argument(
        "--log-file", type=str, default=None, help="Path to a DLLogger log file"
    )
    parser.add_argument(
        "--save-mels", action="store_true", help="Save generator outputs to disk"
    )
    parser.add_argument(
        "--cuda", action="store_true", help="Run inference on a GPU using CUDA"
    )
    parser.add_argument(
        "--cudnn-benchmark", action="store_true", help="Enable cudnn benchmark mode"
    )
    parser.add_argument(
        "--l2-promote",
        action="store_true",
    ...
    parser.add_argument(
        "-s", "--waveglow-sigma-infer", default=0.9, type=float, help="WaveGlow sigma"
    )
    parser.add_argument(
        "--hifigan",
        type=str,
    ...
    parser.add_argument(
        "--repeats", type=int, default=1, help="Repeat inference for benchmarking"
    )
    parser.add_argument(
        "--torchscript",
        action="store_true",
    ...
    parser.add_argument(
        "--report-mel-loss", action="store_true", help="Report mel loss in metrics"
    )
    parser.add_argument(
        "--ema",
        action="store_true",
    ...
    parser.add_argument(
        "--speaker", type=int, default=0, help="Speaker ID for a multi-speaker model"
    )
    ...
    transf.add_argument(
        "--fade-out", type=int, default=10, help="Number of fadeout frames at the end"
    )
    transf.add_argument(
        "--pace", type=float, default=1.0, help="Adjust the pace of speech"
    )
    transf.add_argument(
        "--pitch-transform-flatten", action="store_true", help="Flatten the pitch"
    )
    transf.add_argument(
    ...
    transf.add_argument(
        "--pitch-transform-shift",
        type=float,
        default=0.0,
        help="Raise/lower the pitch by <hz>",
    )
    transf.add_argument(
        "--pitch-transform-custom",
        action="store_true",
    ...
    txt.add_argument(
        "--heteronyms-path", type=str, default="cmudict/heteronyms", help=""
    )
    txt.add_argument(
        "--cmudict-path", type=str, default="cmudict/cmudict-0.7b", help=""
    )
    return parser
...
def main():
    ...
    assert (
        args.waveglow is None or args.hifigan is None
    ), "Specify a single vocoder model"

    def _load_pyt_or_ts_model(model_name, ckpt_path):
        if args.checkpoint_format == "ts":
            model = models.load_and_setup_ts_model(
                model_name, ckpt_path, args.amp, device
            )
            model_train_setup = {}
    ...
    if args.fastpitch is not None:
        gen_name = "fastpitch"
        generator, gen_train_setup = _load_pyt_or_ts_model("FastPitch", args.fastpitch)
    ...
    if args.waveglow is not None:
        ...
        if args.denoising_strength > 0.0:
            denoiser = Denoiser(vocoder, sigma=0.0, win_length=args.win_length).to(
                device
            )
        ...
        if args.torch_tensorrt:
            vocoder = models.convert_ts_to_trt(
                "HiFi-GAN", vocoder, parser, args.amp, unk_args
            )
    ...
    for k in CHECKPOINT_SPECIFIC_ARGS:
        ...
        assert (
            v1 is None or v2 is None or v1 == v2
        ...
        if val and getattr(args, k) != val:
            ...
            print(
                f"Overwriting args.{k}={getattr(args, k)} with {val} "
    ...
    if is_ts_based_infer and generator is not None:
        ...
        print(
            "Note: --pitch-transform-* args are disabled with TorchScript. "
                ...
                if args.output is not None and reps == 1:
                    for i, audio in enumerate(audios):
                        ...
                        fname = (
                            b["output"][i]
                            if "output" in b
                            else f"audio_{all_utterances + i}.wav"
                        )
    ...
    if generator is not None:
        ...
        log(
            (),
            {
                f"90%_{gen_name}_latency": gm.mean()
                + norm.ppf((1.0 + 0.90) / 2) * gm.std()
            },
        )
        log(
            (),
            {
                f"95%_{gen_name}_latency": gm.mean()
                + norm.ppf((1.0 + 0.95) / 2) * gm.std()
            },
        )
        log(
            (),
            {
                f"99%_{gen_name}_latency": gm.mean()
                + norm.ppf((1.0 + 0.99) / 2) * gm.std()
            },
        )
        if args.report_mel_loss:
    ...
    if vocoder is not None:
        ...
        log(
            (),
            {
                f"90%_{voc_name}_latency": vm.mean()
                + norm.ppf((1.0 + 0.90) / 2) * vm.std()
            },
        )
        log(
            (),
            {
                f"95%_{voc_name}_latency": vm.mean()
                + norm.ppf((1.0 + 0.95) / 2) * vm.std()
            },
        )
        log(
            (),
            {
                f"99%_{voc_name}_latency": vm.mean()
                + norm.ppf((1.0 + 0.99) / 2) * vm.std()
            },
        )
        if args.report_mel_loss:
