...
def apply_ema(model, ema_model, decay, patch_conv_wn=False):
    ...
    if patch_conv_wn:
        torch.nn.utils.weight_norm(model.encoder.pos_conv[0], name="weight", dim=2)
...
def gather_predictions(predictions_list, labels, blank_id=None):
    ...
    for prediction in predictions_list:
        results += ctc_decoder_predictions_tensor(
            prediction, labels=labels, blank_id=blank_id
        )
    return results
...
def process_evaluation_epoch(aggregates):
    ...
    if multi_gpu:
        ...
        result_tensor = torch.cat([scores_tensor, num_words_tensor, ids_tensor], dim=-1)
        result_tensor_list = [
            torch.zeros_like(result_tensor) for i in range(dist.get_world_size())
        ]
...
class Checkpointer:
    def __init__(self, args, model_name):
        ...
        tracked = [
            (int(re.search("update(\d+)\.pt", str(f)).group(1)), f)
    ...
    def maybe_load_state(
        ...
        if optimizer is not None:
            if "last_optimizer_state" in self.last_state:
                optimizer.load_state_dict(self.last_state["last_optimizer_state"])
