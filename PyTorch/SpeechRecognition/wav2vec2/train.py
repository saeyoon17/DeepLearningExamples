...
from common.helpers import (Checkpointer, apply_multi_tensor_ema,
                            init_multi_tensor_ema, num_weights, to_gpu)
...
from wav2vec2.criterion import CTCCriterion, Wav2vecCriterion
from wav2vec2.logging import W2v2FineTuningMetrics, W2v2Metrics, init_logger
...
def validate(
    ...
    val_losses = []
    val_wer = []
    for model, metrics, scope in [
        (model, val_metrics, "val"),
        ...
        logger.log(
            () if epoch is None else (epoch,), metrics, scope=scope, tb_iter=step
        )
...
def main():
    parser = argparse.ArgumentParser(description="wav2vec 2.0 Deep Learning Example")
    wav2vec2.arg_parser.populate(parser)
    args = parser.parse_args()

    assert (
        not args.bf16 or args.fp32_pos_conv
    ...
    if not pre_training:
        assert (
            args.labels or checkpointer.last_state
    ...
    checkpointer.maybe_load_state(
        ema_model=ema_model, optimizer=optim, scaler=scaler, train_state=train_state
    )
    ...
    while step < args.max_update:  # training loop
        ...
        for batch, accum_batches in zip(itr, cycle(range(args.update_freq))):
            ...
            if (accum_batches + 1) % args.update_freq == 0:
                ...
                try:
                    ...
                    if args.optimizer == "adam" and not (args.fp16 or args.bf16):
                        scaler.step(optim, scale=1.0 / grads_mult_factor)
                    else:
                        scaler.step(optim)
                ...
                if step % args.log_frequency == 0:
                    ...
                    logger.log(
                        (epoch, epoch_step, steps_per_epoch),
                        metrics,
                        scope="train",
                        tb_iter=step,
                    )
        ...
        logger.log((epoch,), metrics, scope="train_avg", flush_log=True, tb_iter=step)
