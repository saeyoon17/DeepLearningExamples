...
def init_train_metadata():
    for id_, pref in [
        ...
        dllogger.metadata(f"{id_}_loss", {"name": f"{pref} loss", "format": ":>6.3f"})

        dllogger.metadata(f"{id_}_accuracy", {"name": f"{pref}acc", "format": ":>6.3f"})

        dllogger.metadata(
            f"{id_}_prob_perplexity", {"name": f"{pref}p pplx", "format": ":>6.3f"}
        )

        dllogger.metadata(
            f"{id_}_code_perplexity", {"name": f"{pref}c pplx", "format": ":>6.3f"}
        )

        dllogger.metadata(
            f"{id_}_ntokens", {"name": None, "unit": "tokens", "format": ":>8.0f"}
        )
        ...
        dllogger.metadata(
            f"{id_}_ntokens/s", {"name": None, "unit": "tokens/s", "format": ":>8.2f"}
        )

        dllogger.metadata(f"{id_}_uer", {"name": f"{pref} uer", "format": ":>6.2f"})

        dllogger.metadata(f"{id_}_wer", {"name": f"{pref} wer", "format": ":>6.2f"})

        dllogger.metadata(
            f"{id_}_raw_wer", {"name": f"{pref} raw wer", "format": ":>6.2f"}
        )

        dllogger.metadata(f"{id_}_lr", {"name": "lr", "format": ":>3.2e"})

        dllogger.metadata(
            f"{id_}_loss_scale", {"name": "loss scale", "format": ":>3.2e"}
        )
...
class W2v2Metrics(MetricsAggregator):
    ...
    def accumulate(self, scopes=None):
        if "ignore" not in self.partials or self.partials["ignore"] == 0.0:
            ...
            self["accuracy"] = self.partials.pop("correct") / self.partials.pop("count")
