...
def fp_convert_batch(batch, precision):
    ...
    dt = {"fp32": torch.float32, "fp16": torch.half, "bf16": torch.bfloat16}[precision]
...
def main():
    ...
    if is_nv_ckpt:
        ...
        args.fp32_pos_conv = train_args.get("fp32_pos_conv", args.fp16 or args.bf16)
    ...
    if (train_args.get("fp16", False) or train_args.get("amp", False)) and args.bf16:
    ...
    if args.transcribe_wav or args.transcribe_filelist:
        ...
        if args.transcribe_wav:
            dataset = wav2vec2.utils.single_audio_dataset(args.transcribe_wav, args)
        else:
            dataset = wav2vec2.utils.load_dataset(
                args.transcribe_filelist, args, target_dictionary
            )
        ...
        dataset = wav2vec2.utils.load_dataset(
            args.valid_subset, args, target_dictionary, with_labels=True
        )
        sampler = (
    ...
    for it, batch in enumerate(pbar):
        ...
        with torch.no_grad():
            enc_out, padding_mask = model(
                batch["net_input"]["source"], batch["net_input"]["padding_mask"]
            )
            logp = model.get_normalized_probs(
                enc_out, padding_mask, log_probs=True
            ).contiguous()
            # greedy decoding
        ...
        agg["preds"] += gather_predictions([preds], target_dictionary, blank_id=0)
        agg["logits"].append(logp)

        if "target" in batch:
            agg["txts"] += gather_transcripts(
                [batch["target"]], [batch["target_lengths"]], target_dictionary
            )
        if multi_gpu:
    ...
    agg["preds"] = [pred.replace(tdict[tdict.nspecial], " ") for pred in agg["preds"]]
    agg["txts"] = [txt.replace(tdict[tdict.nspecial], " ") for txt in agg["txts"]]
