...
from .model import JasperBlock, JasperDecoderForCTC, JasperEncoder
...
def input(conf_yaml, split="train"):
    ...
    conf_dataset = validate_and_fill(
        AudioDataset, conf_dataset, optional=["data_dir", "labels", "manifest_fpaths"]
    )

    conf_features = validate_and_fill(FilterbankFeatures, conf_features)
...
def encoder(conf):
    ...
    for blk in conf["jasper"]["encoder"]["blocks"]:
        validate_and_fill(
            JasperBlock, blk, optional=["infilters"], ignore_unk=["residual_dense"]
        )
