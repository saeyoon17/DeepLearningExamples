...
def audio_generator_from_file(input_filename, target_sr, int_values, chunk_duration):
    ...
    while not end:
        ...
        audio_signal, end = get_audio_chunk_from_soundfile(sf, chunk_size, int_values)
    ...
    def __init__(self, input_device_id, target_sr, chunk_duration):
        ...
        for i in range(0, num_devices):
            if (
                ...
                devices[i] = self.p.get_device_info_by_host_api_device_index(0, i)

        if len(devices) == 0:
            raise RuntimeError("Cannot find any valid input devices")

        if input_device_id is None or input_device_id not in devices.keys():
            print("\nInput Devices:")
            ...
            try:
                if self.recording_state == "capture":
                    ...
                    audio_segment = AudioSegment(
                        audio_signal, self.target_sr, self.target_sr
                    )
...
def audio_features_generator(
    input_filename, speech_features_params, target_sr, int_values, chunk_duration
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_platform", required=False, default="trt", help="Jasper model platform"
    )
    parser.add_argument(
        "-u",
        "--url",
    ...
    parser.add_argument(
        "--data_dir", type=str, required=False, default=None, help="data directory"
    )
    parser.add_argument(
        "--manifest_filename",
        type=str,
    ...
    if FLAGS.model_platform not in valid_model_platforms:
        raise ValueError(
            "Invalid model_platform {}. Valid choices are {"
            "}".format(FLAGS.model_platform, valid_model_platforms)
        )
    ...
    speech_client = SpeechClient(
        ...
        1,
        FLAGS.batch_size,
        model_platform=FLAGS.model_platform,
    ...
    if FLAGS.audio_filename is not None:
        ...
        if os.path.isdir(audio_file):
            filenames = glob.glob(
                os.path.join(os.path.abspath(audio_file), "**", "*.wav"), recursive=True
            )
        else:
    ...
    while not last_request:
        ...
        for idx in range(FLAGS.batch_size):
            ...
            audio = AudioSegment.from_file(
                filename, offset=0, duration=FLAGS.fixed_size
            ).samples
            if FLAGS.fixed_size:
        ...
        predictions += speech_client.recognize(batch_audio_samples, batch_filenames)
