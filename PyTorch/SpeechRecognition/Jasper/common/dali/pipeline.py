...
class DaliPipeline:
        ...
        *,
        ...
        def dali_jasper_pipe():
            if is_triton_pipeline:
                assert (
                    not self.train
                ), "Pipeline for Triton shall be a validation pipeline"
                if torch.distributed.is_initialized():
                    raise RuntimeError(
                        "You're creating Triton pipeline, using multi-process mode. Please use single-process mode."
                    )
                encoded, label = fn.external_source(
                    device="cpu", name="DALI_INPUT_0", no_copy=True
                )
            else:
            ...
            mel_spec = fn.mel_filter_bank(
                spec, sample_rate=sample_rate, nfilter=self.nfeatures, normalize=True
            )
            ...
            if self.frame_splicing_factor != 1:
                log_features_len = self._div_ceil(
                    log_features_len, self.frame_splicing_factor
                )
        ...
        self.pipe_handle = dali_jasper_pipe(
    ...
    def _do_spectrogram_masking(self):
        return (
            self.mask_params["time_num_regions"] > 0
            or self.mask_params["freq_num_regions"] > 0
            or self.mask_params["both_num_regions"] > 0
        )
    ...
    def _generate_cutouts(self):
        ...
        shapes.extend(
            self._interleave_lists(
                np.random.randint(
                    self.mask_params["freq_min"],
                    self.mask_params["freq_max"] + 1,
        ...
        shapes.extend(
            self._interleave_lists(
                ...
                np.random.randint(
                    self.mask_params["time_min"],
                    self.mask_params["time_max"] + 1,
        ...
        shapes.extend(
            self._interleave_lists(
                np.random.randint(
                    self.mask_params["both_min_freq"],
                    self.mask_params["both_max_freq"] + 1,
                    self.mask_params["both_num_regions"],
                ),
                np.random.randint(
                    self.mask_params["both_min_time"],
                    self.mask_params["both_max_time"] + 1,
                    self.mask_params["both_num_regions"],
                ),
            )
        )
    ...
    def _cutouts_generator(self):
        ...
        [anchors, shapes] = tuples2list(
            [self._generate_cutouts() for _ in range(self.pipe_handle.max_batch_size)]
        )
...
def serialize_dali_triton_pipeline(
    output_path: str, config_data: dict, config_features: dict
):
    pipe = DaliTritonPipeline.from_config(
