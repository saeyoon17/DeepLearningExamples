...
from common.optimizers import AdamW, Novograd, lr_policy
...
def parse_args():
    ...
    optim.add_argument("--lr", default=1e-3, type=float, help="Peak learning rate")
    optim.add_argument(
    ...
    optim.add_argument(
        "--multi_tensor_ema", action="store_true", help="Use multi_tensor_apply for EMA"
    )
...
def main():
    ...
    if use_dali:
        assert train_dataset_kw[
            "ignore_offline_speed_perturbation"
        ...
        train_dataset = AudioDataset(
            args.dataset_dir, args.train_manifests, symbols, **train_dataset_kw
        )
        train_loader = get_data_loader(
            train_dataset, batch_size, multi_gpu=multi_gpu, shuffle=True, num_workers=4
        )
        train_feat_proc = FilterbankFeatures(**train_features_kw)

        val_dataset_kw, val_features_kw = config.input(cfg, "val")
        val_dataset = AudioDataset(
            args.dataset_dir, args.val_manifests, symbols, **val_dataset_kw
        )
        ...
        print_once(
            f"Training samples: {nsampl} ({dur:.1f}h, " f"filtered {dur_f:.1f}h)"
        )
    ...
    if args.optimizer == "novograd":
        ...
        optimizer = FusedLAMB(model.parameters(), betas=(0.9, 0.98), eps=1e-9, **kw)
    ...
    checkpointer = Checkpointer(args.output_dir, "QuartzNet", args.keep_milestones)
    if args.resume:
    ...
    if args.pre_allocate_range is not None:
        ...
        for n_frames in range(a, b + pad_align, pad_align):
            ...
            txt = torch.randint(
                high=len(symbols) - 1, size=(batch_size, 100), device="cuda"
            )
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
            ...
            if accumulated_batches % args.grad_accumulation == 0:
                ...
                if step % args.log_frequency == 0:
                    ...
                    log(
                        (
                            epoch,
                            step % steps_per_epoch or steps_per_epoch,
                            steps_per_epoch,
                        ),
                        step,
                        "train",
        ...
        if epoch % args.save_frequency == 0 or epoch in args.keep_milestones:
            checkpointer.save(
                model, ema_model, optimizer, scaler, epoch, step, best_wer
            )
    ...
    if epoch == args.epochs:
        checkpointer.save(model, ema_model, optimizer, scaler, epoch, step, best_wer)
    flush_log()
