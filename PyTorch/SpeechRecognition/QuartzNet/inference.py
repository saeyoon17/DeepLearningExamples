...
from common.dataset import (AudioDataset, FilelistDataset, SingleAudioDataset,
                            get_data_loader)
...
def get_parser():
    ...
    parser.add_argument(
        "--dataset_dir", type=str, help="Absolute path to dataset folder"
    )
    parser.add_argument(
        "--val_manifests",
        type=str,
    ...
    parser.add_argument(
        "--ckpt", default=None, type=str, help="Path to model checkpoint"
    )
    parser.add_argument(
        "--amp", "--fp16", action="store_true", help="Use FP16 precision"
    )
    parser.add_argument(
    ...
    io.add_argument("--ema", action="store_true", help="Load averaged model weights")
    io.add_argument(
...
def main():
    ...
    for step in ["DNN", "data+DNN", "data"]:
        for c in [0.99, 0.95, 0.9, 0.5]:
            ...
            dllogger.metadata(
                f"{step.lower()}_latency_{c}",
                {"name": f"{step} latency {cs}", "format": ":>7.2f", "unit": "ms"},
            )
    dllogger.metadata("eval_wer", {"name": "WER", "format": ":>3.2f", "unit": "%"})
    ...
    if args.transcribe_wav or args.transcribe_filelist:
        ...
        dataset = AudioDataset(
            args.dataset_dir, args.val_manifests, symbols, **dataset_kw
        )
    ...
    if args.torchscript:
        ...
        feat_proc, model, greedy_decoder = torchscript_export(
            ...
            use_amp=args.amp,
            use_conv_masks=True,
            model_toml=args.model_toml,
            device=device,
            save=args.torchscript_export,
        )
    ...
    with torch.no_grad():
        ...
        for it, batch in enumerate(tqdm(looped_loader, initial=1, total=steps)):
            ...
            if txt is not None:
                agg["txts"] += helpers.gather_transcripts([txt], [txt_lens], symbols)
