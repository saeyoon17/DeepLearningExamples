...
from ssd.data import (get_coco_ground_truth, get_train_loader,
                      get_val_dataloader, get_val_dataset)
from ssd.evaluate import evaluate
from ssd.logger import BenchLogger, Logger
from ssd.model import SSD300, Loss, ResNet
from ssd.train import (benchmark_inference_loop, benchmark_train_loop,
                       load_checkpoint, tencent_trick, train_loop)
...
def train(train_loop_func, logger, args):
    ...
    if args.checkpoint is not None:
        if os.path.isfile(args.checkpoint):
            load_checkpoint(
                ssd300.module if args.distributed else ssd300, args.checkpoint
            )
