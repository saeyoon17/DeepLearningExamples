...
class COCOPipeline(Pipeline):
    def __init__(
        ...
        self.input = dali.ops.readers.COCO(
            file_root=file_root,
        ...
        self.decode_slice = dali.ops.decoders.ImageSlice(
        ...
        self.hsv = dali.ops.Hsv(
            device="gpu", dtype=dali.types.FLOAT
        )  # use float to avoid clipping and quantizing the intermediate result
        self.bc = dali.ops.BrightnessContrast(
            device="gpu",
        ...
        # Resize
        self.resize = dali.ops.Resize(device="cpu", resize_x=300, resize_y=300)
        ...
        self.box_encoder = dali.ops.BoxEncoder(
            device="cpu", criteria=0.5, anchors=self.anchors
        )
...
def feed_ndarray(dali_tensor, arr):
    ...
    assert dali_tensor.shape() == list(arr.size()), (
        ...
        ", but PyTorch Tensor has size {1}".format(
