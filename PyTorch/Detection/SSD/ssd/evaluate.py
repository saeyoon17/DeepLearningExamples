        ...
        with torch.no_grad():
            ...
            for idx in range(ploc.shape[0]):
                ...
                try:
                    ...
                    print(
                        "Skipping idx {}, failed to decode with message {}, Skipping.".format(
                            idx, e
                        )
                    )
                    continue
    ...
    if args.distributed:
        ...
        ret_pad = torch.cat(
            [
                ret_copy,
                torch.zeros(
                    max_size - ret_copy.shape[0], 7, dtype=torch.float32
                ).cuda(),
            ]
        )
        ...
        other_ret = [
            torch.zeros(max_size, 7, dtype=torch.float32).cuda() for i in range(N_gpu)
        ]
