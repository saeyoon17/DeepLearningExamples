...
from utils.optimizers import clip_grad_norm_2, create_optimizer
...
# Misc
parser.add_argument(
    "--dist-group-size", type=int, default=0, help="Group size for sync-bn"
)
parser.add_argument(
...
parser.add_argument(
    "--freeze-layers", nargs="*", default=[], help="Freeze these layers"
)
parser.add_argument(
    "--waymo-train-annotation",
    default=None,
    type=str,
    help='Absolute Path to waymo training annotation (default: "None")',
)
parser.add_argument(
    "--waymo-val-annotation",
    default=None,
    type=str,
...
def main():
    ...
    if args.waymo:
        if (args.waymo_train is not None and args.waymo_val is None) or (
            args.waymo_train is None and args.waymo_val is not None
        ):
    ...
    if args.resume and resume_checkpoint_path is not None:
        ...
        resume_state, resume_epoch = resume_checkpoint(
            unwrap_bench(model), resume_checkpoint_path
        )
    ...
    if args.model_ema:
        ...
        if args.resume and resume_checkpoint_path is not None:
            ...
            resume_path = ""
        model_ema = ModelEma(model, decay=args.model_ema_decay, resume=resume_path)
    ...
    if args.local_rank == 0:
        dllogger.log(
            step="PARAMETER", data={"Scheduled_epochs": num_epochs}, verbosity=0
        )
    ...
    if args.waymo:
        ...
        train_annotation_path = os.path.join(
            args.data, "annotations", f"instances_{train_anno_set}.json"
        )
        train_image_dir = train_anno_set
    dataset_train = CocoDetection(
        os.path.join(args.data, train_image_dir), train_annotation_path, data_config
    )
    ...
    steps_per_epoch = int(
        np.ceil(len(dataset_train) / (args.world_size * args.batch_size))
    )
    ...
    if args.waymo:
        ...
        val_annotation_path = os.path.join(
            args.data, "annotations", f"instances_{val_anno_set}.json"
        )
        val_image_dir = val_anno_set
    dataset_eval = CocoDetection(
        os.path.join(args.data, val_image_dir), val_annotation_path, data_config
    )
    ...
    evaluator = COCOEvaluator(
        dataset_eval.coco, distributed=args.distributed, waymo=args.waymo
    )
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if (
                saver is not None
                and args.rank == 0
                and epoch % args.save_checkpoint_interval == 0
            ):
                if eval_metrics is not None:
                    # save proper checkpoint with eval metric
                    is_best = eval_metrics[eval_metric] > best_metric
                    best_metric = max(eval_metrics[eval_metric], best_metric)
                    best_epoch = epoch
                else:
                    is_best = False
                    best_metric = 0
...
def train_epoch(
    ...
    for batch_idx in range(steps_per_epoch):
        ...
        if last_batch or (batch_idx + 1) % args.log_interval == 0:
            ...
            if args.rank == 0:
                dllogger_data = {
                    "train_batch_time": batch_time_m.avg,
    ...
    metrics = {
        "train_loss": losses_m.avg,
        "train_batch_time": batch_time_m.avg,
        "train_throughout": throughput_m.avg,
    }
...
def validate(model, loader, args, evaluator=None, epoch=0, log_suffix=""):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            batch_time_m.update(time.time() - end)
            throughput_m.update(
                float(input.size(0) * args.world_size / batch_time_m.val)
            )
            end = time.time()
            if args.rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                log_name = "Test" + log_suffix
                dllogger_data = {
                    "eval_batch_time": batch_time_m.val,
                    "eval_loss": losses_m.val,
                }
                dllogger.log(
                    step=(epoch, last_idx, batch_idx), data=dllogger_data, verbosity=0
                )

    metrics = {
        "eval_batch_time": batch_time_m.avg,
        "eval_throughput": throughput_m.avg,
