...
def focal_loss(logits, targets, alpha: float, gamma: float, normalizer):
    ...
    modulator = torch.exp(
        gamma * targets * neg_logits - gamma * torch.log1p(torch.exp(neg_logits))
    )
...
def _classification_loss(
    ...
    classification_loss = new_focal_loss(
        cls_outputs, cls_targets, alpha, gamma, normalizer, label_smoothing
    )
...
def _box_loss(box_outputs, box_targets, num_positives, delta: float = 0.1):
    ...
    box_loss = huber_loss(
        box_outputs, box_targets, weights=mask, delta=delta, size_average=False
    )
    box_loss /= normalizer
    return box_loss
    ...
    def forward(
        ...
        for l in range(levels):
            ...
            if self.fused_focal_loss:
                ...
                cls_targets_at_level_oh = one_hot(
                    cls_targets_at_level, self.num_classes
                )

                bs, height, width, _, _ = cls_targets_at_level_oh.shape
                cls_targets_at_level_oh = cls_targets_at_level_oh.view(
                    bs, height, width, -1
                )
