...
from utils.utils import AverageMeter, setup_default_logging, setup_dllogger
    ...
    if args.waymo:
        ...
        if "test" in args.anno:
            annotation_path = os.path.join(
                args.data, "annotations", f"image_info_{args.anno}.json"
            )
            image_dir = "test2017"
        else:
            annotation_path = os.path.join(
                args.data, "annotations", f"instances_{args.anno}.json"
            )
            image_dir = args.anno
    dataset = CocoDetection(
        os.path.join(args.data, image_dir), annotation_path, data_config
    )

    evaluator = COCOEvaluator(
        dataset.coco, distributed=args.distributed, waymo=args.waymo
    )
