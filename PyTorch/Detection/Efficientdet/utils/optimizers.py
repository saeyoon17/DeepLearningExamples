...
def clip_grad_norm_2(parameters: _tensor_or_tensors, max_norm: float):
    ...
    total_norm, _ = multi_tensor_applier(
        multi_tensor_l2norm, dummy_overflow_buf, [grads], False
    )
...
def create_optimizer(args, model, filter_bias_and_bn=True):
    ...
    opt_args = dict(lr=args.lr, weight_decay=weight_decay)
    if (
        hasattr(args, "opt_eps")
        and args.opt_eps is not None
        and opt_lower not in ["sgd", "momentum", "fusedmomentum", "fusedsgd"]
    ):
