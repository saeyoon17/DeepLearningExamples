...
def get_latest_file(files):
    ...
    max_checkpoint_number = max(
        [int(f.split("checkpoint_")[1].split(".")[0]) for f in files]
    )  # checkpoint_name_convention = checkpoint_ + number + .pth.tar
...
class CheckpointSaver:
    def __init__(self, args=None, checkpoint_dir="", unwrap_fn=unwrap_model):
    ...
    def save_checkpoint(
        ...
        actual_save_path = os.path.join(
            self.checkpoint_dir, "checkpoint_" + str(epoch) + self.extension
        )
        self._save(
            model,
            optimizer,
...
def load_checkpoint(
    model, checkpoint_path, use_ema=False, strict=True, remove_params=[]
):
    ...
    state_dict, checkpoint = load_state_dict(
        checkpoint_path, has_module, use_ema, remove_params
    )
