...
def parse_args():
    ...
    parser.add_argument(
        "--no_membind", action="store_true", help="Flag to disable memory binding"
    )
...
def main():
    ...
    ), "ERROR: launch {} processes, but " "there are only {} gpus available".format(
        args.nproc_per_node, num_gpus
    )
    ...
    print(
        "num_sockets = {} num_nodes={} cores_per_socket={}".format(
    ...
    for local_rank in range(0, args.nproc_per_node):
        ...
        cpu_ranges = [
            ...
            (local_rank + 1) * cores_per_gpu
            + (cores_per_gpu * gpus_per_node * num_nodes)
            - 1,
        ]
