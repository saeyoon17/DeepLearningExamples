...
def parse_args_and_arch(parser, input_args=None, parse_known=False):
    ...
    if (
        hasattr(args, "target_bleu")
        and (args.online_eval or args.target_bleu)
        and not args.remove_bpe
    ):
        args.remove_bpe = "@@ "
...
def add_optimization_args(parser):
    ...
    group.add_argument(
        ...
        help="optimizer: {} (default: nag)".format(
            ", ".join(OPTIMIZER_REGISTRY.keys())
        ),
    )
    group.add_argument(
        "--lr",
    ...
    group.add_argument(
        "--momentum", default=0.99, type=float, metavar="M", help="momentum factor"
    )
    group.add_argument(
        "--weight-decay",
        "--wd",
    ...
    group.add_argument(
        "--min-lr", default=1e-5, type=float, metavar="LR", help="minimum learning rate"
    )
...
def add_checkpoint_args(parser):
    ...
    group.add_argument(
        "--no-save", action="store_true", help="don't save models or checkpoints"
    )
    group.add_argument(
...
def add_common_eval_args(group):
    group.add_argument(
        "--path", metavar="FILE", help="path(s) to model file(s), colon separated"
    )
    group.add_argument(
        "--file",
        metavar="FILE",
...
def add_inference_args(parser):
    ...
    group.add_argument("--beam", default=4, type=int, metavar="N", help="beam size")
    group.add_argument(
        "--nbest",
        default=1,
        type=int,
    ...
    group.add_argument(
        "--online-eval", action="store_true", help="score model at the end of epoch"
    )
    group.add_argument(
        "--save-predictions",
        action="store_true",
    ...
    group.add_argument(
        "--test-cased-bleu", action="store_true", help="Use cased bleu for online eval"
    )
    group.add_argument(
        "--bpe-codes",
        default=None,
...
def add_perf_args(parser):
    ...
    group.add_argument(
        "--fuse-relu-dropout", action="store_true", help="Fuse Relu and Dropout."
    )
    group.add_argument(
        "--fuse-layer-norm",
        action="store_true",
