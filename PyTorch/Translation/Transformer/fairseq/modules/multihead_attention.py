...
def query_linear(input: Tensor, weights_q: Tensor, scale: float):
    if not torch.jit.is_scripting():
        ...
        q = scale * torch.einsum(
            "ij,jk->ik", input.view(input.size(0) * input.size(1), -1), weights_q
        )
...
def key_value_linears(input: Tensor, weights_k: Tensor, weights_v: Tensor):
    if not torch.jit.is_scripting():
        ...
        k = torch.einsum(
            "ij,jk->ik", input.view(input.size(0) * input.size(1), -1), weights_k
        )
        k = k.view(input.shape)
        v = torch.einsum(
            "ij,jk->ik", input.view(input.size(0) * input.size(1), -1), weights_v
        )
        v = v.view(input.shape)
        return k, v


def self_attn_linears(
    input: Tensor, weights_q: Tensor, weights_k: Tensor, weights_v: Tensor, scale: float
):
    if not torch.jit.is_scripting():
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if qkv_same:
            ...
            if not (
                saved_state is not None and "prev_key" in saved_state and static_kv
            ):
    ...
    def buffered_mask(self, tensor):
        ...
        if self._mask.size(0) == 0:
            ...
            self._mask = torch.triu(
                utils.fill_with_neg_inf(tensor.new_empty(dim, dim)), 1
            )
        if self._mask.size(0) < dim:
            self._mask = torch.triu(
                utils.fill_with_neg_inf(self._mask.resize_(dim, dim)), 1
            )
    ...
    def _get_input_buffer(
        self, incremental_state: Optional[Dict[str, Dict[str, Tensor]]]
    ):
