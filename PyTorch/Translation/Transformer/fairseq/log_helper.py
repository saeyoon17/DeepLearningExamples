...
class AggregatorBackend(Backend):
    def __init__(self, verbosity, agg_dict):
        ...
        self.metrics = OrderedDict(
            {k: [METRIC[x]() for x in v] for k, v in agg_dict.items()}
        )
    ...
    def flush(self):
        if self.metrics.flushed:
            return
        result_string = "Transformer | epoch {} | step {} |".format(
...
def setup_logger(args):
    ...
    dllogger.metadata(
        "speed",
        {"unit": "tokens/s", "format": ":.3f", "GOAL": "MAXIMIZE", "STAGE": "TRAIN"},
    )
    dllogger.metadata(
        "accuracy",
        {"unit": "bleu", "format": ":.2f", "GOAL": "MAXIMIZE", "STAGE": "VAL"},
    )
