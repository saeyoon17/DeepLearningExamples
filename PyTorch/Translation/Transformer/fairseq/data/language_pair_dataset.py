...
class LanguagePairDataset(Dataset):
    ...
    def _get_max_positions(self, max_positions):
        ...
        return min(self.max_source_positions, max_src_pos), min(
            self.max_target_positions, max_tgt_pos
        )
        ...
        if split_exists(split_k, src, tgt, src):
            ...
            if k > 0:
                ...
                raise FileNotFoundError(
                    "Dataset not found: {} ({})".format(split, args.data)
                )
    ...
    for split in splits:
        if split == "train":
            ...
            for k in itertools.count():
                ...
                try:
                    load_dataset(
                        args, datasets, split_k, src_dict, tgt_dict, combine=False
                    )
