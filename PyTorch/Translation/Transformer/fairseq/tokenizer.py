...
path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "prefixes/nonbreaking_prefix.en"
)
prefixes = {}
    ...
    for i in range(len(words)):
        ...
        if match:
            ...
            if i == len(words) - 1:
                ...
                (re.search(r"\.", pre) and re.search(r"[^\.\W\d]", pre))
                or (pre in prefixes and prefixes[pre] == 1)
                or re.search(r"^[a-z]", words[i + 1])
                or (
                    pre in prefixes
                    and prefixes[pre] == 2
                    and re.search(r"^[0-9]+", words[i + 1])
                )
            ):
                pass
            else:
    ...
    def detokenize(line, lang):
        ...
        for i in range(len(words)):
            ...
            if re.search(r"^[\u20ac\x24\(\[\{]+$", words[i]):
                ...
                lang == "en"
                and i > 0
                and re.search(r"^[\'][\w\D]", words[i])
                and re.search(r"\w$", words[i - 1])
            ):
                line += words[i]
                prepend_space = " "
            elif (
                lang == "cs"
                and i > 1
                and re.search(r"^\d+$", words[i - 2])
                and re.search(r"^[.,]$", words[i - 1])
                and re.search(r"^\w+$", words[i])
            ):
                line += words[i]
                prepend_space = " "
            elif (
                ...
                and re.search(r"[\w\D]$", words[i])
                and re.search(r"^-$", words[i + 1])
                and re.search(r"^li$|^mail.*", words[i + 2], re.I)
            ):
                # line += ' '+words[i]+words[i+1]
                pass  # TODO: skip one word
            elif re.search(r"^[\'\"\x60\u201c\u201d]+$", words[i]):
                normalized_quo = (
                    '"' if re.search(r"^[\u201c\u201d]+$", words[i]) else words[i]
                )
                quote_count[normalized_quo] = (
                    0
                    if normalized_quo not in quote_count.keys()
                    else quote_count[normalized_quo]
                )
                ...
                and re.search(r":$", words[i - 1])
                and re.search(
                    r"^(N|n|A|a|Ä|ä|ssa|Ssa|ssä|Ssä|sta|stä|Sta|Stä|hun|Hun|hyn|Hyn|han|Han|hän|Hän|hön|Hön|un|Un|yn|Yn|an|An|än|Än|ön|Ön|seen|Seen|lla|Lla|llä|Llä|lta|Lta|ltä|Ltä|lle|Lle|ksi|Ksi|kse|Kse|tta|Tta|ine|Ine)(ni|si|mme|nne|nsa)?(ko|kö|han|hän|pa|pä|kaan|kään|kin)?$",
                    words[i],
                )
            ):
                line += words[i].lower()
