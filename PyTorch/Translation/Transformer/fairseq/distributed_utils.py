...
def distributed_init(args):
    ...
    if args.distributed_world_size > 1:
        ...
        print(
            "| distributed init (rank {}): env://".format(args.distributed_rank),
            flush=True,
        )
        print(
            f"| distributed env init. MASTER_ADDR: {os.environ['MASTER_ADDR']}, MASTER_PORT: {os.environ['MASTER_PORT']}"
        ...
        print(
            "| initialized host {} as rank {} and device id {}".format(
                socket.gethostname(), args.distributed_rank, args.local_rank
            )
        )
...
def all_gather_list(data, max_size=16384):
    ...
    if not hasattr(all_gather_list, "_in_buffer") or max_size != len(
        all_gather_list._in_buffer
    ):
