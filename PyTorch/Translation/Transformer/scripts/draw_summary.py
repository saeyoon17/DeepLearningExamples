...
def main(args):
    ...
    ax1.plot(
        steps, smoothed_loss + stdev, "--", color="orange", linewidth=0.3, label="Stdev"
    )
    ...
    if "test_bleu" in jlog:
        min_loss_test_bleu = test_bleu[
            val_steps.index(min_val_loss_step)
        ]  # BLEU score on test set when validation loss is minimal
        ax2.plot(min_val_loss_step, min_loss_test_bleu, "bo-", color=TEST_BLEU_COLOR)
        ax2.annotate(
            "{:.2f}".format(min_loss_test_bleu), (min_val_loss_step, min_loss_test_bleu)
        )

        max_val_bleu_test = test_bleu[
            val_steps.index(max_val_bleu_step)
        ]  # BLEU score on test set when BLEU score on dev set is maximal
        ax2.plot(mvb_step, max_val_bleu_test, "bo-", color=TEST_BLEU_COLOR)
        ax2.annotate(
            "{:.2f}".format(max_val_bleu_test), (max_val_bleu_step, max_val_bleu_test)
        )
    ...
    if args.dump_json:
        ...
        summary["avg_epoch_loss"] = [
            x.mean()
            for x in np.array_split(np.array(loss), jlog["parameters"]["max_epoch"])
        ]
