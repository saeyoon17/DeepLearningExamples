...
import sacrebleu
import torch
from fairseq import data, distributed_utils, options, tokenizer, utils
from fairseq.data import data_utils, load_dataset_splits
from fairseq.ddp_trainer import DDPTrainer
from fairseq.log_helper import reset_perf_meters, setup_logger
...
def main(args):
    ...
    ctypes.CDLL("libcudart.so").cudaDeviceSetLimit(
    ...
    if args.do_sanity_check:
        ...
        sanity_score = score(
            args, trainer, datasets["test"], src_dict, tgt_dict, "test.raw.de"
        )
        DLLogger.log(
            step="SANITY_CHECK", data={"sanity_check_score": sanity_score}, verbosity=1
        )
    ...
    while (
        ...
        and current_bleu < tgt_bleu
    ):
        DLLogger.log(
            step=trainer.get_num_updates() + 1,
        ...
        DLLogger.log(
            ...
            data={"walltime": train_meter.sum},
            verbosity=1,
        )
        DLLogger.log(
            step=trainer.get_num_updates(),
            data={"avg_epoch_loss": trainer.avg_loss_meter.avg},
            verbosity=1,
        )
        ...
        if epoch_itr.epoch % args.validate_interval == 0:
            ...
            DLLogger.log(
                step=trainer.get_num_updates(),
                data={"val_loss": valid_losses[0], "val_bleu": valid_bleu},
                verbosity=1,
            )
...
def train(args, trainer, epoch_itr):
    ...
    for i, sample in enumerate(itr):
        if i < num_batches - 1 and (i + 1) % update_freq > 0:
            ...
            trainer.train_step(
                sample, update_params=False, last_step=(i == len(itr) - 1)
            )
            continue
        else:
            trainer.train_step(
                sample, update_params=True, last_step=(i == len(itr) - 1)
            )
...
def score(args, trainer, dataset, src_dict, tgt_dict, ref_file):
    ...
    sacrebleu_score = sacrebleu.corpus_bleu(
        predictions, refs, lowercase=not args.test_cased_bleu
    ).score
    ...
    torch.cuda.synchronize()
    print(
        "| Eval completed in: {:.2f}s | {}CASED BLEU {:.2f}".format(
            time.time() - begin, "" if args.test_cased_bleu else "UN", sacrebleu_score
        )
    )
...
def _all_gather_predictions(predictions):
    ...
    while not all_ready:
        ...
        while n < lst_len:
            str_len = (
                len(predictions[n].encode("utf8")) + 8
            )  # per string pickle overhead
...
def save_checkpoint(args, trainer, epoch_itr, val_loss):
    ...
    checkpoints = [
        os.path.join(args.save_dir, "checkpoints", fn)
