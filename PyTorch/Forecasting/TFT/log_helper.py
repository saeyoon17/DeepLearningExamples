...
class TensorBoardBackend(Backend):
    ...
    def log(self, timestamp, elapsedtime, step, data):
        if step == "HPARAMS":
            parameters = {
                k: v for k, v in data.items() if not isinstance(v, (list, tuple))
            }
            # Unpack list and tuples
            for d in [
                {k + f"_{i}": v for i, v in enumerate(l)}
        ...
        if step == ():
            if self.hp_cache is None:
                print(
                    "Warning: Cannot save HParameters. Please log HParameters with step='HPARAMS'",
                    file=sys.stderr,
                )
                return
...
def setup_logger(args):
    ...
    if os.path.exists(log_path):
        for i in itertools.count():
            ...
            fname = (
                ".".join(s_fname[:-1]) + f"_{i}." + s_fname[-1]
                if len(s_fname) > 1
                else args.stat_file + f".{i}"
            )
    ...
    def metric_format(metric, metadata, value):
        return "{}: {}".format(
            metric, f"{value:.5f}" if isinstance(value, float) else value
        )
    ...
    if (
        ...
        or not args.distributed_world_size > 1
        or args.distributed_rank == 0
    ):
        dllogger.init(
            backends=[
                ...
                StdOutBackend(
                    verbosity=2, step_format=step_format, prefix_format=lambda x: ""
                )  # ,
    ...
    dllogger.metadata(
        "loss", {"GOAL": "MINIMIZE", "STAGE": "TRAIN", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "P10", {"GOAL": "MINIMIZE", "STAGE": "TRAIN", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "P50", {"GOAL": "MINIMIZE", "STAGE": "TRAIN", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "P90", {"GOAL": "MINIMIZE", "STAGE": "TRAIN", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "items/s",
        {"GOAL": "MAXIMIZE", "STAGE": "TRAIN", "format": ":1f", "unit": "items/s"},
    )
    dllogger.metadata(
        "val_loss", {"GOAL": "MINIMIZE", "STAGE": "VAL", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "val_P10", {"GOAL": "MINIMIZE", "STAGE": "VAL", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "val_P50", {"GOAL": "MINIMIZE", "STAGE": "VAL", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "val_P90", {"GOAL": "MINIMIZE", "STAGE": "VAL", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "val_items/s",
        {"GOAL": "MAXIMIZE", "STAGE": "VAL", "format": ":1f", "unit": "items/s"},
    )
    dllogger.metadata(
        "test_P10", {"GOAL": "MINIMIZE", "STAGE": "TEST", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "test_P50", {"GOAL": "MINIMIZE", "STAGE": "TEST", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "test_P90", {"GOAL": "MINIMIZE", "STAGE": "TEST", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "sum", {"GOAL": "MINIMIZE", "STAGE": "TEST", "format": ":5f", "unit": None}
    )
    dllogger.metadata(
        "throughput",
        {"GOAL": "MAXIMIZE", "STAGE": "TEST", "format": ":1f", "unit": "items/s"},
    )
    dllogger.metadata(
        "latency_avg",
        {"GOAL": "MIMIMIZE", "STAGE": "TEST", "format": ":5f", "unit": "s"},
    )
    dllogger.metadata(
        "latency_p90",
        {"GOAL": "MIMIMIZE", "STAGE": "TEST", "format": ":5f", "unit": "s"},
...
def get_system_info():
    system_info = subprocess.run(
        "nvidia-smi --query-gpu=gpu_name,memory.total,enforced.power.limit --format=csv".split(),
        capture_output=True,
    ).stdout
