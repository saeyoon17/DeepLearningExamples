...
def predict(
    args, config, model, data_loader, scalers, cat_encodings, extend_targets=False
):
    model.eval()
    predictions = []
    targets = []
    ...
    n_workers = (
        args.distributed_world_size if hasattr(args, "distributed_world_size") else 1
    )
    ...
    if config.scale_per_id:
        ...
        unscaled_predictions = np.stack(
            [
                _unscale_per_id(config, predictions[:, :, i], ids, scalers)
                for i in range(len(config.quantiles))
            ],
            axis=-1,
        )
        unscaled_targets = np.expand_dims(
            _unscale_per_id(config, targets[:, :, 0], ids, scalers), axis=-1
        )
    else:
        ids = None
        unscaled_predictions = np.stack(
            [
                _unscale(config, predictions[:, :, i], scalers[""])
                for i in range(len(config.quantiles))
            ],
            axis=-1,
        )
        unscaled_targets = np.expand_dims(
            _unscale(config, targets[:, :, 0], scalers[""]), axis=-1
        )
...
def visualize_v2(args, config, model, data_loader, scalers, cat_encodings):
    unscaled_predictions, unscaled_targets, ids, _ = predict(
        args, config, model, data_loader, scalers, cat_encodings, extend_targets=True
    )

    num_horizons = config.example_length - config.encoder_length + 1
    pad = unscaled_predictions.new_full(
        (
            unscaled_targets.shape[0],
            unscaled_targets.shape[1] - unscaled_predictions.shape[1],
            unscaled_predictions.shape[2],
        ),
    ...
    for key, g in graphs.items():
        for i, ex in enumerate(g):
            ...
            os.makedirs(
                os.path.join(args.results, "single_example_vis", str(key)),
                exist_ok=True,
            )
            fig.savefig(
                os.path.join(args.results, "single_example_vis", str(key), f"{i}.pdf")
            )


def inference(args, config, model, data_loader, scalers, cat_encodings):
    unscaled_predictions, unscaled_targets, ids, perf_meter = predict(
        args, config, model, data_loader, scalers, cat_encodings
    )
        ...
        for key, g in graphs.items():  # timeseries id, joint targets and predictions
            ...
            if args.joint_visualization:
                summary_writer = SummaryWriter(
                    log_dir=os.path.join(args.results, "predictions_vis", str(key))
                )
                for q, t in _g.items():  # target and quantiles, timehorizon values
                    ...
                    y = t.new_full(
                        (t.shape[0] + t.shape[1] - 1, t.shape[1]), float("nan")
                    )
                    ...
                    for i, vals in enumerate(y):  # timestep, timehorizon values value
                        summary_writer.add_scalars(
                            str(key),
                            {f"{q}_t+{j+1}": v for j, v in enumerate(vals) if v == v},
                            i,
                        )
            ...
            if args.save_predictions:
                for q, t in _g.items():
                    ...
                    os.makedirs(
                        os.path.join(args.results, "predictions", str(key)),
                        exist_ok=True,
                    )
                    df.to_csv(
                        os.path.join(args.results, "predictions", str(key), q + ".csv")
                    )
...
def main(args):
    ...
    quantiles, perf_dict = inference(
        args, config, model, data_loader, scalers, cat_encodings
    )
    quantiles = {
        "test_p10": quantiles[0].item(),
        "test_p50": quantiles[1].item(),
        "test_p90": quantiles[2].item(),
        "sum": sum(quantiles).item(),
    }
    finish_log = {**quantiles, **perf_dict}
    dllogger.log(step=(), data=finish_log, verbosity=1)
    print(
        "Test q-risk: P10 {test_p10} | P50 {test_p50} | P90 {test_p90}".format(
            **quantiles
        )
    )
    print(
        "Latency:\n\tAverage {:.3f}s\n\tp90 {:.3f}s\n\tp95 {:.3f}s\n\tp99 {:.3f}s".format(
            perf_dict["latency_avg"],
            perf_dict["latency_p90"],
            perf_dict["latency_p95"],
            perf_dict["latency_p99"],
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", type=str, help="Path to the test split of the dataset"
    )
    parser.add_argument(
        "--tgt_scalers",
        type=str,
    ...
    parser.add_argument(
        "--visualize",
        action="store_true",
        help="Visualize predictions - each example on the separate plot",
    )
    parser.add_argument(
        "--joint_visualization",
        action="store_true",
    ...
    parser.add_argument(
        "--disable_benchmark", action="store_true", help="Disable benchmarking mode"
    )
