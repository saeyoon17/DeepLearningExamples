...
NGC_CHECKPOINT_URLS[
    "electricity"
] = "https://api.ngc.nvidia.com/v2/models/nvidia/dle/tft_base_pyt_ckpt_ds-electricity/versions/22.11.0_amp/zip"
NGC_CHECKPOINT_URLS[
    "traffic"
] = "https://api.ngc.nvidia.com/v2/models/nvidia/dle/tft_base_pyt_ckpt_ds-traffic/versions/22.11.0_amp/zip"
...
def nvidia_tft_data_utils(**kwargs):
    ...
    class Processing:
        ...
        def preprocess(path):
            ...
            preprocess(
                os.path.join(path, "raw/electricity/standarized.csv"),
                os.path.join(path, "processed/electricity_bin/"),
                config,
            )
        ...
        def get_batch(path):
            config = ElectricityConfig()
            test_split = TFTDataset(
                os.path.join(path, "processed/electricity_bin/", "test.csv"), config
            )
