...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        validate(
            args,
            config,
            model_ema if args.ema_decay else model,
            criterion,
            valid_loader,
            global_step,
        )
    ...
    state_dict = torch.load(
        os.path.join(args.results, "checkpoint.pt"), map_location="cpu"
    )
    ...
    tgt_scalers = pickle.load(
        open(os.path.join(args.data_path, "tgt_scalers.bin"), "rb")
    )
    cat_encodings = pickle.load(
        open(os.path.join(args.data_path, "cat_encodings.bin"), "rb")
    )

    unscaled_predictions, unscaled_targets, _, _ = predict(
        args, config, model, test_loader, tgt_scalers, cat_encodings
    )
    ...
    finish_log = {
        **quantiles,
        "average_ips": perf_meter.avg,
        "convergence_step": validate.conv_step,
    }
...
def validate(args, config, model, criterion, dataloader, global_step):
    ...
    for batch in dataloader:
        with torch.jit.fuser("fuser2"), amp.autocast(
    ...
    p_losses = sum([l[0] * l[1] for l in losses]) / sum(
        [l[1] for l in losses]
    )  # takes into accunt that the last batch is not full
    if args.distributed_world_size > 1:
        dist.all_reduce(p_losses)
    ...
    if log_dict["loss"] < validate.best_valid_loss:
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            state_dict = (
                ...
                if isinstance(model, (DDP, ModelEma))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epochs", type=int, default=25, help="Default number of training epochs"
    )
    parser.add_argument(
        "--sample_data",
    ...
    parser.add_argument(
        "--ema_decay", type=float, default=0.0, help="Use exponential moving average"
    )
    parser.add_argument(
