...
from utils.exp_utils import (AverageMeter, TimeoutHandler, benchmark,
                             create_exp_dir, l2_promote, log_env_info,
...
def parse_args():
    ...
    general.add_argument(
        "--work_dir", default="LM-TFM", type=str, help="Directory for the results"
    )
    general.add_argument(
        "--append_dataset",
        action="store_true",
    ...
    general.add_argument(
        "--cuda", action="store_true", help="Run training on a GPU using CUDA"
    )
    general.add_argument(
        "--fp16", action="store_true", help="Run training in fp16/mixed precision"
    )
    general.add_argument(
        "--restart",
        type=str,
    ...
    general.add_argument(
        "--txtlog_file",
        type=str,
        default="train_log.log",
        help="Name of the txt log file",
    )
    general.add_argument("--save_all", action="store_true", help="Save all checkpoints")
    general.add_argument(
        "--no_env", action="store_true", help="Do not print info on execution env"
    )
    general.add_argument(
        "--no_eval", action="store_true", help="Disable model evaluation"
    )
    general.add_argument(
        "--no_test", action="store_true", help="Disable model evaluation on test data"
    )
    ...
    model.add_argument("--n_layer", type=int, default=16, help="Number of total layers")
    ...
    model.add_argument("--d_embed", type=int, default=-1, help="Embedding dimension")
    ...
    model.add_argument("--adaptive", action="store_true", help="Use adaptive softmax")
    model.add_argument(
        "--div_val",
        type=int,
        default=1,
    ...
    opt.add_argument("--lr", type=float, default=0.01, help="Initial learning rate")
    ...
    opt.add_argument(
        "--weight_decay", type=float, default=0.0, help="Weight decay for adam|lamb"
    )
    opt.add_argument(
        "--clip_nonemb",
        action="store_true",
    ...
    training.add_argument(
        "--max_step", type=int, default=40000, help="Max number of training steps"
    )
    training.add_argument(
        "--batch_size", type=int, default=256, help="Global batch size"
    )
    training.add_argument(
        "--local_batch_size",
        type=int,
    ...
    training.add_argument(
        "--tgt_len", type=int, default=192, help="Number of tokens to predict"
    )
    training.add_argument(
        "--ext_len", type=int, default=0, help="Length of the extended context"
    )
    training.add_argument(
        "--mem_len", type=int, default=192, help="Length of the retained previous heads"
    )
    ...
    training.add_argument("--varlen", action="store_true", help="Use variable length")
    training.add_argument(
        "--swap_mem", action="store_true", help="Swap memory tensors to cpu"
    )
    ...
    val.add_argument("--eval_batch_size", type=int, default=16, help="Eval batch size")
    ...
    if args.mem_len == 0:
        if args.eval_tgt_len > args.ext_len + args.tgt_len:
            raise RuntimeError(
                "eval_tgt_len should be <= tgt_len + ext_len; "
        ...
        if args.eval_tgt_len > args.mem_len + args.tgt_len:
            raise RuntimeError(
                "eval_tgt_len should be <= tgt_len + mem_len; "
                f"eval_tgt_len: {args.eval_tgt_len}, "
                f"tgt_len: {args.tgt_len}, "
    ...
    if (
        ...
        raise RuntimeError("Local batch size needs to be divisible by " "batch chunk")
...
def save_checkpoint(
    ...
    memory = [utils.distributed.all_gather_tensors(mem, device) for mem in mems]
...
def evaluate(eval_iter, model, args):
    ...
    model.reset_length(tgt_len=args.tgt_len, ext_len=args.ext_len, mem_len=args.mem_len)
    model.train()
...
def train(
    ...
    for batch, (data, target, seq_len, _) in enumerate(
        ...
        for i in range(args.batch_chunk):
            if i < args.batch_chunk - 1 and isinstance(
        ...
        if train_step % args.log_interval == 0:
            ...
            log_str = (
                "| epoch {:3d} step {:>8d} | batches {:>6d} / {:d} | lr {:.3e} "
...
def main():
    ...
    if args.local_batch_size is not None:
        ...
        logging.info(
            f"--local_batch_size was set, adjusting global batch size"
    ...
    if args.optim.lower() == "sgd":
        ...
        if args.sample_softmax > 0:
            ...
            optimizer = optim.Adam(
                dense_params, lr=args.lr, weight_decay=args.weight_decay
            )
        else:
            optimizer = optim.Adam(
    ...
    if args.scheduler == "cosine":
        ...
        if args.sample_softmax > 0 and optimizer_sparse is not None:
            scheduler_sparse = optim.lr_scheduler.CosineAnnealingLR(
                optimizer_sparse, max_step - args.warmup_step, eta_min=args.eta_min
            )
        else:
        ...
        if args.sample_softmax > 0 and optimizer_sparse is not None:
            scheduler_sparse = optim.lr_scheduler.LambdaLR(
                optimizer_sparse, lr_lambda=lr_lambda
            )
        else:
            scheduler_sparse = None
    ...
    if args.restart:
        try:
            ...
            utils.exp_utils.set_default_rng_states(checkpoint["rng_states"], device)
            ...
            logging.info(
                f"Could not load checkpoint from {args.restart}, "
    ...
    if (
        ...
        if args.dataset in ["enwik8", "text8"]:
            logging.info(
                "| End of training | test time: {:5.2f}s | test loss {:5.2f} | test bpc {:9.5f}".format(
                    test_elapsed, test_loss, test_loss / math.log(2)
                )
            )
        else:
            logging.info(
                "| End of training | test time: {:5.2f}s | test loss {:5.2f} | test ppl {:9.3f}".format(
