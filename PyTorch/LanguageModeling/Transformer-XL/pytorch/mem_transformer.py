        ...
        if div_val == 1:
            ...
            if d_proj != d_embed:
                self.emb_projs.append(
                    nn.Parameter(torch.Tensor(d_proj, d_embed).zero_())
                )
        else:
            for i in range(len(self.cutoffs)):
                ...
                self.emb_projs.append(
                    nn.Parameter(torch.Tensor(d_proj, d_emb_i).zero_())
                )
...
class MemTransformerLM(nn.Module):
    ...
    def _create_params(self):
        ...
        # learnable
            ...
            self.r_w_bias = nn.Parameter(
                torch.Tensor(self.n_layer, self.n_head, self.d_head).zero_()
            )
            self.r_bias = nn.Parameter(
                torch.Tensor(self.n_layer, self.max_klen, self.n_head).zero_()
            )
        # absolute standard
        ...
        # absolute deeper SA
            ...
            self.r_emb = nn.Parameter(
                torch.Tensor(self.n_layer, self.max_klen, self.d_model).zero_()
            )
    ...
    def init_mems(self):
        if self.mem_len > 0:
            param = next(self.parameters())
            mems = torch.empty(self.n_layer, 0, dtype=param.dtype, device=param.device)
            return mems
        else:
            return None
