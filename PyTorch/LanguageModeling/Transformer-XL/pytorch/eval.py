...
import utils
import yaml
from data_utils import get_lm_corpus, tokenize_raw
from utils.exp_utils import (AverageMeter, benchmark, create_exp_dir,
...
def parse_args():
    ...
    parser.add_argument(
        "--tgt_len", type=int, default=64, help="number of tokens to predict"
    )
    parser.add_argument(
        "--ext_len", type=int, default=0, help="length of the extended context"
    )
    parser.add_argument(
        "--mem_len", type=int, default=640, help="length of the retained previous heads"
    )
    ...
    parser.add_argument(
        "--cuda", action="store_true", help="Run evaluation on a GPU using CUDA"
    )
    parser.add_argument("--model", type=str, default="", help="path to the checkpoint")
    parser.add_argument(
        "--manual_config",
        type=json.loads,
        default=None,
    ...
    parser.add_argument(
        "--target_perplexity", type=float, default=None, help="target perplexity"
    )
    parser.add_argument(
        "--target_throughput", type=float, default=None, help="target throughput"
    )
    parser.add_argument(
        "--save_data",
        action="store_true",
    ...
    parser.add_argument(
        "--repeat", type=int, default=1, help="loop over the dataset REPEAT times"
    )
    parser.add_argument(
        "--max_size",
        type=int,
    ...
    if args.same_length and args.tgt_len > args.mem_len:
        warnings.warn(
            "--same_length is intended to be used with large "
...
def evaluate(eval_iter, model, device, meters, log_interval, max_size=None, repeat=1):
...
def main():
    ...
    if args.manual:
        ...
        iter = data_utils.LMOrderedIterator(
            tensor,
            bsz=args.batch_size,
            bptt=args.tgt_len,
            device=device,
    ...
    if args.type == "torchscript" and not args.manual_config:
        ...
        for i in range(len(model.crit.out_projs)):
            ...
            if model.crit.out_projs[i] is not None:
                if tie_projs[i]:
                    model.crit.out_projs[i] = state[f"word_emb.emb_projs.{src}"].to(
                        dtype
                    )
                else:
        ...
        for i in range(len(model.crit.out_layers_biases)):
            model.crit.out_layers_biases[i] = state[f"crit.out_layers_biases.{i}"].to(
                dtype
            )
        ...
        if tie_weight:
            for i in range(len(model.crit.out_layers_weights)):
                model.crit.out_layers_weights[i] = state[
                    f"word_emb.emb_layers.{i}.weight"
                ].to(dtype)
        else:
            for i in range(len(model.crit.out_layers_weights)):
                model.crit.out_layers_weights[i] = state[
                    f"crit.out_layers_weights.{i}"
                ].to(dtype)
    ...
    logging.info(
        f"Evaluating with: math {math_str} type {args.type} "
    ...
    loss = evaluate(
        iter, model, device, meters, args.log_interval, args.max_size, args.repeat
    )
