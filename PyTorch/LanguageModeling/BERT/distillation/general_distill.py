...
from hooks import *
from losses import *
from modeling import (CONFIG_NAME, WEIGHTS_NAME, BertForPreTraining, BertModel,
                      Project)
...
def main():
    ...
    teacher_model, teacher_config = BertModel.from_pretrained(
        args.teacher_model, distill_config=args.distill_config
    )
    ...
    if args.continue_train:
        student_model, student_config = BertForPreTraining.from_pretrained(
            args.student_model, distill_config=args.distill_config
        )
    else:
        student_model, student_config = BertForPreTraining.from_scratch(
            args.student_model, distill_config=args.distill_config
        )
    ...
    if is_main_process():
        print(
            "get_bert_pretrain_data_loader took {} s!".format(time.perf_counter() - tic)
        )
    train_dataloader = (
    ...
    tr_loss, tr_att_loss, tr_rep_loss, tr_value_loss = 0.0, 0.0, 0.0, 0.0
    ...
    while global_step < args.max_steps:
        for batch in train_dataloader:
            ...
            if global_step % args.steps_per_epoch == 0:
                tr_loss, tr_att_loss, tr_rep_loss, tr_value_loss = 0.0, 0.0, 0.0, 0.0
                mean_loss, mean_att_loss, mean_rep_loss, mean_value_loss = (
                    0.0,
                    0.0,
                    0.0,
                    0.0,
            ...
            with torch.cuda.amp.autocast(enabled=args.amp):
                ...
                # MiniLM
                if (
                    student_config.distillation_config["student_teacher_layer_mapping"]
                    == "last_layer"
                ):
                    ...
                    if student_config.distillation_config["use_value_states"]:
                        new_teacher_values = [
                            teacher_values[i * layers_per_block + layers_per_block - 1]
                            for i in range(student_layer_num)
                        ]

                    if student_config.distillation_config["use_hidden_states"]:
                        new_teacher_reps = [
                            teacher_reps[i * layers_per_block + layers_per_block - 1]
                            for i in range(student_layer_num)
                        ]
                ...
                if student_config.distillation_config["use_value_states"]:
                    value_loss = transformer_losses.compute_loss(
                        student_values, new_teacher_values, loss_name="value_state_loss"
                    )
            ...
            if local_step % args.gradient_accumulation_steps == 0:
                ...
                global_step = (
                    ...
                    if "step" in optimizer.param_groups[0]
                    else 0
                )
                ...
                if (global_step + 1) % args.eval_step == 0 and is_main_process():
                    ...
                    if is_main_process():
                        ...
                        if use_projection:
                            project_to_save = (
                                project.module
                                if hasattr(project, "module")
                                else project
                            )

                        output_model_file = os.path.join(args.output_dir, model_name)
                        output_config_file = os.path.join(args.output_dir, CONFIG_NAME)
                        output_project_file = os.path.join(
                            args.output_dir, "project.bin"
                        )
                        torch.save(model_to_save.state_dict(), output_model_file)
                        if use_projection:
                            torch.save(
                                project_to_save.state_dict(), output_project_file
                            )
