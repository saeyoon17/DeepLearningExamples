        ...
        while ptr < len(all_tokens):
            ...
            for (idx, word) in enumerate(tokens):
                ...
                if (
                    ...
                    and word not in blacklist_tokens
                ):
...
class AugmentProcessor(object):
    ...
    def read_augment_write(self):
        ...
        if "SQuADv2.0" in self.task_name:
            raise ValueError(
                "Data augmentation not implemented for task: %s" % self.task_name
            )
        if "SQuAD" in self.task_name:
            train_samples = json.load(
                open(
                    os.path.join(self.glue_dir, "train-v1.1.json"),
                    "r",
                    encoding="utf-8",
                )
            )
        ...
        if "SQuAD" in self.task_name:
            for it, entry in enumerate(train_samples["data"]):
                ...
                for i, paragraph in enumerate(entry["paragraphs"]):
                    ...
                    if "context" in self.augment_ids[self.task_name]:
                        ...
                        augmented_paragraph_contexts = self.augmentor.augment(
                            paragraph["context"], all_answers
                        )[1:]

                        for augmented_paragraph_context in augmented_paragraph_contexts:

                            good_context = True
                            entry_aug["paragraphs"][i][
                                "context"
                            ] = augmented_paragraph_context
                            # fix indices of start position.
                            for qas_id, qa in enumerate(
                                entry_aug["paragraphs"][i]["qas"]
                            ):
                                ...
                                answer_tokens = (
                                    self.augmentor.tokenizer.basic_tokenizer.tokenize(
                                        qa["answers"][0]["text"]
                                    )
                                )
                                entry_aug["paragraphs"][i]["qas"][qas_id]["answers"][0][
                                    "text"
                                ] = " ".join(answer_tokens)
                                # correct answer start based on new context
                                answer_start_index = augmented_paragraph_context.find(
                                    " ".join(answer_tokens)
                                )
                                if answer_start_index == -1:
                                    logger.info(
                                        'Answer: "{}" not found in Context "{}"'.format(
                                            " ".join(answer_tokens),
                                            augmented_paragraph_context,
                                        )
                                    )
                                    ...
                                    entry_aug["paragraphs"][i]["qas"][qas_id][
                                        "answers"
                                    ][0]["answer_start"] = answer_start_index
                            if good_context:
                                train_samples_aug["data"][it]["paragraphs"].append(
                                    copy.deepcopy(entry_aug["paragraphs"][i])
                                )
                    ...
                    if "question" in self.augment_ids[self.task_name]:
                        for qas_id, qa in enumerate(paragraph["qas"]):
                            # ignore first since it is the original sample
                            augmented_question_texts = self.augmentor.augment(
                                qa["question"]
                            )[1:]
                            for augmented_question_text in augmented_question_texts:
                                entry_aug["paragraphs"][i]["qas"][qas_id][
                                    "question"
                                ] = augmented_question_text
                                train_samples_aug["data"][it]["paragraphs"][i][
                                    "qas"
                                ].append(
                                    copy.deepcopy(
                                        entry_aug["paragraphs"][i]["qas"][qas_id]
                    ...
                    logger.info(
                        "Having been processing {} paragraphs".format(str(i + 1))
                    )
            ...
            with open(output_filename, "w", newline="", encoding="utf-8") as f:
                ...
                for (i, line) in enumerate(train_samples):
                    ...
                    if (i + 1) % 1000 == 0:
                        logger.info(
                            "Having been processing {} examples".format(str(i + 1))
                        )
