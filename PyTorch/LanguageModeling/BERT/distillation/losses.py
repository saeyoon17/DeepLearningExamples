...
import torch
from torch.nn import CosineEmbeddingLoss, KLDivLoss, MSELoss
...
class TransformerLosses:
    ...
    def compute_loss_(self, pred, target, loss_name):
        ...
        if self.distill_config[loss_name] == "mse":
            ...
            seq_length = (
                pred.size(0) if loss_name == "value_state_loss" else pred.size(-1)
            )
            if loss_name == "value_state_loss":
                ...
                student_values = pred.view(
                    seq_length,
                    self.batch_size * self.student_config.num_attention_heads,
                    dk_student,
                )
                student_values = student_values.transpose(0, 1)
                teacher_values = target.view(
                    seq_length,
                    self.batch_size * self.teacher_config.num_attention_heads,
                    dk_teacher,
                )
                ...
                ) / math.sqrt(dk_student)
                target = torch.bmm(
                    teacher_values, teacher_values.transpose(1, 2)
                ) / math.sqrt(dk_teacher)

                pred = pred.view(
                    self.batch_size,
                    self.student_config.num_attention_heads,
                    seq_length,
                    seq_length,
                )
                target = target.view(
                    self.batch_size,
                    self.teacher_config.num_attention_heads,
                    seq_length,
                    seq_length,
                )

            return self.kl_loss(
                torch.nn.LogSoftmax(dim=-1)(pred), torch.nn.Softmax(dim=-1)(target)
            ...
            return self.cosine_loss(
                pred.view(-1, self.teacher_config.hidden_size),
    ...
    def compute_loss(self, pred, target, loss_name):
        ...
        for student, teacher in zip(pred, target):
            if loss_name == "attention_loss":
                student = torch.where(
                    student <= -1e2, torch.zeros_like(student).to(self.device), student
                )
                teacher = torch.where(
                    teacher <= -1e2, torch.zeros_like(teacher).to(self.device), teacher
                )
