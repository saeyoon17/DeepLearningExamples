...
from modeling import (CONFIG_NAME, WEIGHTS_NAME, BertConfig,
                      BertForQuestionAnswering)
from run_squad import (_compute_softmax, _get_best_indices, get_answer_text,
...
def preprocess_tokenized_text(
    doc_tokens, query_tokens, tokenizer, max_seq_length, max_query_length
):
...
def get_answer(doc_tokens, tokens_for_postprocessing, start_logits, end_logits, args):
    ...
    for pred in prelim_predictions:
        ...
        if pred.end_index > 0:  # this is a non-null prediction
            final_text = get_answer_text(
                doc_tokens_obj, tokens_for_postprocessing, pred, args
            )
...
def main():
    ...
    feature = preprocess_tokenized_text(
        doc_tokens,
        query_tokens,
        tokenizer,
    ...
    input_ids = torch.tensor(
        tensors_for_inference.input_ids, dtype=torch.long
    ).unsqueeze(0)
    segment_ids = torch.tensor(
        tensors_for_inference.segment_ids, dtype=torch.long
    ).unsqueeze(0)
    input_mask = torch.tensor(
        tensors_for_inference.input_mask, dtype=torch.long
    ).unsqueeze(0)
    ...
    answer, answers = get_answer(
        doc_tokens, tokens_for_postprocessing, start_logits, end_logits, args
    )
