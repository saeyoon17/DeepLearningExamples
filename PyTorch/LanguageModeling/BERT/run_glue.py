...
from utils import (format_step, get_world_size, is_main_process,
                   mkdir_by_main_process)
...
def parse_args(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument(
        "--do_eval",
        action="store_true",
        help="Whether to get model-task performance on the dev" " set by running eval.",
    )
    parser.add_argument(
        "--do_predict",
        action="store_true",
        help="Whether to output prediction results on the dev " "set by running eval.",
    )
    parser.add_argument(
        "--do_lower_case",
        action="store_true",
    ...
    parser.add_argument(
        "--eval_batch_size", default=8, type=int, help="Batch size per GPU for eval."
    )
    parser.add_argument(
        "--learning_rate",
        default=5e-5,
...
def init_optimizer_and_amp(
    ...
    if use_fp16:
        ...
        model, optimizer = (
            amp_inits if num_train_optimization_steps is not None else (amp_inits, None)
        )
        if num_train_optimization_steps is not None:
...
def main(args):
    ...
    if args.gradient_accumulation_steps > args.train_batch_size:
        raise ValueError(
            "gradient_accumulation_steps ({}) cannot be larger "
        ...
        results.update(
            {
                ...
                "train:loss": tr_loss,
                "train:latency": latency_train,
                "train:num_samples_per_gpu": nb_tr_examples,
                "train:num_steps": nb_tr_steps,
                "train:throughput": get_world_size() * nb_tr_examples / latency_train,
            }
        )
    ...
    if is_main_process():
        ...
        for key, query in dllogger_queries_from_results.items():
            results_key, convert = (
                query if isinstance(query, tuple) else (query, lambda x: x)
            )
