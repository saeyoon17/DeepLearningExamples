...
class SyncFreeStats:
    ...
    def add_stat(self, name, dtype=torch.int32, device_tensor=None, device_func=None):
        if device_tensor is not None:
            ...
            ), "Error: dtype do not match: {} {}".format(dtype, device_tensor.dtype)
    ...
    def copy_from_device(self):
        for name in self.host_stats.keys():
            ...
            if (
                self.device_stats[name] is not None
                and self.device_funcs[name] is not None
            ):
                self.host_stats[name].copy_(
                    self.device_funcs[name](self.device_stats[name]), non_blocking=True
                )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--log_freq", type=float, default=1.0, help="frequency of logging loss."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--resume_step", type=int, default=-1, help="Step to resume training from."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--profile-start", default=0, type=int, help="Delay profiling to start step."
    )
    parser.add_argument(
        "--num_workers",
        type=int,
    ...
    parser.add_argument(
        "--cuda_graphs", default=False, action="store_true", help="Enable Cuda Graphs."
    )
...
def prepare_model_and_optimizer(args, device, sequence_output_is_dense):
    ...
    optimizer = FusedLAMBAMP(optimizer_grouped_parameters, lr=args.learning_rate)
    ...
    if args.local_rank != -1:
        ...
        def scale_by_grad_accum_steps_wrapper(
            ...
            def scale_by_grad_accum_steps_wrapper_hook(
                ...
                bucket.set_buffer(
                    bucket.buffer().div_(args.gradient_accumulation_steps)
                )
    ...
    if (
        (args.resume_from_checkpoint and not args.phase2)
        or (args.resume_phase2)
...
def checkpoint_step(
    args, epoch, global_step, model, optimizer, grad_scaler, last3_checkpoint_paths
):
...
def take_training_step(args, grad_scaler, model, criterion, batch, stats):
    with torch.cuda.amp.autocast(
        enabled=(args.fp16 and not args.allreduce_post_accumulation_fp16)
    ):
...
def main():
    ...
    if is_main_process():
        ...
        dllogger.log(
            step="PARAMETER", data={"batch_size_per_gpu": args.train_batch_size}
        )
    ...
    if grad_scaler.is_enabled():
        ...
        stats.add_stat(
            "skip_optimizer_step",
            dtype=torch.float32,
            device_func=(lambda: grad_scaler._found_inf_per_device(optimizer)[device]),
        )
        stats.add_stat(
            ...
            device_tensor=torch.zeros(1, dtype=torch.float32, device=device),
            device_func=(
                lambda x: x.add_(grad_scaler._found_inf_per_device(optimizer)[device])
            ),
        )
    else:
    ...
    if args.cuda_graphs:
        static_gpu_batch = {
            ...
            "next_sentence_labels": torch.ones(
                args.train_batch_size, dtype=torch.int64, device=device
            ),
        }
        ...
        with torch.cuda.stream(side_stream):
            for _ in range(11):
                ...
                take_optimizer_step(
                    args, lr_scheduler, optimizer, grad_scaler, device, stats
                )
        torch.cuda.current_stream().wait_stream(side_stream)
    ...
    while True:
        for step, batch in enumerate(train_iter):
            ...
            grad_accumulation_step = (
                stats.host_stat_value("model_step") % args.gradient_accumulation_steps
            ) != 0
            ...
            if (not grad_accumulation_step) or timeout_sent:
                ...
                if (
                    static_optimizer_step + global_resume_step >= args.steps_this_run
                    or timeout_sent
                ):
                    torch.cuda.synchronize()
                    dynamic_optimizer_step = (
                    ...
                    if dynamic_optimizer_step >= args.steps_this_run or timeout_sent:
                        ...
                        last_num_steps = (
                            args.log_freq if no_log_steps == 0 else no_log_steps
                        )
                ...
                if no_log_steps == 0:
                    if is_main_process():
                        dllogger.log(
                            ...
                            data={
                                "average_loss": stats.host_stat_value("average_loss")
                        ...
                        if stats.host_stat_value("skip_optimizer_step") > 0.0:
                            dllogger.log(
                                step="PARAMETER",
                                data={
                    ...
                    if not args.skip_checkpoint and (
                        dynamic_optimizer_step % args.num_steps_per_checkpoint == 0
                    ):
                        checkpoint_step(
                            args,
                            epoch,
...
if __name__ == "__main__":
    ...
    if is_main_process():
        ...
        training_perf = (
            args.train_batch_size
            * gpu_count
            * (stats.host_stat_value("model_step") - skip_fwd_bwd_for_perf)
            / train_time_raw
        )
        dllogger.log(
            step=tuple(),
