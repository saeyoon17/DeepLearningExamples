...
def _collate_batch(examples, tokenizer, masks=None, max_length=None):
    ...
    are_tensors_same_length = all(x.size(0) == length_of_first for x in examples) and (
    ...
    max_length = (
        max_length if max_length is not None else max(x.size(0) for x in examples)
    )
...
class DataCollatorForBART(DataCollatorForLanguageModeling):
    ...
    def __call__(
        ...
        if input_attention_mask is None:
            ...
            batch_input, input_attention_mask = _collate_batch(
                input_ids, self.tokenizer, input_attention_mask
            )
        ...
        batch["decoder_input_ids"] = shift_tokens_right(
            batch_input, self.tokenizer.pad_token_id, self.decoder_start_token_id
        )
        ...
        batch_mask = _collate_batch(mask_labels, self.tokenizer)
        batch_input, input_attention_mask = self.mask_tokens_span(
            batch_input, batch_mask, input_attention_mask
        )

        # Collate to max_length to match decoder inputs and labels
        if input_attention_mask is None:
            batch["input_ids"] = _collate_batch(
                batch_input, self.tokenizer, max_length=max_length
            )
        else:
            batch["input_ids"], batch["attention_mask"] = _collate_batch(
                batch_input, self.tokenizer, input_attention_mask, max_length
            )
    ...
    def _whole_word_mask(self, input_tokens: List[str], max_predictions=512):
        ...
        for (i, token) in enumerate(input_tokens):
            ...
            if len(cand_indexes) >= 1 and (
                not token.startswith("Ġ") or token.startswith("##")
            ):  # @TODO hf error in start with token?
                cand_indexes[-1].append(i)
            else:
                cand_indexes.append([i])
    ...
    def mask_tokens_span(
        self, inputs: torch.Tensor, mask_labels: torch.Tensor, attention_mask
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original. Set
        'mask_labels' means we use whole word mask (wwm), we directly mask idxs according to it's ref.
        ...
        mask = torch.cat(
            (torch.full((inputs.shape[0], 1), True), mask_left_shift[:, :-1]), dim=-1
        ) | torch.not_equal(inputs, mask_token_id)
