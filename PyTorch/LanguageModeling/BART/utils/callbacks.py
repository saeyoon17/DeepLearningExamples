...
class Seq2SeqLoggingCallback(pl.Callback):
    ...
    def on_train_batch_end(
        ...
        self.train_time_epoch_list.append(
            time.time() - self.t0
        )  # Measures ~time for forward + backward + optimizer_step
    ...
    def on_train_epoch_end(self, trainer, pl_module, outputs):
        ...
        try:
            ...
            (
                ...
                train_batches,
                avg_steps_per_sec,
            ) = self.process_stats(self.train_time_epoch_list, outputs)
            pl_module.log(
                "train_throughput",
                unpadded_tokens / train_time,
                on_step=False,
                on_epoch=True,
                prog_bar=True,
                logger=True,
            ...
            # Accumulate
            self.tokens = ((self.tokens * self.epochs) + all_reduce_tokens) / (
                self.epochs + 1
            )
            self.train_time = ((self.train_time * self.epochs) + all_reduce_time) / (
                self.epochs + 1
            )
            self.avg_steps_per_sec = (
                (self.avg_steps_per_sec * self.epochs) + all_reduce_avg_steps_per_sec
            ) / (self.epochs + 1.0)
            self.epochs += 1
            ...
            print(
                "Train time is reported as 0? It's possible training is already complete!"
            )
            pass
