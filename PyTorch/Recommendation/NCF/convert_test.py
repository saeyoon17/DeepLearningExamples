...
import pandas as pd
import torch
from convert import (TEST_0, TEST_1, TRAIN_0, TRAIN_1, _TestNegSampler,
                     save_feature_spec)
...
def parse_args():
    ...
    parser.add_argument(
        "--seed", "-s", type=int, default=1, help="Manually set random seed for torch"
    )
    parser.add_argument(
        "--test", type=str, help="select modification to be applied to the set"
    )
...
def main():
    ...
    if args.test == "more_user":
        ...
        df = df[
            users.isin(users[users.duplicated(keep=False)])
        ]  # make sure something remains in the train set
    ...
    test_users_pos = test_ratings[
        :, 0:1
    ]  # slicing instead of indexing to keep dimensions
    test_items_pos = test_ratings[:, 1:2]
    test_users = test_users_pos.repeat_interleave(args.valid_negative + 1, dim=0)
    test_items = torch.cat((test_items_pos.reshape(-1, 1), test_negs), dim=1).reshape(
        -1, 1
    )
    positive_labels = torch.ones_like(test_users_pos, dtype=torch.float32)
    negative_labels = torch.zeros_like(test_users_pos, dtype=torch.float32).repeat(
