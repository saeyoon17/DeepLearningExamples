...
def parse_args():
    ...
    parser.add_argument(
        "--beta2", "-b2", type=float, default=0.5, help="Beta1 for Adam"
    )
...
def val_epoch(
    model, dataloader: dataloading.TestDataLoader, k, distributed=False, world_size=1
):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for batch_dict in dataloader.get_epoch_data():
            ...
            loss_batch = torch.nn.functional.binary_cross_entropy(
                input=prediction_batch.reshape([-1]), target=label_batch
            )
        ...
        ignore_mask = dataloader.get_ignore_mask().view(
            -1, dataloader.samples_in_series
        )
        ...
        ndcg = (
            math.log(2)
            / (torch.nonzero(ifzero)[:, 1].view(-1).to(torch.float) + 2).log_()
        ).sum()
    ...
    if distributed:
        ...
        torch.distributed.all_reduce(
            total_validation_loss, op=torch.distributed.ReduceOp.SUM
        )
...
def main():
    ...
    dllogger.metadata(
        "validation_loss", {"name": "validation_loss", "unit": None, "format": ":.5f"}
    )
    dllogger.metadata(
        "train_loss", {"name": "train_loss", "unit": None, "format": ":.5f"}
    )
    ...
    trainset = dataloading.TorchTensorDataset(
        feature_spec, mapping_name="train", args=args
    )
    testset = dataloading.TorchTensorDataset(
        feature_spec, mapping_name="test", args=args
    )
    ...
    traced_criterion = torch.jit.trace(
        criterion.forward, (torch.rand(local_batch, 1), torch.rand(local_batch, 1))
    )
