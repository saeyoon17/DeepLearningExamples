...
def get_acc_table(arch, numgpus, fullprec):
    headers = [
        ...
        f"Time to train - {fullprec}",
        "Time to train - mixed precision",
        f"Time to train speedup ({fullprec} to mixed precision)",
    ]
    table = []
    for numgpus in numgpus:
        data_full = [
            get_training_data(
                arch.format(numgpus=numgpus, num_run=num_run, precision=fullprec)
            )
            for num_run in range(1, 21)
        ]
        data_mixed = [
            get_training_data(
                arch.format(numgpus=numgpus, num_run=num_run, precision=amp)
            )
            for num_run in range(1, 21)
        ]
