...
import numpy as np
import torch
from dlrm.data.defaults import (CATEGORICAL_CHANNEL, DTYPE_SELECTOR,
                                FEATURES_SELECTOR, FILES_SELECTOR,
                                LABEL_CHANNEL, NUMERICAL_CHANNEL)
...
class ParametricDataset(Dataset):
    def __init__(
        ...
        categorical_features_to_read: List[
            str
        ] = None,  # This parameter dictates order of returned features
        ...
        **kwargs
        ...
        if len(categorical_features_to_read) > 0:
            self._categorical_features_files = [
                categorical_feature_files[feature]
                for feature in categorical_features_to_read
            ]
            ...
            self._categorical_types = [
                feature_spec.feature_spec[feature][DTYPE_SELECTOR]
                for feature in categorical_features_to_read
            ]
