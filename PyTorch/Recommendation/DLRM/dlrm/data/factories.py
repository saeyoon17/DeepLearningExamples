...
import torch
from dlrm.data.datasets import ParametricDataset, SyntheticDataset
...
from dlrm.utils.distributed import get_rank, is_distributed
...
class DatasetFactory:
    ...
    def create_sampler(self, dataset: Dataset) -> Optional[Sampler]:
        return (
            RandomDistributedSampler(dataset)
            if is_distributed()
...
class SyntheticGpuDatasetFactory(DatasetFactory):
    def __init__(
        self, flags, local_numerical_features_num, local_categorical_feature_sizes
    ):
...
class ParametricDatasetFactory(DatasetFactory):
    ...
    def create_collate_fn(self):
        orig_stream = (
            torch.cuda.current_stream() if self._base_device == "cuda" else None
        )
...
def create_dataset_factory(
    ...
    if is_distributed() or device_mapping:
        ...
        ), "Distributed dataset requires information about model device mapping."
        rank = get_rank()
    ...
    if dataset_type == "synthetic_gpu":
        local_numerical_features = (
            num_numerical_features if numerical_features_enabled else 0
        )
