...
from dlrm.utils.checkpointing.distributed import \
    make_distributed_checkpoint_loader
...
def get_model_args(model_args):
    ...
    parser.add_argument(
        "--embedding_type", type=str, default="joint", choices=["joint", "multi_table"]
    )
    parser.add_argument(
        "--top_mlp_sizes", type=int, nargs="+", default=[1024, 1024, 512, 256, 1]
    )
    parser.add_argument(
        "--bottom_mlp_sizes", type=int, nargs="+", default=[512, 256, 128]
    )
    parser.add_argument(
        "--interaction_op", type=str, default="dot", choices=["dot", "cat"]
    )
...
def initialize_model(args, categorical_sizes, device_mapping):
    ...
    if args.model_checkpoint:
        checkpoint_loader = make_distributed_checkpoint_loader(
            device_mapping=device_mapping, rank=0
        )
...
def get_dataloader(args, categorical_sizes):
    ...
    test_loader = torch.utils.data.DataLoader(
        RemoveOutput(dataset_test), batch_size=None, num_workers=0, pin_memory=False
    )
...
def main():
    ...
    deployer, model_args = deployer_lib.create_deployer(sys.argv[1:], get_model_args)
    ...
    if model_args.dump_perf_data:
        ...
        input_1.detach().cpu().numpy()[0].tofile(
            os.path.join(model_args.dump_perf_data, "input__1")
        )
